...
int make_nbp(int nn, int* active, double* center, double* width, double* nlow,
    double* nhigh, double minwidth, int autoincr, double flow, double fhigh,
    double* bplow, double* bphigh, int* havnotch) {
    int nbp;
    int nnbp, adds;
    int i, j, k;
    double nl, nh;
...
void calc_nbp_lightweight(NBP a) { // calculate and set new impulse response;
                                   // used when changing tune freq or shift freq
    int i;
    double fl, fh;
    double offset;
    NOTCHDB b = *a->ptraddr;
    if (a->fnfrun) {
        ...
        fl = a->flow + offset;
        fh = a->fhigh + offset;
        a->numpb = make_nbp(b->nn, b->active, b->fcenter, b->fwidth, b->nlow,
            b->nhigh, min_notch_width(a), a->autoincr, fl, fh, a->bplow,
            a->bphigh, &a->havnotch);
        // when tuning, no need to recalc filter if there were not and are not
        // any notches in passband
...
void calc_nbp_impulse(NBP a) { // calculates impulse response; for
                               // create_fircore() and parameter changes
    int i;
    double fl, fh;
    double offset;
    NOTCHDB b = *a->ptraddr;
    if (a->fnfrun) {
        ...
        a->impulse = fir_bandpass(a->nc, a->flow, a->fhigh, a->rate, a->wintype,
            1, a->gain / (double)(2 * a->size));
    }
}

NBP create_nbp(int run, int fnfrun, int position, int size, int nc, int mp,
    double* in, double* out, double flow, double fhigh, int rate, int wintype,
    double gain, int autoincr, int maxpb, NOTCHDB* ptraddr) {
...
void UpdateNBPFiltersLightWeight(
    int channel) { // called when setting tune freq or shift freq
...
PORT int RXANBPAddNotch(
    int channel, int notch, double fcenter, double fwidth, int active) {
    NOTCHDB b;
    int i, j;
    int rval;
    b = rxa[channel].ndb.p;
...
PORT int RXANBPDeleteNotch(int channel, int notch) {
    int i, j;
    int rval;
    NOTCHDB a;
    a = rxa[channel].ndb.p;
...
PORT void RXANBPGetNumNotches(int channel, int* nnotches) {
    NOTCHDB a;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].ndb.p;
    *nnotches = a->nn;
...
PORT void RXANBPSetTuneFrequency(int channel, double tunefreq) {
    NOTCHDB a;
    a = rxa[channel].ndb.p;
...
PORT void RXANBPSetNotchesRun(int channel, int run) {
    ...
    if (run != a->master_run) {
        ...
        EnterCriticalSection(
            &ch[channel].csDSP); // block DSP channel processing
...
PORT void RXANBPSetRun(int channel, int run) {
    NBP a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void RXANBPSetFreqs(int channel, double flow, double fhigh) {
    NBP a;
    a = rxa[channel].nbp0.p;
...
PORT void RXANBPSetWindow(int channel, int wintype) {
    NBP a;
    BPSNBA b;
    a = rxa[channel].nbp0.p;
...
PORT void RXANBPSetNC(int channel, int nc) {
...
PORT void RXANBPSetMP(int channel, int mp) {
    NBP a;
    a = rxa[channel].nbp0.p;
    if (a->mp != mp) {
        a->mp = mp;
...
PORT void RXANBPGetMinNotchWidth(int channel, double* minwidth) {
    NBP a;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].nbp0.p;
...
PORT void RXANBPSetAutoIncrease(int channel, int autoincr) {
    NBP a;
    BPSNBA b;
    a = rxa[channel].nbp0.p;
    b = rxa[channel].bpsnba.p;
