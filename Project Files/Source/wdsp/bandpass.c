...
void SetRXABPSFreqs (int channel, double f_low, double f_high)
{
    ...
    if ((f_low != a1->f_low) || (f_high != a1->f_high))
    {
        ...
        impulse = fir_bandpass(a1->size + 1, f_low, f_high, a1->samplerate,
a1->wintype, 1, 1.0 / (double)(2 * a1->size)); a1->mults = fftcv_mults (2 *
a1->size, impulse); _aligned_free (impulse);
    }
    LeaveCriticalSection (&ch[channel].csDSP);
}
...
void SetRXABPSWindow (int channel, int wintype)
{
    ...
    if ((a1->wintype != wintype))
    {
        a1->wintype = wintype;
        _aligned_free (a1->mults);
        impulse = fir_bandpass(a1->size + 1, a1->f_low, a1->f_high,
a1->samplerate, a1->wintype, 1, 1.0 / (double)(2 * a1->size)); a1->mults =
fftcv_mults (2 * a1->size, impulse); _aligned_free (impulse);
    }
    LeaveCriticalSection (&ch[channel].csDSP);
}
*/
...
void SetTXABPSFreqs (int channel, double f_low, double f_high)
{
    ...
    if ((f_low != a->f_low) || (f_high != a->f_high))
    {
        ...
        a->f_high = f_high;
        _aligned_free (a->mults);
        impulse = fir_bandpass(a->size + 1, f_low, f_high, a->samplerate,
a->wintype, 1, 1.0 / (double)(2 * a->size)); a->mults = fftcv_mults (2 *
a->size, impulse); _aligned_free (impulse);
    }
    a = txa[channel].bp1.p;
    if ((f_low != a->f_low) || (f_high != a->f_high))
    {
...
BANDPASS create_bandpass(int run, int position, int size, int nc, int mp,
    double* in, double* out, double f_low, double f_high, int samplerate,
    int wintype, double gain) {
...
PORT void SetRXABandpassFreqs(int channel, double f_low, double f_high) {
    double* impulse;
    BANDPASS a = rxa[channel].bp1.p;
    if ((f_low != a->f_low) || (f_high != a->f_high)) {
        impulse = fir_bandpass(a->nc, f_low, f_high, a->samplerate, a->wintype,
...
PORT void SetRXABandpassWindow(int channel, int wintype) {
    double* impulse;
    BANDPASS a = rxa[channel].bp1.p;
    if ((a->wintype != wintype)) {
        impulse = fir_bandpass(a->nc, a->f_low, a->f_high, a->samplerate,
...
PORT void SetRXABandpassNC(int channel, int nc) {
    // NOTE:  'nc' must be >= 'size'
    double* impulse;
    BANDPASS a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXABandpassMP(int channel, int mp) {
    BANDPASS a;
    a = rxa[channel].bp1.p;
    if (mp != a->mp) {
        a->mp = mp;
...
PORT void SetTXABandpassNC(int channel, int nc) {
    // NOTE:  'nc' must be >= 'size'
    double* impulse;
    BANDPASS a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetTXABandpassMP(int channel, int mp) {
    BANDPASS a;
    a = txa[channel].bp0.p;
    if (mp != a->mp) {
        a->mp = mp;
