...
Copyright (C) 2012 David McQuate, WA8YWQ - Kaiser window & Bessel function
added.
...
void new_window(int disp, int type, int size, double PiAlpha) {
    ...
    switch (type) {
        ...
        case 1: // blackman-harris window (4 term)
        {
            ...
            for (i = 0; i < size; i++) {
                arg1 = arg0 * (double)i;
                a->window[i] = 0.35875 - 0.48829 * cos(arg1)
                    + 0.14128 * cos(2.0 * arg1) - 0.01168 * cos(3.0 * arg1);
                cgsum += a->window[i];
                igsum += a->window[i] * a->window[i];
            }
            a->inv_coherent_gain = (double)size / cgsum;
            for (i = 0; i < size; i++) a->window[i] *= a->inv_coherent_gain;
            break;
        }
        case 2: // hann window
        {
        ...
        case 3: // flat-top window
        {
            ...
            for (i = 0; i < size; i++) {
                arg1 = arg0 * (double)i;
                a->window[i] = 0.21557895 - 0.41663158 * cos(arg1)
                    + 0.277263158 * cos(2.0 * arg1)
                    - 0.083578947 * cos(3.0 * arg1)
                    + 0.006947368 * cos(4.0 * arg1);
                cgsum += a->window[i];
                igsum += a->window[i] * a->window[i];
            }
            a->inv_coherent_gain = (double)size / cgsum;
        ...
        case 5: // Kaiser window
        {
            ...
            for (i = 0; i < size; ++i) {
                a->window[i]
                    = bessi0(PiAlpha
                          * sqrt(1.0 - pow(2.0 * (double)i / arg1 - 1.0, 2)))
                    / arg0;
                cgsum += a->window[i];
                igsum += a->window[i] * a->window[i];
            }
                ...
                a->window[i] = +6.3964424114390378e-02
                    + arg1
                        * (-2.3993864599352804e-01
                            + arg1
                                * (+3.5015956323820469e-01
                                    + arg1
                                        * (-2.4774111897080783e-01
                                            + arg1
                                                * (+8.5438256055858031e-02
                                                    + arg1
                                                        * (-1.2320203369293225e-02
                                                            + arg1
    ...
    a->inv_enb = 1.0
        / (a->inherent_power_gain * a->inv_coherent_gain
            * a->inv_coherent_gain);
    // print_window_gain ("windows.txt", type, a->inv_coherent_gain,
    // a->inherent_power_gain);
}

// spur elimination, REAL input data
void eliminate(uintptr_t disp, int ss, int LO) {
    DP a = pdisp[disp];
    ...
    if (a->flip[LO])
        for (i = ilim - begin, k = 0; i > ilim - end; i--, k++) {
            mag = (a->fft_out[ss][LO])[i][0] * (a->fft_out[ss][LO])[i][0]
                + (a->fft_out[ss][LO])[i][1] * (a->fft_out[ss][LO])[i][1];
...
void Celiminate(uintptr_t disp, uintptr_t ss, uintptr_t LO) {
    DP a = pdisp[disp];
    ...
    if (pix_per_bin <= 1.0) {
        switch (det_type) {
            ...
            case 0: // positive peak
                for (i = 0; i < num_pixels; i++) pixels[i] = -1.0e300;
            ...
            case 1: // rosenfell
                ...
                for (i = 0; i < m; i++) // for each FFT bin
                {
                    ...
                    // update the minimum and maximum of the set of bins within
                    // the pixel
                    if (bins[i] < mini) mini = bins[i];
                    if (bins[i] > maxi) maxi = bins[i];
                    // if the next bin is also within the pixel && there is a
                    // next bin,
                    //    compare its value with the current bin and update rose
                    //    and fell
                    if (next_pix_count == pix_count && i < m - 1) {
                        // NOTE:  when next_pix_count != pix_count, rose and
                        // fell do not get updated;
                        //    that's OK because we do NOT need to know if
                        //    there's a rise or fall across bins
                        if (bins[i + 1] > bins[i]) rose = 1;
                        if (bins[i + 1] < bins[i]) fell = 1;
                    }
                    // if the next bin is NOT within the pixel || there is no
                    // next bin, finalize the pixel
            ...
            case 2: // rms - adjusted for window's equivalent noise bandwidth
                ...
                for (i = 0; i < m; i++) {
                    ...
                    if (pix_count == last_pix_count) {
                        ...
                        pixels[last_pix_count]
                            = psum / (double)bcount * inv_enb;
                        psum = bins[i];
                        bcount = 1;
                    }
                    if (i == m - 1) {
...
void avenger(int av_mode, // averaging mode
    ...
    switch (av_mode) {
        case -1: // peak-hold
        {
            for (i = 0; i < num_pixels; i++) {
                if (t_pixels[i] > av_sum[i]) av_sum[i] = t_pixels[i];
                pixels[i] = (dOUTREAL)(10.0
                    * mlog10(scale * cd[i] * av_sum[i] + 1.0e-60));
            }
            break;
        }
        case 0: // no averaging
        default: {
            for (i = 0; i < num_pixels; i++)
                pixels[i] = (dOUTREAL)(10.0
                    * mlog10(scale * cd[i] * t_pixels[i] + 1.0e-60));
            break;
        }
        ...
        case 2: // window averaging of linear data
        {
            if (*avail_frames < num_average) {
                ...
                for (i = 0; i < num_pixels; i++) {
                    ...
                    pixels[i] = (dOUTREAL)(10.0
                        * mlog10(cd[i] * av_sum[i] * factor + 1.0e-60));
                }
            } else {
                factor = scale / (double)(*avail_frames);
                for (i = 0; i < num_pixels; i++) {
                ...
                if (++(*av_out_idx) == dMAX_AVERAGE) *av_out_idx = 0;
            }
            if (++(*av_in_idx) == dMAX_AVERAGE) *av_in_idx = 0;
            break;
        }
        case 3: // weighted averaging of log data - looks nice, not accurate for
                // time-varying signals
        {
            double onem_avb = 1.0 - av_backmult;
            for (i = 0; i < num_pixels; i++) {
                av_sum[i] = av_backmult * av_sum[i]
                    + onem_avb
                        * (10.0 * mlog10(scale * cd[i] * t_pixels[i] + 1e-60));
    ...
    if (norm)
        for (i = 0; i < num_pixels; i++) pixels[i] += (dOUTREAL)norm_oneHz;
}

void stitch(uintptr_t disp) {
    DP a = pdisp[disp];
    int i, j, k, n, m;
    double* ptr;
    ...
    for (i = 0; i < a->num_pixout; i++) // for each output
    {
        ...
        // if a detection of the same 'det_type' has already been done, use that
        // result
        j = i - 1;
        k = i;
        while (j >= 0) {
            if (a->det_type[i] == a->det_type[j]) k = j;
            j--;
        }
        if (k == i)
            // detect
            detector(a->det_type[i], m, a->num_pixels, a->pix_per_bin,
                a->bin_per_pix, a->pre_av_out, a->t_pixels[i], a->inv_enb);
        else
            memcpy(
                a->t_pixels[i], a->t_pixels[k], a->num_pixels * sizeof(double));
        // average & convert to dBm
        avenger(a->av_mode[i], a->num_pixels, &a->avail_frames[i],
            a->num_average[i], &a->av_in_idx[i], &a->av_out_idx[i],
            a->av_backmult[i], a->scale, a->t_pixels[i], a->av_sum[i],
            a->av_buff[i], a->cd, a->normalize[i], a->norm_oneHz,
        ...
        while ((a->w_pix_buff[i] = (a->w_pix_buff[i] + 1) % dNUM_PIXEL_BUFFS)
            == a->r_pix_buff[i])
            ;
...
DWORD WINAPI spectra(void* pargs) {
    int i, j;
    uintptr_t disp = ((uintptr_t)pargs) >> 12;
...
DWORD WINAPI Cspectra(void* pargs) {
    int i, j;
    uintptr_t disp = ((uintptr_t)pargs) >> 12;
    uintptr_t ss = (((uintptr_t)pargs) >> 4) & 255;
    int LO = ((uintptr_t)pargs) & 15;
    ...
    if ((ss >= a->begin_ss) && (ss <= a->end_ss)) {
        for (i = 0; i < a->size; i++) {
            (a->Cfft_in[ss][LO])[i][0] = a->window[i]
                * (double)((a->I_samples[ss][LO])[a->IQO_idx[ss][LO]]);
            (a->Cfft_in[ss][LO])[i][1] = a->window[i]
                * (double)((a->Q_samples[ss][LO])[a->IQO_idx[ss][LO]]);
            if (++a->IQO_idx[ss][LO] >= a->bsize)
                a->IQO_idx[ss][LO] -= a->bsize;
        }
    ...
    if (InterlockedBitTestAndReset(&(a->snap[ss][LO]), 0)) {
        memcpy((char*)(a->snap_buff[ss][LO]),
            (char*)(a->fft_out[ss][LO]) + trans_size, trans_size);
        memcpy((char*)(a->snap_buff[ss][LO]) + trans_size,
            (char*)(a->fft_out[ss][LO]), trans_size);
...
void interpolate(int disp, int set, double fmin, double fmax, int num_pixels) {
    ...
    for (i = 0; i < num_pixels; i++) {
        ...
        mag = (((a->ac3[set][0])[k] * dx + (a->ac2[set][0])[k]) * dx
                  + (a->ac1[set][0])[k])
                * dx
            + (a->ac0[set][0])[k];
        a->cd[i] = mag * mag;
...
int build_interpolants(
    ...
    for (i = 1; i <= n - 2; i++) {
        ...
        for (j = 0; j < m; j++)
            d[i][j] = 6.0
                * ((y[i + 1][j] - y[i][j]) * idx[i]
                    - (y[i][j] - y[i - 1][j]) * idx[i - 1]);
    }

    b[1] = dmain[1];
    for (j = 0; j < m; j++) v[1][j] = d[1][j];

    for (i = 2; i <= n - 2; i++) {
        tmp = dsub[i] / b[i - 1];
        b[i] = dmain[i] - tmp * dsup[i - 1];
        for (j = 0; j < m; j++) v[i][j] = d[i][j] - tmp * v[i - 1][j];
    }

    for (j = 0; j < m; j++) S[n - 2][j] = v[n - 2][j] / b[n - 2];
    ...
    for (i = 0; i < n - 1; i++)
        for (j = 0; j < m; j++) {
            ...
            (a->ac1[set][j])[i] = (y[i + 1][j] - y[i][j]) * idx[i]
                - (2.0 * dx[i] * S[i][j] + dx[i] * S[i + 1][j]) / 6.0;
            (a->ac0[set][j])[i] = y[i][j];
        }
    return 0;
}
    ...
    while (!a->end_dispatcher) {
        for (a->ss = 0; a->ss < a->num_stitch; a->ss++)
            for (a->LO = 0; a->LO < a->num_fft; a->LO++) {
                if (!_InterlockedAnd(&(a->input_busy[a->ss][a->LO]), 1)
                    && _InterlockedAnd(&(a->buff_ready[a->ss][a->LO]), 1)) {
                    ...
                    EnterCriticalSection(
                        &(a->BufferControlSection[a->ss][a->LO]));
                    if ((a->have_samples[a->ss][a->LO] -= a->incr) < a->size)
                        InterlockedBitTestAndReset(
                            &(a->buff_ready[a->ss][a->LO]), 0);
...
PORT void SetAnalyzer(int disp, // display identifier
    ...
    int* flp, // vector with one elt for each LO frequency, 1 if high-side LO, 0
              // otherwise
    int sz, // size of the fft, i.e., number of input samples
    int bf_sz, // number of samples transferred for each
    ...
    int ovrlp, // number of samples each fft (other than the first) is to re-use
               // from the previous
    int clp, // number of fft output bins to be clipped from EACH side of each
             // sub-span
    ...
    int n_pix, // number of pixel values to return.  may be either <= or >
               // number of bins
    ...
    a->end_dispatcher = 1;
    while (a->dispatcher) Sleep(1);
    a->stop = 1;
    while (_InterlockedAnd(a->pnum_threads, 1023)) Sleep(1);
    a->num_pixout = n_pixout;
    a->num_fft = n_fft;
    a->type = typ;
    a->buff_size = bf_sz;
    for (i = 0; i < a->num_fft; i++) a->flip[i] = *(flp + i);
    a->overlap = ovrlp;
    a->clip = clp;
    a->fsclipL = fscLin;
    a->fsclipH = fscHin;
    ...
    if (sz != a->size) {
        for (i = 0; i < a->max_stitch; i++)
            for (j = 0; j < a->max_num_fft; j++) {
                ...
                a->plan[i][j] = fftw_plan_dft_r2c_1d(
                    sz, a->fft_in[i][j], a->fft_out[i][j], FFTW_PATIENT);
                a->Cplan[i][j] = fftw_plan_dft_1d(sz, a->Cfft_in[i][j],
                    a->fft_out[i][j], FFTW_FORWARD, FFTW_PATIENT);
            }
    }
    ...
    if (((fmin != a->f_min) || (fmax != a->f_max))
        && ((fmin == 0.0) && (fmax == 0.0)))
        for (i = 0; i < dMAX_PIXELS; i++) a->cd[i] = 1.0;

    if (((fmax != 0.0) || (fmin != 0.0))
        && ((n_pix != a->num_pixels) || (fmin != a->f_min) || (fmax != a->f_max)
    ...
    a->pix_per_bin = (double)a->num_pixels
        / (double)(a->num_stitch * (a->out_size - 2 * a->clip) - a->fsclipL
            - a->fsclipH);
    a->bin_per_pix = (double)(a->num_stitch * (a->out_size - 2 * a->clip) - 1
                         - a->fsclipL - a->fsclipH)
        / (double)a->num_pixels;

    for (i = 0; i < dMAX_STITCH; i++)
        for (j = 0; j < dMAX_NUM_FFT; j++) a->input_busy[i][j] = 0;

    for (i = 0; i < dMAX_STITCH; i++) a->spec_flag[i] = 0;
    a->stitch_flag = 0;
    for (i = 0; i < dMAX_PIXOUTS; i++) {
        a->w_pix_buff[i] = 0;
        a->r_pix_buff[i] = 0;
        a->last_pix_buff[i] = 0;
        for (j = 0; j < dNUM_PIXEL_BUFFS; j++) a->pb_ready[i][j] = 0;
    }
    a->ss = 0;
    a->LO = 0;
    for (i = 0; i < dMAX_STITCH; i++)
...
PORT void XCreateAnalyzer(int disp, int* success, int m_size, int m_num_fft,
    ...
    for (i = 0; i < a->max_stitch; i++)
        for (j = 0; j < a->max_num_fft; j++) {
            ...
            a->Cfft_in[i][j] = (fftw_complex*)fftw_malloc(
                sizeof(fftw_complex) * a->max_size);
            a->fft_out[i][j] = (fftw_complex*)fftw_malloc(
                sizeof(fftw_complex) * a->max_size);
        }
    a->pre_av_sum
        = (double*)malloc0(sizeof(double) * a->max_size * a->max_stitch);
    a->pre_av_out
        = (double*)malloc0(sizeof(double) * a->max_size * a->max_stitch);
    for (i = 0; i < dMAX_PIXOUTS; i++) {
        a->det_type[i] = 0;
        ...
        for (j = 0; j < dNUM_PIXEL_BUFFS; j++)
            a->pixels[i][j]
                = (dOUTREAL*)malloc0(sizeof(dOUTREAL) * dMAX_PIXELS);
    }

    a->cd = (double*)malloc0(sizeof(double) * dMAX_PIXELS);
    for (j = 0; j < dMAX_PIXELS; j++) a->cd[j] = 1.0;
...
PORT void DestroyAnalyzer(int disp) {
    ...
    for (i = 0; i < dMAX_PIXOUTS; i++) {
        for (j = 0; j < dNUM_PIXEL_BUFFS; j++) _aligned_free(a->pixels[i][j]);
        _aligned_free(a->t_pixels[i]);
        for (j = 0; j < dMAX_AVERAGE; j++) _aligned_free(a->av_buff[i][j]);
    ...
    for (i = 0; i < a->max_stitch; i++) _aligned_free(a->result[i]);
    ...
    for (i = 0; i < a->max_stitch; i++)
        for (j = 0; j < a->max_num_fft; j++) CloseHandle(a->hSnapEvent[i][j]);
...
PORT void GetPixels(int disp, int pixout,
    ...
    if (_InterlockedAnd(&(a->pb_ready[pixout][a->r_pix_buff[pixout]]), 1)) {
        memcpy(pix, a->pixels[pixout][a->r_pix_buff[pixout]],
            a->num_pixels * sizeof(dOUTREAL));
        *flag = 1;
        InterlockedBitTestAndReset(
            &(a->pb_ready[pixout][a->r_pix_buff[pixout]]), 0);
    } else
        *flag = 0;
}

PORT void SnapSpectrum(int disp, int ss, int LO, double* snap_buff) {
    DP a = pdisp[disp];
...
PORT void SetCalibration(int disp,
      ...
      //	 data for variables being calibrated
{
    ...
    for (i = 0; i < n_points; i++) {
        if ((i == n_points - 1) || (cal[i][0] != cal[i + 1][0])) {
            ...
            for (j = 0; j < dMAX_M; j++) y[k][j] = cal[i][j + 1];
            k++;
        }
    }
    ...
    build_interpolants(
        disp, set_num, a->n_freqs[set_num], dMAX_M, a->freqs[set_num], y);
    a->cal_changed = 1;
}

PORT void OpenBuffer(
    int disp, int ss, int LO, void** Ipointer, void** Qpointer) {
    DP a = pdisp[disp];
    EnterCriticalSection(&a->SetAnalyzerSection);
...
PORT void CloseBuffer(int disp, int ss, int LO) {
    DP a = pdisp[disp];
    EnterCriticalSection(&a->SetAnalyzerSection);
    EnterCriticalSection(&(a->BufferControlSection[ss][LO]));
    if (a->have_samples[ss][LO] > a->max_writeahead) {
        // if we're receiving samples too much faster than we're consuming them,
        // skip some
        if ((a->IQout_index[ss][LO]
    ...
    if ((a->IQin_index[ss][LO] += a->buff_size)
        >= a->bsize) // REQUIRES buff_size IS A SUB-MULTIPLE OF SIZE OF INPUT
                     // SAMPLE BUFFS!
...
PORT void Spectrum2(int run, int disp, int ss, int LO, dINREAL* pbuff) {
    if (run) {
        int i;
        dINREAL* Ipointer;
        dINREAL* Qpointer;
        ...
        EnterCriticalSection(&(a->BufferControlSection[ss][LO]));
        if (a->have_samples[ss][LO] > a->max_writeahead) {
            // if we're receiving samples too much faster than we're consuming
            // them, skip some
            if ((a->IQout_index[ss][LO]
                    += a->have_samples[ss][LO] - a->max_writeahead)
                >= a->bsize)
        ...
        LeaveCriticalSection(&(a->BufferControlSection[ss][LO]));
        if ((a->IQin_index[ss][LO] += a->buff_size)
            >= a->bsize) // REQUIRES buff_size IS A SUB-MULTIPLE OF SIZE OF
                         // INPUT SAMPLE BUFFS!
            a->IQin_index[ss][LO] = 0;
...
PORT void Spectrum0(int run, int disp, int ss, int LO, double* pbuff) {
    if (run) {
        int i;
        dINREAL* Ipointer;
        dINREAL* Qpointer;
    ...
    if (a->av_mode[pixout] != mode) {
        ...
        switch (mode) {
            ...
            case 2:
                ...
                break;
            case 3:
                for (i = 0; i < dMAX_PIXELS; i++) a->av_sum[pixout][i] = -120.0;
                break;
            default:
                memset(
                    (void*)a->av_sum[pixout], 0, sizeof(double) * dMAX_PIXELS);
                break;
        }
        LeaveCriticalSection(&a->ResampleSection);
    }
...
PORT void SetDisplaySampleRate(int disp, int rate) {
    DP a = pdisp[disp];
...
PORT void SetDisplayNormOneHz(int disp, int pixout, int norm) {
    DP a = pdisp[disp];
