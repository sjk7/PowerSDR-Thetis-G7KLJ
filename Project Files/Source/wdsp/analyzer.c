    ...
    switch (av_mode) {
        case -1: // peak-hold
        {
            for (i = 0; i < num_pixels; i++) {
                ...
                pixels[i] = (dOUTREAL)(
                    10.0 * mlog10(scale * cd[i] * av_sum[i] + 1.0e-60));
            }
            break;
        }
        case 0: // no averaging
        default: {
            for (i = 0; i < num_pixels; i++)
                pixels[i] = (dOUTREAL)(
                    10.0 * mlog10(scale * cd[i] * t_pixels[i] + 1.0e-60));
            break;
        }
        ...
        case 2: // window averaging of linear data
        {
            if (*avail_frames < num_average) {
                ...
                for (i = 0; i < num_pixels; i++) {
                    ...
                    pixels[i] = (dOUTREAL)(
                        10.0 * mlog10(cd[i] * av_sum[i] * factor + 1.0e-60));
                }
            } else {
                factor = scale / (double)(*avail_frames);
                for (i = 0; i < num_pixels; i++) {
        ...
        for (i = 0; i < a->max_stitch; i++)
            for (j = 0; j < a->max_num_fft; j++) {
                ...
                a->plan[i][j] = fftw_plan_dft_r2c_1d(
                    sz, a->fft_in[i][j], a->fft_out[i][j], MY_PATIENCE);
                a->Cplan[i][j] = fftw_plan_dft_1d(sz, a->Cfft_in[i][j],
                    a->fft_out[i][j], FFTW_FORWARD, MY_PATIENCE);
            }
    }
