...
typedef struct _fmsq {
    ...
    double* trigger; // buffer used to trigger mute/unmute (may be same as
                     // input; matches timing of input buffer)
...
extern FMSQ create_fmsq(int run, int size, double* insig, double* outsig,
...
extern __declspec(dllexport) void SetRXAFMSQThreshold(
