...
void calc_icfir(ICFIR a) {
    ...
    impulse = icfir_impulse(a->nc, a->DD, a->R, a->Pairs, a->runrate,
        a->cicrate, a->cutoff, a->xtype, a->xbw, 1, a->scale, a->wintype);
...
ICFIR create_icfir(int run, int size, int nc, int mp, double* in, double* out,
    int runrate, int cicrate, int DD, int R, int Pairs, double cutoff,
    int xtype, double xbw, int wintype)
...
double* icfir_impulse(int N, int DD, int R, int Pairs, double runrate,
    ...
    // Pairs:	number of comb-integrator pairs in the CIC
    // runrate:	sample rate at which this filter is to run (assumes there may be
    // flat interp. between this filter and the CIC) cicrate: sample rate at
    // interface to CIC cutoff:	cutoff frequency xtype:	transition type, 0 for
    // 4th-power rolloff, 1 for raised cosine xbw:		transition bandwidth for
    // raised cosine rtype:	0 for real output, 1 for complex output scale:
    // scale factor to be applied to the output
    int i, j;
    ...
    int u_samps
        = (N + 1) / 2; // number of unique samples,  OK for odd or even N
    int c_samps = (int)(cutoff / runrate * N) + (N + 1) / 2
        - N / 2; // number of unique samples within bandpass, OK for odd or even
                 // N
    int x_samps
        = (int)(xbw / runrate * N); // number of unique samples in transition
                                    // region, OK for odd or even N
    double offset = 0.5
        - 0.5
            * (double)((N + 1) / 2
                - N / 2); // sample offset from center, OK for odd or even N
    ...
    if (xtype == 0) {
        for (i = 0, ri = offset; i < u_samps; i++, ri += 1.0) {
            ...
            if (fn <= ft) {
                if (fn == 0.0)
                    tmp = 1.0;
                else if ((tmp = sin(PI * DD * fn) / (DD * R * sin(PI * fn / R)))
                    < 0.0)
                    tmp = -tmp;
    ...
    if (N & 1)
        for (i = u_samps, j = 2; i < N; i++, j++) A[i] = A[u_samps - j];
    else
        for (i = u_samps, j = 1; i < N; i++, j++) A[i] = A[u_samps - j];
