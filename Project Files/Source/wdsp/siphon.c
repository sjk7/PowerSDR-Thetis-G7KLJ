...
void build_window(SIPHON a) {
    ...
    scale = 1.0 / sum;
    for (i = 0; i < a->fftsize; i++) a->window[i] *= scale;
}
    ...
    if (a->run && a->position == pos) {
        switch (a->mode) {
            case 0:
                ...
                break;
            case 1: Spectrum0(1, a->disp, 0, 0, a->in); break;
        }
    }
...
PORT void RXAGetaSipF(
    int channel, float* out, int size) { // return raw samples as floats
...
PORT void TXASetSipPosition(int channel, int pos) {
...
PORT void TXASetSipMode(int channel, int mode) {
    SIPHON a = txa[channel].sip1.p;
    EnterCriticalSection(&a->update);
    a->mode = mode;
    LeaveCriticalSection(&a->update);
...
PORT void TXASetSipDisplay(int channel, int disp) {
    SIPHON a = txa[channel].sip1.p;
    EnterCriticalSection(&a->update);
    a->disp = disp;
    LeaveCriticalSection(&a->update);
...
PORT void TXASetSipSpecmode(int channel, int mode) {
    SIPHON a = txa[channel].sip1.p;
    if (mode == 0)
...
PORT void TXAGetSpecF1(
    int channel, float* out) { // return spectrum magnitudes in dB
    SIPHON a = txa[channel].sip1.p;
    int i, j, mid, m, n;
    EnterCriticalSection(&a->update);
    a->outsize = a->fftsize;
    ...
    if (!InterlockedAnd(&a->specmode, 1))
        ...
        for (i = 0, j = mid; i < mid; i++, j++) {
            out[i] = (float)(10.0
                * mlog10(a->specout[2 * j + 0] * a->specout[2 * j + 0]
                    + a->specout[2 * j + 1] * a->specout[2 * j + 1] + 1.0e-60));
            out[j] = (float)(10.0
                * mlog10(a->specout[2 * i + 0] * a->specout[2 * i + 0]
                    + a->specout[2 * i + 1] * a->specout[2 * i + 1] + 1.0e-60));
        }
    else
        // mirror each half of the spectrum in-place
        for (i = 0, j = mid - 1, m = mid, n = a->fftsize - 1; i < mid;
             i++, j--, m++, n--) {
            out[i] = (float)(10.0
                * mlog10(a->specout[2 * j + 0] * a->specout[2 * j + 0]
                    + a->specout[2 * j + 1] * a->specout[2 * j + 1] + 1.0e-60));
            out[m] = (float)(10.0
                * mlog10(a->specout[2 * n + 0] * a->specout[2 * n + 0]
                    + a->specout[2 * n + 1] * a->specout[2 * n + 1] + 1.0e-60));
        }
}
...
#define MAX_EXT_SIPHONS                                                        \
    (2) // maximum number of Siphons called from outside wdsp
__declspec(
    align(16)) SIPHON psiphon[MAX_EXT_SIPHONS]; // array of pointers for Siphons
                                                // used EXTERNAL to wdsp

PORT void create_siphonEXT(
    int id, int run, int insize, int sipsize, int fftsize, int specmode) {
    psiphon[id]
        = create_siphon(run, 0, 0, 0, insize, 0, sipsize, fftsize, specmode);
}

PORT void destroy_siphonEXT(int id) {
    destroy_siphon(psiphon[id]);
}

PORT void flush_siphonEXT(int id) {
    flush_siphon(psiphon[id]);
}

PORT void xsiphonEXT(int id, double* buff) {
    SIPHON a = psiphon[id];
    a->in = buff;
    xsiphon(a, 0);
}

PORT void GetaSipF1EXT(
    int id, float* out, int size) { // return raw samples as floats
    SIPHON a = psiphon[id];
    int i;
    EnterCriticalSection(&a->update);
    a->outsize = size;
...
PORT void SetSiphonInsize(int id, int size) {
    SIPHON a = psiphon[id];
    EnterCriticalSection(&a->update);
    a->insize = size;
    LeaveCriticalSection(&a->update);
