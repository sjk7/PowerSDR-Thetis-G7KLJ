...
typedef struct //_iob
{
    ...
    int r1_outsize; // number of complex samples taken out of the
                    // input-pseudo-ring for processing
    int r2_insize; // number of processed complex samples returned into the
                   // output-pseudo-ring
    ...
    int r1_unqueuedsamps; // number of input samples not yet queued/released for
                          // execution
    ...
    int r2_unqueuedsamps; // number of output samples not yet queued / released
                          // for output
    CRITICAL_SECTION r2_ControlSection;

    int bfo; // block_for_output, wait until output is available before
             // proceeding
    HANDLE Sem_OutReady; // count = number of 'out_size' buffers processed and
                         // available for output
    HANDLE Sem_BuffReady; // count = number of 'dsp_size' buffers queued for
                          // processing
...
PORT // separate I/Q buffers
    extern void
    fexchange2(int channel, INREAL* Iin, INREAL* Qin, OUTREAL* Iout,
        OUTREAL* Qout, int* error);
