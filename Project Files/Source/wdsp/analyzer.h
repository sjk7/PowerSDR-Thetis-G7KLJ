...
typedef struct _dp {
    ...
    int max_stitch; // maximum number of sub-spans to be concatenated
                    //		NOTE:  max_size, max_num_fft, and max_stitch MUST BE
                    //<= THE 		CORRESPONDING VALUES IN <analyzer.h>!!
    ...
    int flip[dMAX_NUM_FFT]; // 0 for low-side LO => do NOT flip; 1 for high-side
                            // LO => FLIP
    ...
    int ss_bins[dMAX_STITCH]; // number of bins delivered by
                              // eliminate()/Celiminate in each sub-span
    volatile LONG input_busy[dMAX_STITCH][dMAX_NUM_FFT];
    int num_pixels; // number of pixels requested
    int num_stitch; // number of results to be stitched together to generate the
                    // pixel frame
    unsigned long long stitch_flag;
    int spec_flag[dMAX_STITCH]; // flags showing if all ffts for a sub-span are
                                // done so elimination can proceed
    double pix_per_bin; // number of pixels per fft bin, note that this is
                        // fractional, not integral
    double bin_per_pix; // number of fft bins per pixel, this is fractional and
                        // != 1.0/pix_per_bin
    ...
    double* result[dMAX_STITCH]; // pointers to buffer to hold elimination
                                 // results for each sub-span
    dOUTREAL* pixels[dMAX_PIXOUTS]
                    [dNUM_PIXEL_BUFFS]; // pointers pixel output buffers
    double* t_pixels[dMAX_PIXOUTS]; // pointer to temporary pixel buffer
                                    // //pointer to temporary pixel buffer for
                                    // non-averaged data
    int w_pix_buff[dMAX_PIXOUTS]; // number of pixel buffer owned by writing
                                  // process
    int r_pix_buff[dMAX_PIXOUTS]; // number of pixel buffer owned by reading
                                  // process
    int last_pix_buff[dMAX_PIXOUTS]; // number of the last pixel buffer written
    volatile LONG
        pb_ready[dMAX_PIXOUTS]
                [dNUM_PIXEL_BUFFS]; // if value is 0, this data has already been
                                    // read; 1 = fresh data to read
    int num_average[dMAX_PIXOUTS]; // number of spans to average to create the
                                   // pixels
    int avail_frames[dMAX_PIXOUTS]; // number of pixel frames currently
    ...
    double* av_buff[dMAX_PIXOUTS]
                   [dMAX_AVERAGE]; // pointers to ring of buffers to hold pixel
    ...
    double*
        freqs[dMAX_CAL_SETS]; // pointers to vectors of calibration frequencies
    double(*ac3[dMAX_CAL_SETS]
               [dMAX_M]); // pointers to amplitude interpolant coefficients
    ...
    double* fft_in[dMAX_STITCH]
                  [dMAX_NUM_FFT]; // pointers to fftw real input vectors
    fftw_complex* Cfft_in[dMAX_STITCH][dMAX_NUM_FFT]; // pointers to fftw
                                                      // complex input vectors
    fftw_complex* fft_out[dMAX_STITCH][dMAX_NUM_FFT]; // pointers to fftw
    ...
    int flag;
    int have_samples[dMAX_STITCH][dMAX_NUM_FFT]; // number of unused samples
                                                 // remaining in a buffer
    int type; // 0 for REAL, 1 for COMPLEX
    int incr; // size - overlap
    int buff_size; // amount of data to be stored each time an input buffer is
                   // opened and closed = JanusAudio/BlockSize
    dINREAL* I_samples[dMAX_STITCH][dMAX_NUM_FFT]; // pointers to current input
                                                   // position in I/Q buffers
    dINREAL* Q_samples[dMAX_STITCH][dMAX_NUM_FFT];
    int bsize; // size of I_samples[][] and Q_samples[][] (number of samples
               // they hold)
    int IQout_index[dMAX_STITCH]
                   [dMAX_NUM_FFT]; // current output index for I_samples[ss][LO]
                                   // and Q_samples[ss][LO]
    int IQO_idx[dMAX_STITCH][dMAX_NUM_FFT];
    int IQin_index[dMAX_STITCH]
                  [dMAX_NUM_FFT]; // current input index for I_samples[ss][LO]
                                  // and Q_samples[ss][LO]
    volatile LONG buff_ready[dMAX_STITCH]
                            [dMAX_NUM_FFT]; // 1 if buffer ready to read; 0 if
                                            // needs to be filled
    int max_writeahead; // max allowed input samples ahead of where reading
                        // output samples

    volatile LONG snap[dMAX_STITCH][dMAX_NUM_FFT]; // set to 1 to allow a snap
                                                   // of raw spectrum data
    HANDLE hSnapEvent[dMAX_STITCH]
                     [dMAX_NUM_FFT]; // mutex handles; mutexes will be used to
                                     // signal a snap is complete
    double* snap_buff[dMAX_STITCH]
                     [dMAX_NUM_FFT]; // pointers to buffers for the snap
...
extern __declspec(dllexport) void CreateAnalyzer(
    int disp, int* success, char* app_data_path);

extern __declspec(dllexport) void XCreateAnalyzer(int disp,
    int* success, // writes '0' to success if all went well, <0 if mem alloc
                  // failed
...
extern __declspec(dllexport) void DestroyAnalyzer(int disp);

extern __declspec(dllexport) void SetCalibration(int disp,
...
extern __declspec(dllexport) void OpenBuffer(
    int disp, int ss, int LO, void** Ipointer, void** Qpointer);

extern __declspec(dllexport) void CloseBuffer(int disp, int ss, int LO);

extern __declspec(dllexport) void Spectrum(
    int disp, int ss, int LO, dINREAL* pI, dINREAL* pQ);

extern __declspec(dllexport) void Spectrum2(
    int run, int disp, int ss, int LO, dINREAL* pbuff);

extern __declspec(dllexport) void Spectrum0(
    int run, int disp, int ss, int LO, double* pbuff);

extern __declspec(dllexport) void SnapSpectrum(
    int disp, int ss, int LO, double* snap_buff);
