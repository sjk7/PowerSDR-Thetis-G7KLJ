...
VARSAMP create_varsamp(int run, int size, double* in, double* out, int in_rate,
    int out_rate, double fc, double fc_low, int R, double gain, double var,
    int varmode) {
...
void hshift(VARSAMP a) {
    ...
    for (i = a->rsize - 1, j = hidx, k = hidx + 1; i >= 0;
         i--, j += a->R, k += a->R)
...
PORT void* create_varsampV(int in_rate, int out_rate, int R) {
    return (void*)create_varsamp(
        1, 0, 0, 0, in_rate, out_rate, 0.0, -1.0, R, 1.0, 1.0, 1);
}

PORT void xvarsampV(double* input, double* output, int numsamps, double var,
...
PORT void destroy_varsampV(void* ptr) {
