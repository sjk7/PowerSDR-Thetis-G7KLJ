...
void calc_resample(RESAMPLE a) {
    ...
    impulse = fir_bandpass(
        a->ncoef, fc_norm_low, fc_norm_high, 1.0, 1, 0, a->gain * (double)a->L);
    i = 0;
    for (j = 0; j < a->L; j++)
...
PORT RESAMPLE create_resample(int run, int size, double* in, double* out,
...
PORT void destroy_resample(RESAMPLE a) {
    decalc_resample(a);
    _aligned_free(a);
}
...
PORT void* create_resampleV(int in_rate, int out_rate) {
    return (void*)create_resample(1, 0, 0, 0, in_rate, out_rate, 0.0, 0, 1.0);
}

PORT void xresampleV(
...
PORT void destroy_resampleV(void* ptr) {
...
RESAMPLEF create_resampleF(
    int run, int size, float* in, float* out, int in_rate, int out_rate) {
    ...
    a->h = (double*)malloc0(a->ncoef * sizeof(double));
    impulse
        = fir_bandpass(a->ncoef, -fc_norm, +fc_norm, 1.0, 1, 0, (double)a->L);
    i = 0;
    for (j = 0; j < a->L; j++)
        for (k = 0; k < a->ncoef; k += a->L) a->h[i++] = impulse[j + k];
    a->ringsize = a->cpp;
...
PORT void xresampleFV(
    float* input, float* output, int numsamps, int* outsamps, void* ptr) {
