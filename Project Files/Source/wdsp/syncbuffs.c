...
SYNCB create_syncbuffs(int accept, int nstreams, int max_insize,
    int max_outsize, int outsize, double** out, void (*exf)(void)) {
...
void destroy_syncbuffs(SYNCB a) {
    ...
    InterlockedBitTestAndReset(
        &a->accept, 0); // shut the Syncbound() gate to prevent new infusions
    EnterCriticalSection(
        &a->csIN); // wait until the current Inbound() infusion is finished
    EnterCriticalSection(
        &a->csOUT); // block the syncb thread before syncbdata()
    Sleep(25); // wait for the thread to arrive at the top of the syncb_main()
               // loop
    InterlockedBitTestAndReset(&a->run, 0); // set a trap for the syncb thread
    ReleaseSemaphore(a->Sem_BuffReady, 1,
        0); // be sure the syncb thread can pass WaitForSingleObject in
            // syncb_main()									//
    LeaveCriticalSection(
        &a->csOUT); // let the thread pass to the trap in syncbdata()
    ...
    for (i = 0; i < a->nstreams; i++) _aligned_free(a->r1_baseptr[i]);
...
void SetSYNCBRingOutsize(SYNCB a, int size) {
    InterlockedBitTestAndReset(
        &a->accept, 0); // shut the Syncbound() gate to prevent new infusions
    EnterCriticalSection(
        &a->csIN); // wait until the current Syncbound() infusion is finished
    EnterCriticalSection(
        &a->csOUT); // block the syncb thread before syncbdata()
    Sleep(25); // wait for the thread to arrive at the top of the syncb_main()
               // loop
...
DUMFILT create_dumfilt(
    int run, int delay, int opsize, double* in, double* out) {
