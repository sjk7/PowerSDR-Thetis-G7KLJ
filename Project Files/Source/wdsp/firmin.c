...
FIRMIN create_firmin(int run, int position, int size, double* in, double* out,
    int nc, double f_low, double f_high, int samplerate, int wintype,
    double gain) {
...
void xfirmin(FIRMIN a, int pos) {
    if (a->run && a->position == pos) {
        ...
        for (i = 0; i < a->size; i++) {
            ...
            k = a->idx;
            for (j = 0; j < a->nc; j++) {
                a->out[2 * i + 0] += a->h[2 * j + 0] * a->ring[2 * k + 0]
                    - a->h[2 * j + 1] * a->ring[2 * k + 1];
                a->out[2 * i + 1] += a->h[2 * j + 0] * a->ring[2 * k + 1]
                    + a->h[2 * j + 1] * a->ring[2 * k + 0];
                k = (k + a->mask) & a->mask;
            }
    ...
    if (a->run && (a->position == pos)) {
        ...
        for (j = 0; j < a->nfor; j++) {
            for (i = 0; i < 2 * a->size; i++) {
                a->accum[2 * i + 0]
                    += a->fftout[k][2 * i + 0] * a->fmask[j][2 * i + 0]
                    - a->fftout[k][2 * i + 1] * a->fmask[j][2 * i + 1];
                a->accum[2 * i + 1]
                    += a->fftout[k][2 * i + 0] * a->fmask[j][2 * i + 1]
                    + a->fftout[k][2 * i + 1] * a->fmask[j][2 * i + 0];
            }
...
void plan_fircore(FIRCORE a) {
    ...
    for (i = 0; i < a->nfor; i++) {
        ...
        a->pcfor[i] = fftw_plan_dft_1d(2 * a->size, (fftw_complex*)a->fftin,
            (fftw_complex*)a->fftout[i], FFTW_FORWARD, MY_PATIENCE);
        a->maskplan[0][i]
            = fftw_plan_dft_1d(2 * a->size, (fftw_complex*)a->maskgen,
                (fftw_complex*)a->fmask[0][i], FFTW_FORWARD, MY_PATIENCE);
        a->maskplan[1][i]
            = fftw_plan_dft_1d(2 * a->size, (fftw_complex*)a->maskgen,
                (fftw_complex*)a->fmask[1][i], FFTW_FORWARD, MY_PATIENCE);
    }
    a->accum = (double*)malloc0(2 * a->size * sizeof(complex));
    a->crev = fftw_plan_dft_1d(2 * a->size, (fftw_complex*)a->accum,
        (fftw_complex*)a->out, FFTW_BACKWARD, MY_PATIENCE);
    a->masks_ready = 0;
}
...
FIRCORE create_fircore(
    int size, double* in, double* out, int nc, int mp, double* impulse) {
...
void xfircore(FIRCORE a) {
    ...
    for (j = 0; j < a->nfor; j++) {
        for (i = 0; i < 2 * a->size; i++) {
            a->accum[2 * i + 0]
                += a->fftout[k][2 * i + 0] * a->fmask[a->cset][j][2 * i + 0]
                - a->fftout[k][2 * i + 1] * a->fmask[a->cset][j][2 * i + 1];
            a->accum[2 * i + 1]
                += a->fftout[k][2 * i + 0] * a->fmask[a->cset][j][2 * i + 1]
                + a->fftout[k][2 * i + 1] * a->fmask[a->cset][j][2 * i + 0];
        }
        k = (k + a->idxmask) & a->idxmask;
    }
    LeaveCriticalSection(&a->update);
...
void setNc_fircore(FIRCORE a, int nc, double* impulse) {
    // because of FFT planning, this will probably cause a glitch in audio if
    // done during dataflow
    deplan_fircore(a);
    _aligned_free(a->impulse);
    _aligned_free(a->imp);
    a->nc = nc;
