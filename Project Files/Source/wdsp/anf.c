...
ANF create_anf(int run, int position, int buff_size, double* in_buff,
    double* out_buff, int dline_size, int n_taps, int delay, double two_mu,
    double gamma,

    double lidx, double lidx_min, double lidx_max, double ngamma,
    double den_mult, double lincr, double ldecr) {
    ...
    if (a->run && (a->position == position)) {
        for (i = 0; i < a->buff_size; i++) {
            ...
            if ((nel = error * (1.0 - a->two_mu * sigma * inv_sigp)) < 0.0)
                nel = -nel;
            if ((nev = a->d[a->in_idx] - (1.0 - a->two_mu * a->ngamma) * y
                        - a->two_mu * error * sigma * inv_sigp)
                < 0.0)
                nev = -nev;
            if (nev < nel) {
            ...
            a->ngamma = a->gamma * (a->lidx * a->lidx) * (a->lidx * a->lidx)
                * a->den_mult;
...
PORT void SetRXAANFVals(
    int channel, int taps, int delay, double gain, double leakage) {
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANFTaps(int channel, int taps) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anf.p->n_taps = taps;
    flush_anf(rxa[channel].anf.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANFDelay(int channel, int delay) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anf.p->delay = delay;
    flush_anf(rxa[channel].anf.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANFGain(int channel, double gain) {
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANFLeakage(int channel, double leakage) {
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANFPosition(int channel, int position) {
    EnterCriticalSection(&ch[channel].csDSP);
