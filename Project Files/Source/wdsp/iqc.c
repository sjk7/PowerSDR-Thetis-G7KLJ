...
void size_iqc(IQC a) {
    ...
    for (i = 0; i <= a->ints; i++) a->t[i] = (double)i / (double)a->ints;
    for (i = 0; i < 2; i++) {
...
IQC create_iqc(int run, int size, double* in, double* out, double rate,
    int ints, double tup, int spi) {
...
enum _iqcstate { RUN = 0, BEGIN, SWAP, END, DONE };
...
void xiqc(IQC a) {
    if (_InterlockedAnd(&a->run, 1)) {
        ...
        for (i = 0; i < a->size; i++) {
            ...
            switch (a->state) {
                case RUN:
                    if (a->dog.cpi[k] != a->dog.spi)
                        if (++a->dog.cpi[k] == a->dog.spi) a->dog.full_ints++;
                    ...
                    break;
                case BEGIN:
                    PRE0 = (1.0 - a->cup[a->count]) * I
                        + a->cup[a->count] * PRE0;
                    PRE1 = (1.0 - a->cup[a->count]) * Q
                        + a->cup[a->count] * PRE1;
                    if (a->count++ == a->ntup) {
                        a->state = RUN;
                        a->count = 0;
                ...
                case SWAP:
                        ...
                        + dx
                            * (a->cs[mset][4 * k + 1]
                                + dx
                                    * (a->cs[mset][4 * k + 2]
                                        + dx * a->cs[mset][4 * k + 3]));
                    PRE0 = (1.0 - a->cup[a->count]) * ym * (I * yc - Q * ys)
                        + a->cup[a->count] * PRE0;
                    PRE1 = (1.0 - a->cup[a->count]) * ym * (I * ys + Q * yc)
                        + a->cup[a->count] * PRE1;
                    if (a->count++ == a->ntup) {
                        a->state = RUN;
                        a->count = 0;
                    ...
                    break;
                case END:
                    PRE0 = (1.0 - a->cup[a->count]) * PRE0
                        + a->cup[a->count] * I;
                    PRE1 = (1.0 - a->cup[a->count]) * PRE1
                        + a->cup[a->count] * Q;
                    if (a->count++ == a->ntup) {
                        a->state = DONE;
                        a->count = 0;
            ...
            a->out[2 * i + 0] = PRE0;
            a->out[2 * i + 1] = PRE1;
            // print_iqc_values("iqc.txt", a->state, env, PRE0, PRE1, ym, yc,
            // ys, 1.1);
        }
    } else if (a->out != a->in)
...
PORT void GetTXAiqcValues(int channel, double* cm, double* cc, double* cs) {
    IQC a;
...
PORT void SetTXAiqcValues(int channel, double* cm, double* cc, double* cs) {
    IQC a;
    EnterCriticalSection(&ch[channel].csDSP);
    a = txa[channel].iqc.p0;
    a->cset = 1 - a->cset;
...
PORT void SetTXAiqcSwap(int channel, double* cm, double* cc, double* cs) {
...
PORT void SetTXAiqcStart(int channel, double* cm, double* cc, double* cs) {
    IQC a = txa[channel].iqc.p1;
    EnterCriticalSection(&ch[channel].csDSP);
    a->cset = 0;
    memcpy(a->cm[a->cset], cm, a->ints * 4 * sizeof(double));
...
PORT void SetTXAiqcEnd(int channel) {
    IQC a = txa[channel].iqc.p1;
    EnterCriticalSection(&ch[channel].csDSP);
    InterlockedBitTestAndSet(&a->busy, 0);
    a->state = END;
