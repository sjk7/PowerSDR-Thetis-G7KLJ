...
typedef struct _dexp {
    ...
    double
        hysteresis_ratio; // ratio hold_thresh/attack_thresh.  0.0 < ratio < 1.0
    double low_gain; // gain when gate is closed
    double*
        trigsig; // buffer for trigger signal (signal after side-channel filter)
    ...
    int run_filt; // 1 = side-channel filter and compensating delay are ON, 0 =
                  // OFF
    ...
    int antivox_new; // internal variable indicating new anti-vox data is
                     // available
...
__declspec(dllexport) void create_dexp(int id, int run_dexp, int size,
    double* in, double* out, int rate, double dettau, double tattack,
    double tdecay, double thold, double exp_ratio, double hyst_ratio,
    double attack_thresh, int nc, int wtype, double lowcut, double highcut,
