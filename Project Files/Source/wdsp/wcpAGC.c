...
WCPAGC create_wcpagc(int run, int mode, int pmode, double* in, double* out,
    int io_buffsize, int sample_rate, double tau_attack, double tau_decay,
    int n_tau, double max_gain, double var_gain, double fixed_gain,
    double max_input, double out_targ, double tau_fast_backaverage,
    double tau_fast_decay, double pop_ratio, int hang_enable,
    double tau_hang_backmult, double hangtime, double hang_thresh,
...
void loadWcpAGC(WCPAGC a) {
    ...
    a->fast_backmult
        = 1.0 - exp(-1.0 / (a->sample_rate * a->tau_fast_backaverage));
    ...
    a->hang_level
        = (a->max_input * tmp
              + (a->out_target / (a->var_gain * a->max_gain)) * (1.0 - tmp))
        * 0.637;

    a->hang_backmult
        = 1.0 - exp(-1.0 / (a->sample_rate * a->tau_hang_backmult));
...
void xwcpagc(WCPAGC a) {
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->io_buffsize; i++) {
            ...
            if (a->pmode == 0)
                a->abs_ring[a->in_index]
                    = max(fabs(a->ring[2 * a->in_index + 0]),
                        fabs(a->ring[2 * a->in_index + 1]));
            else
                a->abs_ring[a->in_index] = sqrt(
                    a->ring[2 * a->in_index + 0] * a->ring[2 * a->in_index + 0]
                    + a->ring[2 * a->in_index + 1]
                        * a->ring[2 * a->in_index + 1]);

            a->fast_backaverage = a->fast_backmult * a->abs_out_sample
                + a->onemfast_backmult * a->fast_backaverage;
            a->hang_backaverage = a->hang_backmult * a->abs_out_sample
                + a->onemhang_backmult * a->hang_backaverage;

            if ((a->abs_out_sample >= a->ring_max)
                && (a->abs_out_sample > 0.0)) {
                a->ring_max = 0.0;
                k = a->out_index;
                for (j = 0; j < a->attack_buffsize; j++) {
                    if (++k == a->ring_buffsize) k = 0;
            ...
            if (a->hang_counter > 0) --a->hang_counter;
            ...
            switch (a->state) {
                case 0: {
                    if (a->ring_max >= a->volts) {
                        ...
                        if (a->volts > a->pop_ratio * a->fast_backaverage) {
                            a->state = 1;
                            a->volts += (a->ring_max - a->volts)
                                * a->fast_decay_mult;
                        } else {
                            if (a->hang_enable
                                && (a->hang_backaverage > a->hang_level)) {
                                a->state = 2;
                                a->hang_counter
                                    = (int)(a->hangtime * a->sample_rate);
                                a->decay_type = 1;
                            } else {
                                a->state = 3;
                                a->volts
                                    += (a->ring_max - a->volts) * a->decay_mult;
                                a->decay_type = 0;
                            }
                        }
                    }
                ...
                case 4: {
                    if (a->ring_max >= a->volts) {
                        ...
                        a->volts
                            += (a->ring_max - a->volts) * a->hang_decay_mult;
                    }
                    break;
                }
            }

            if (a->volts < a->min_volts) a->volts = a->min_volts;
            a->gain = a->volts * a->inv_out_target;
            mult = (a->out_target
                       - a->slope_constant
                           * min(0.0, log10(a->inv_max_input * a->volts)))
                / a->volts;
...
PORT void SetRXAAGCMode(int channel, int mode) {
    ...
    switch (mode) {
        ...
        case 4: // agcFAST
            ...
            loadWcpAGC(rxa[channel].agc.p);
            break;
        default: rxa[channel].agc.p->mode = 5; break;
    }
...
// for line on bandscope
{
    ...
    if (rxa[channel].agc.p->max_input > rxa[channel].agc.p->min_volts) {
        ...
        tmp = max(1e-8,
            (convert - rxa[channel].agc.p->min_volts)
                / (rxa[channel].agc.p->max_input
                    - rxa[channel].agc.p->min_volts));
...
// for line on bandscope.
{
    ...
    EnterCriticalSection(&ch[channel].csDSP);
    noise_offset = 10.0
        * log10((rxa[channel].nbp0.p->fhigh - rxa[channel].nbp0.p->flow) * size
            / rate);
...
// for line on bandscope
{
    ...
    rxa[channel].agc.p->max_gain = rxa[channel].agc.p->out_target
        / (rxa[channel].agc.p->var_gain
            * pow(10.0, (thresh + noise_offset) / 20.0));
    loadWcpAGC(rxa[channel].agc.p);
    LeaveCriticalSection(&ch[channel].csDSP);
}
...
PORT void SetRXAAGCMaxInputLevel(int channel, double level) {
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetTXAALCMaxGain(int channel, double maxgain) {
    EnterCriticalSection(&ch[channel].csDSP);
