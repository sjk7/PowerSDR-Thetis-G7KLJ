...
PANEL create_panel(int channel, int run, int size, double* in, double* out,
    double gain1, double gain2I, double gain2Q, int inselect, int copy) {
...
PORT void SetTXAPanelGain1(int channel, double gain) {
    ...
    // print_message ("micgainset.txt", "Set MIC Gain to", (int)(100.0 * gain),
    // 0, 0);
    LeaveCriticalSection(&ch[channel].csDSP);
}

PORT void SetTXAPanelSelect(int channel, int select) {
    EnterCriticalSection(&ch[channel].csDSP);
    if (select == 1)
