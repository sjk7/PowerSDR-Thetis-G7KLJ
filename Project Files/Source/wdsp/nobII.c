...
PORT NOB create_nob(int run, int buffsize, double* in, double* out,
    double samplerate, int mode, double advslewtime, double advtime,
    ...
    a->dline_size = (int)(MAX_SAMPLERATE
...
PORT void flush_nob(NOB a) {
    a->out_idx = 0;
    a->scan_idx = a->out_idx + a->adv_slew_count + a->adv_count + 1;
    a->in_idx = a->scan_idx + a->max_imp_seq + a->hang_count
        + a->hang_slew_count + a->filterlen;
    a->state = 0;
    a->overflow = 0;
    a->avg = 1.0;
...
PORT void xnob(NOB a) {
    ...
    if (a->run) {
        for (i = 0; i < a->buffsize; i++) {
            ...
            mag = sqrt(a->dline[2 * a->in_idx + 0] * a->dline[2 * a->in_idx + 0]
                + a->dline[2 * a->in_idx + 1] * a->dline[2 * a->in_idx + 1]);
            ...
            switch (a->state) {
                        ...
                        if (a->overflow == 0) {
                            ...
                            if (a->mode == 1 || a->mode == 2 || a->mode == 4) {
                                ...
                                for (k = 0; k < a->filterlen; k++) {
                                    a->I1 += a->fcoefs[k]
                                        * a->bfbuff[2 * bfboutidx + 0];
                                    a->Q1 += a->fcoefs[k]
                                        * a->bfbuff[2 * bfboutidx + 1];
                                    if (--bfboutidx < 0)
                            ...
                            if (a->mode == 2 || a->mode == 3 || a->mode == 4) {
                                if ((ff_idx = a->scan_idx + a->blank_count)
                                    >= a->dline_size)
                                    ff_idx -= a->dline_size;
                                ffcount = 0;
                                while (ffcount < a->filterlen) {
                                    if (a->imp[ff_idx] == 0) {
                                        ...
                                        a->ffbuff[2 * a->ffb_in_idx + 0]
                                            = a->dline[2 * ff_idx + 0];
                                        a->ffbuff[2 * a->ffb_in_idx + 1]
                                            = a->dline[2 * ff_idx + 1];
                                        ++ffcount;
                                    }
                                ...
                                for (k = 0; k < a->filterlen; k++) {
                                    a->I2 += a->fcoefs[k]
                                        * a->ffbuff[2 * ffboutidx + 0];
                                    a->Q2 += a->fcoefs[k]
                                        * a->ffbuff[2 * ffboutidx + 1];
                ...
                case 4: // slew output after blanking period
                {
                    ...
                    if (++a->time == a->hang_slew_count) a->state = 0;
                    break;
                }
                case 5: {
                    scale = 0.5 + a->awave[a->time];
                ...
                case 6: {
                    a->out[2 * i + 0] = 0.0;
                    a->out[2 * i + 1] = 0.0;
                    if (++a->time == a->blank_count) a->state = 7;
                    break;
                }
                case 7: {
                    a->out[2 * i + 0] = 0.0;
                    ...
                    a->time = 0;
                    if ((tidx
                            = a->scan_idx + a->hang_slew_count + a->hang_count)
                        >= a->dline_size)
                        tidx -= a->dline_size;
                    while (a->time++ <= a->adv_count + a->adv_slew_count
                            + a->hang_slew_count
                            + a->hang_count) //  CHECK EXACT
                                             //  COUNTS!!!!!!!!!!!!!!!!!!!!!!!
                    {
                    ...
                    if (staydown == 0) {
                        if (a->hang_count > 0) {
                            ...
                            a->state = 9;
                            a->time = 0;
                            if ((tidx = a->scan_idx + a->hang_slew_count
                                        + a->hang_count - a->adv_count
                                        - a->adv_slew_count)
                                >= a->dline_size)
                                tidx -= a->dline_size;
                            if (tidx < 0) tidx += a->dline_size;
...
PORT void pSetRCVRNOBRun(NOB a, int run) {
    EnterCriticalSection(&a->cs_update);
    a->run = run;
    LeaveCriticalSection(&a->cs_update);
}

PORT void pSetRCVRNOBMode(NOB a, int mode) {
    EnterCriticalSection(&a->cs_update);
    a->mode = mode;
    LeaveCriticalSection(&a->cs_update);
}

PORT void pSetRCVRNOBBuffsize(NOB a, int size) {
    EnterCriticalSection(&a->cs_update);
    a->buffsize = size;
    LeaveCriticalSection(&a->cs_update);
}

PORT void pSetRCVRNOBSamplerate(NOB a, int rate) {
    EnterCriticalSection(&a->cs_update);
...
PORT void pSetRCVRNOBTau(NOB a, double tau) {
    EnterCriticalSection(&a->cs_update);
    a->advslewtime = tau;
    a->hangslewtime = tau;
    init_nob(a);
...
PORT void pSetRCVRNOBHangtime(NOB a, double time) {
    EnterCriticalSection(&a->cs_update);
    a->hangtime = time;
    init_nob(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRNOBAdvtime(NOB a, double time) {
    EnterCriticalSection(&a->cs_update);
    a->advtime = time;
    init_nob(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRNOBBacktau(NOB a, double tau) {
    EnterCriticalSection(&a->cs_update);
    a->backtau = tau;
    init_nob(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRNOBThreshold(NOB a, double thresh) {
    EnterCriticalSection(&a->cs_update);
    a->threshold = thresh;
    LeaveCriticalSection(&a->cs_update);
}
...
PORT void create_nobEXT(int id, int run, int mode, int buffsize,
    double samplerate, double slewtime, double hangtime, double advtime,
    double backtau, double threshold) {
    double advslewtime = slewtime;
    double hangslewtime = slewtime;
    double max_imp_seq_time = 0.025;
    pnob[id] = create_nob(run, buffsize, 0, 0, samplerate, mode, advslewtime,
        advtime, hangslewtime, hangtime, max_imp_seq_time, backtau, threshold);
}

PORT void destroy_nobEXT(int id) {
    destroy_nob(pnob[id]);
}

PORT void flush_nobEXT(int id) {
    flush_nob(pnob[id]);
}

PORT void xnobEXT(int id, double* in, double* out) {
    NOB a = pnob[id];
    a->in = in;
    a->out = out;
    xnob(a);
...
PORT void SetEXTNOBRun(int id, int run) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->run = run;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetEXTNOBMode(int id, int mode) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->mode = mode;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetEXTNOBBuffsize(int id, int size) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->buffsize = size;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetEXTNOBSamplerate(int id, int rate) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->samplerate = (double)rate;
    init_nob(a);
...
PORT void SetEXTNOBTau(int id, double tau) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->advslewtime = tau;
    a->hangslewtime = tau;
...
PORT void SetEXTNOBHangtime(int id, double time) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->hangtime = time;
    init_nob(a);
...
PORT void SetEXTNOBAdvtime(int id, double time) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->advtime = time;
    init_nob(a);
...
PORT void SetEXTNOBBacktau(int id, double tau) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->backtau = tau;
    init_nob(a);
...
PORT void SetEXTNOBThreshold(int id, double thresh) {
    NOB a = pnob[id];
    EnterCriticalSection(&a->cs_update);
    a->threshold = thresh;
    LeaveCriticalSection(&a->cs_update);
...
PORT void xnobEXTF(int id, float* I, float* Q) {
    int i;
    NOB a = pnob[id];
    a->in = a->legacy;
    a->out = a->legacy;
