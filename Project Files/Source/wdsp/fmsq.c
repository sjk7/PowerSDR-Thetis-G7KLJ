...
void calc_fmsq(FMSQ a) {
    ...
    impulse = eq_impulse(
        a->nc, 3, a->F, a->G, a->rate, 1.0 / (2.0 * a->size), 0, 0);
...
FMSQ create_fmsq(int run, int size, double* insig, double* outsig,
    double* trigger, int rate, double fc, double* pllpole, double tdelay,
    double avtau, double longtau, double tup, double tdown, double tail_thresh,
...
enum _fmsqstate { MUTED, INCREASE, UNMUTED, TAIL, DECREASE };
...
void xfmsq(FMSQ a) {
    if (a->run) {
        ...
        xfircore(a->p);
        for (i = 0; i < a->size; i++) {
            noise = sqrt(a->noise[2 * i + 0] * a->noise[2 * i + 0]
                + a->noise[2 * i + 1] * a->noise[2 * i + 1]);
            ...
            switch (a->state) {
                ...
                case INCREASE:
                    a->outsig[2 * i + 0]
                        = a->insig[2 * i + 0] * a->cup[a->ntup - a->count];
                    a->outsig[2 * i + 1]
                        = a->insig[2 * i + 1] * a->cup[a->ntup - a->count];
                ...
                case DECREASE:
                    a->outsig[2 * i + 0]
                        = a->insig[2 * i + 0] * a->cdown[a->ntdown - a->count];
                    a->outsig[2 * i + 1]
                        = a->insig[2 * i + 1] * a->cdown[a->ntdown - a->count];
...
PORT void SetRXAFMSQNC(int channel, int nc) {
    FMSQ a;
    double* impulse;
    EnterCriticalSection(&ch[channel].csDSP);
