...
double* eq_impulse(int N, int nfreqs, double* F, double* G, double samplerate,
    ...
    if (N & 1) {
        for (i = 0; i <= mid; i++) {
            ...
            A[i]
                = pow(10.0,
                      0.05
                          * (frac * gp[j + 1] + (1.0 - frac) * gp[j] + gpreamp))
                * scale;
        }
    } else {
        for (i = 0; i < mid; i++) {
...
EQP create_eqp(int run, int size, int nc, int mp, double* in, double* out,
    int nfreqs, double* F, double* G, int ctfmode, int wintype,
    int samplerate) {
    ...
    impulse = eq_impulse(a->nc, a->nfreqs, a->F, a->G, a->samplerate,
        1.0 / (2.0 * a->size), a->ctfmode, a->wintype);
...
PORT void SetRXAEQProfile(int channel, int nfreqs, double* F, double* G) {
    EQP a;
    double* impulse;
    a = rxa[channel].eqp.p;
    _aligned_free(a->G);
...
PORT void SetRXAEQCtfmode(int channel, int mode) {
    EQP a;
    double* impulse;
    a = rxa[channel].eqp.p;
    a->ctfmode = mode;
...
PORT void SetRXAEQWintype(int channel, int wintype) {
    EQP a;
    double* impulse;
    a = rxa[channel].eqp.p;
    a->wintype = wintype;
...
PORT void SetRXAGrphEQ(
    int channel, int* rxeq) { // three band equalizer (legacy compatibility)
    EQP a;
    double* impulse;
    a = rxa[channel].eqp.p;
    _aligned_free(a->G);
...
PORT void SetRXAGrphEQ10(
    int channel, int* rxeq) { // ten band equalizer (legacy compatibility)
    EQP a;
    double* impulse;
    int i;
    a = rxa[channel].eqp.p;
    ...
    a->F[9] = 8000.0;
    a->F[10] = 16000.0;
    for (i = 0; i <= a->nfreqs; i++) a->G[i] = (double)rxeq[i];
    a->ctfmode = 0;
    impulse = eq_impulse(a->nc, a->nfreqs, a->F, a->G, a->samplerate,
        1.0 / (2.0 * a->size), a->ctfmode, a->wintype);
...
PORT void SetTXAEQProfile(int channel, int nfreqs, double* F, double* G) {
    EQP a;
    double* impulse;
    a = txa[channel].eqp.p;
    _aligned_free(a->G);
...
PORT void SetTXAEQCtfmode(int channel, int mode) {
    EQP a;
    double* impulse;
    a = txa[channel].eqp.p;
    a->ctfmode = mode;
...
PORT void SetTXAEQWintype(int channel, int wintype) {
    EQP a;
    double* impulse;
    a = txa[channel].eqp.p;
    a->wintype = wintype;
...
PORT void SetTXAGrphEQ(
    int channel, int* txeq) { // three band equalizer (legacy compatibility)
    EQP a;
    double* impulse;
    a = txa[channel].eqp.p;
    _aligned_free(a->G);
...
PORT void SetTXAGrphEQ10(
    int channel, int* txeq) { // ten band equalizer (legacy compatibility)
    EQP a;
    double* impulse;
    int i;
    a = txa[channel].eqp.p;
    ...
    a->F[9] = 8000.0;
    a->F[10] = 16000.0;
    for (i = 0; i <= a->nfreqs; i++) a->G[i] = (double)txeq[i];
    a->ctfmode = 0;
    impulse = eq_impulse(a->nc, a->nfreqs, a->F, a->G, a->samplerate,
        1.0 / (2.0 * a->size), a->ctfmode, a->wintype);
    setImpulse_fircore(a->p, impulse, 1);
...
double* eq_mults(int size, int nfreqs, double* F, double* G, double samplerate,
    double scale, int ctfmode, int wintype) {
    double* impulse = eq_impulse(
        size + 1, nfreqs, F, G, samplerate, scale, ctfmode, wintype);
...
void calc_eq(EQ a) {
    ...
    a->mults = eq_mults(a->size, a->nfreqs, a->F, a->G, a->samplerate, a->scale,
        a->ctfmode, a->wintype);
}
...
EQ create_eq(int run, int size, double* in, double* out, int nfreqs, double* F,
    double* G, int ctfmode, int wintype, int samplerate) {
...
void xeq(EQ a) {
    ...
    if (a->run) {
        ...
        for (i = 0; i < 2 * a->size; i++) {
            ...
            Q = a->product[2 * i + 1];
            a->product[2 * i + 0]
                = I * a->mults[2 * i + 0] - Q * a->mults[2 * i + 1];
            a->product[2 * i + 1]
...
void SetRXAEQProfile (int channel, int nfreqs, double* F, double* G)
{
    ...
    memcpy (a->G, G, (nfreqs + 1) * sizeof (double));
    _aligned_free (a->mults);
    a->mults = eq_mults (a->size, a->nfreqs, a->F, a->G, a->samplerate,
a->scale, a->ctfmode, a->wintype); LeaveCriticalSection (&ch[channel].csDSP);
}
