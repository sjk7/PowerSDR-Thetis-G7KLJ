...
SNOTCH create_snotch(
    int run, int size, double* in, double* out, int rate, double f, double bw) {
...
void xsnotch(SNOTCH a) {
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            a->x0 = a->in[2 * i + 0];
            a->out[2 * i + 0] = a->a0 * a->x0 + a->a1 * a->x1 + a->a2 * a->x2
                + a->b1 * a->y1 + a->b2 * a->y2;
            a->y2 = a->y1;
            a->y1 = a->out[2 * i + 0];
            a->x2 = a->x1;
    ...
    switch (a->design) {
        ...
        case 1:
            ...
            }
            {
                ...
                c = sn
                    * sinh(0.5
                        * log((a->f + 0.5 * a->cbw * bw_parm)
                            / (a->f - 0.5 * a->cbw * bw_parm))
                        * w0 / sn);
                den = 1.0 + c / A;
                a->a0 = (1.0 + c * A) / den;
                a->a1 = -2.0 * cos(w0) / den;
...
SPEAK create_speak(int run, int size, double* in, double* out, int rate,
    double f, double bw, double gain, int nstages, int design) {
...
void flush_speak(SPEAK a) {
    ...
    for (i = 0; i < a->nstages; i++) {
        a->x1[2 * i + 0] = a->x2[2 * i + 0] = a->y1[2 * i + 0]
            = a->y2[2 * i + 0] = 0.0;
        a->x1[2 * i + 1] = a->x2[2 * i + 1] = a->y1[2 * i + 1]
            = a->y2[2 * i + 1] = 0.0;
    }
}
            ...
            for (j = 0; j < 2; j++) {
                ...
                for (n = 0; n < a->nstages; n++) {
                    ...
                    a->y0[2 * n + j] = a->a0 * a->x0[2 * n + j]
                        + a->a1 * a->x1[2 * n + j] + a->a2 * a->x2[2 * n + j]
                        + a->b1 * a->y1[2 * n + j] + a->b2 * a->y2[2 * n + j];
...
PORT void SetRXASPCWRun(int channel, int run) {
...
PORT void SetRXASPCWFreq(int channel, double freq) {
    SPEAK a = rxa[channel].speak.p;
    EnterCriticalSection(&a->cs_update);
    a->f = freq;
    calc_speak(a);
...
PORT void SetRXASPCWBandwidth(int channel, double bw) {
    SPEAK a = rxa[channel].speak.p;
    EnterCriticalSection(&a->cs_update);
    a->bw = bw;
    calc_speak(a);
...
PORT void SetRXASPCWGain(int channel, double gain) {
    SPEAK a = rxa[channel].speak.p;
    EnterCriticalSection(&a->cs_update);
    a->gain = gain;
    calc_speak(a);
...
void decalc_mpeak(MPEAK a) {
    int i;
    for (i = 0; i < a->npeaks; i++) destroy_speak(a->pfil[i]);
    _aligned_free(a->mix);
    _aligned_free(a->tmp);
}

MPEAK create_mpeak(int run, int size, double* in, double* out, int rate,
    int npeaks, int* enable, double* f, double* bw, double* gain, int nstages) {
...
void flush_mpeak(MPEAK a) {
    int i;
    for (i = 0; i < a->npeaks; i++) flush_speak(a->pfil[i]);
}
...
PORT void SetRXAmpeakRun(int channel, int run) {
...
PORT void SetRXAmpeakNpeaks(int channel, int npeaks) {
    MPEAK a = rxa[channel].mpeak.p;
    EnterCriticalSection(&a->cs_update);
    a->npeaks = npeaks;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetRXAmpeakFilEnable(int channel, int fil, int enable) {
    MPEAK a = rxa[channel].mpeak.p;
    EnterCriticalSection(&a->cs_update);
    a->enable[fil] = enable;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetRXAmpeakFilFreq(int channel, int fil, double freq) {
    MPEAK a = rxa[channel].mpeak.p;
    EnterCriticalSection(&a->cs_update);
    a->f[fil] = freq;
    a->pfil[fil]->f = freq;
...
PORT void SetRXAmpeakFilBw(int channel, int fil, double bw) {
    MPEAK a = rxa[channel].mpeak.p;
    EnterCriticalSection(&a->cs_update);
    a->bw[fil] = bw;
    a->pfil[fil]->bw = bw;
...
PORT void SetRXAmpeakFilGain(int channel, int fil, double gain) {
    MPEAK a = rxa[channel].mpeak.p;
    EnterCriticalSection(&a->cs_update);
    a->gain[fil] = gain;
...
PHROT create_phrot(int run, int size, double* in, double* out, int rate,
    double fc, int nstages) {
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            ...
            for (n = 0; n < a->nstages; n++) {
                ...
                a->y0[n]
                    = a->b0 * a->x0[n] + a->b1 * a->x1[n] - a->a1 * a->y1[n];
                a->y1[n] = a->y0[n];
                a->x1[n] = a->x0[n];
            }
...
PORT void SetTXAPHROTRun(int channel, int run) {
...
PORT void SetTXAPHROTCorner(int channel, double corner) {
    PHROT a = txa[channel].phrot.p;
    EnterCriticalSection(&a->cs_update);
    decalc_phrot(a);
    a->fc = corner;
...
PORT void SetTXAPHROTNstages(int channel, int nstages) {
    PHROT a = txa[channel].phrot.p;
    EnterCriticalSection(&a->cs_update);
    decalc_phrot(a);
    a->nstages = nstages;
