...
EMPHP create_emphp(int run, int position, int size, int nc, int mp, double* in,
    double* out, int rate, int ctype, double f_low, double f_high) {
    ...
    a->f_low = f_low;
    a->f_high = f_high;
    impulse = fc_impulse(a->nc, a->f_low, a->f_high,
...
/********************************************************************************************************
 *																										*
 *						Partitioned Overlap-Save FM Pre-Emphasis:  TXA
 *Properties
 **
 *																										*
 ********************************************************************************************************/
...
PORT void SetTXAFMEmphMP(int channel, int mp) {
    EMPHP a;
...
PORT void SetTXAFMEmphNC(int channel, int nc) {
    EMPHP a;
    double* impulse;
    EnterCriticalSection(&ch[channel].csDSP);
    a = txa[channel].preemph.p;
...
EMPH create_emph(int run, int position, int size, double* in, double* out,
