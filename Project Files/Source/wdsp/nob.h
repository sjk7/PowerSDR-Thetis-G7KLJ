...
typedef struct _anb {
    ...
    double power; // level at which signal was increasing when a new decrease is
                  // started
...
__declspec(dllexport) ANB create_anb(int run, int buffsize, double* in,
    double* out, double samplerate, double tau, double hangtime, double advtime,
...
extern __declspec(dllexport) void create_anbEXT(int id, int run, int buffsize,
