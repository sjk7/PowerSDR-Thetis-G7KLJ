...
void calc_fmd(FMD a) {
    ...
    a->g2 = -a->g1
        + 2.0
            * (1
                - exp(-a->omegaN * a->zeta / a->rate)
                    * cos(a->omegaN / a->rate * sqrt(1.0 - a->zeta * a->zeta)));
    a->phs = 0.0;
    a->fil_out = 0.0;
    a->omega = 0.0;
    a->pllpole = a->omegaN
        * sqrt(2.0 * a->zeta * a->zeta + 1.0
            + sqrt((2.0 * a->zeta * a->zeta + 1.0)
                    * (2.0 * a->zeta * a->zeta + 1.0)
                + 1))
        / TWOPI;
    // dc removal
    ...
    // CTCSS Removal
    a->sntch = create_snotch(
        1, a->size, a->out, a->out, (int)a->rate, a->ctcss_freq, 0.0002);
}
...
FMD create_fmd(int run, int size, double* in, double* out, int rate,
    double deviation, double f_low, double f_high, double fmin, double fmax,
    double zeta, double omegaN, double tau, double afgain, int sntch_run,
    double ctcss_freq, int nc_de, int mp_de, int nc_aud, int mp_aud) {
    ...
    impulse = fc_impulse(a->nc_de, a->f_low, a->f_high,
        ...
        1.0 / (2.0 * a->size), 0, 0);
    a->pde = create_fircore(
        a->size, a->audio, a->out, a->nc_de, a->mp_de, impulse);
    _aligned_free(impulse);
    // audio filter
    impulse = fir_bandpass(a->nc_aud, 0.8 * a->f_low, 1.1 * a->f_high, a->rate,
        0, 1, a->afgain / (2.0 * a->size));
    a->paud = create_fircore(
        a->size, a->out, a->out, a->nc_aud, a->mp_aud, impulse);
    _aligned_free(impulse);
    return a;
}
...
PORT void SetRXACTCSSFreq(int channel, double freq) {
    FMD a;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].fmd.p;
    a->ctcss_freq = freq;
...
PORT void SetRXACTCSSRun(int channel, int run) {
    FMD a;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].fmd.p;
    a->sntch_run = run;
...
PORT void SetRXAFMNCde(int channel, int nc) {
    FMD a;
    double* impulse;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].fmd.p;
...
PORT void SetRXAFMMPde(int channel, int mp) {
    FMD a;
    a = rxa[channel].fmd.p;
    if (a->mp_de != mp) {
        a->mp_de = mp;
...
PORT void SetRXAFMNCaud(int channel, int nc) {
    FMD a;
    double* impulse;
    EnterCriticalSection(&ch[channel].csDSP);
    a = rxa[channel].fmd.p;
    if (a->nc_aud != nc) {
        a->nc_aud = nc;
        impulse = fir_bandpass(a->nc_aud, 0.8 * a->f_low, 1.1 * a->f_high,
            a->rate, 0, 1, a->afgain / (2.0 * a->size));
...
PORT void SetRXAFMMPaud(int channel, int mp) {
    FMD a;
    a = rxa[channel].fmd.p;
    if (a->mp_aud != mp) {
        a->mp_aud = mp;
