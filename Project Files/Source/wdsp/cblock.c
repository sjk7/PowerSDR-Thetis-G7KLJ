...
CBL create_cbl(int run, int buff_size, double* in_buff, double* out_buff,
...
void xcbl(CBL a) {
    if (a->run) {
        ...
        for (i = 0; i < a->buff_size; i++) {
            ...
            a->out_buff[2 * i + 0]
                = a->in_buff[2 * i + 0] - a->prevIin + a->mtau * a->prevIout;
            a->out_buff[2 * i + 1]
                = a->in_buff[2 * i + 1] - a->prevQin + a->mtau * a->prevQout;
            a->prevIin = tempI;
            a->prevQin = tempQ;
            if (fabs(a->prevIout = a->out_buff[2 * i + 0]) < 1.0e-100)
                a->prevIout = 0.0;
            if (fabs(a->prevQout = a->out_buff[2 * i + 1]) < 1.0e-100)
                a->prevQout = 0.0;
        }
...
PORT void SetRXACBLRun(int channel, int setit) {
