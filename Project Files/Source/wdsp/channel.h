...
struct _ch {
    ...
    volatile long
        exchange; // when 1, fexchange() operates; when 0, it just returns
    ...
    int dsp_size; // number complex samples processed per buffer in mainstream
                  // dsp processing
    int dsp_insize; // size (complex samples) of the output of the r1 (input)
                    // buffer
    int dsp_outsize; // size (complex samples) of the input of the r2 (output)
                     // buffer
    int out_size; // output buffsize (complex samples) in a fexchange()
                  // operation
    CRITICAL_SECTION csDSP; // used to block dsp while parameters are updated or
                            // buffers flushed
    CRITICAL_SECTION csEXCH; // used to block fexchange() while parameters are
                             // updated or buffers flushed
    ...
    {
        IOB pc, pd, pe,
            pf; // copies for console calls, dsp, exchange, and flush thread
