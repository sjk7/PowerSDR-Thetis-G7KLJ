...
void calc_slews(DEXP a) {
    ...
    for (i = 0; i <= a->nattack; i++) {
        a->cattack[i]
            = a->low_gain + (1.0 - a->low_gain) * 0.5 * (1.0 - cos(theta));
        theta += delta;
    }
...
void calc_dexp(DEXP a) {
    ...
    a->count = 0;
    a->hold_thresh
        = a->hysteresis_ratio * a->attack_thresh; // hysteresis ratio < 1.0
    a->nhold = (int)(a->thold * a->rate);
    // vox
    a->vox_count = (int)(a->audelay * a->rate);
    // audio delay
    a->audring = calc_delring((int)a->rate, a->size,
        (int)(a->audelay * a->rate), a->audbuffer, a->out);
}
...
void calc_filter(DEXP a) {
    double* impulse;
    // 2.0 gain on filter is somewhat arbitrarily chosen to get trigger input
    // similar to that without the filter, knowing
    //    that for any reasonable use of the filter there will be a reduction in
    //    trigger signal.
    impulse = fir_bandpass(a->nc, a->low_cut, a->high_cut, a->rate, a->wintype,
        1, 2.0 / (double)(2 * a->size));
    // print_impulse ("scf.txt", a->nc, impulse, 1, 0);
    a->p = create_fircore(a->size, a->in, a->trigsig, a->nc, 1, impulse);
    _aligned_free(impulse);
    a->scdring = calc_delring(
        a->size + a->nc / 2, a->size, a->nc / 64, a->in, a->delsig);
}
...
PORT void create_dexp(int id, int run_dexp, int size, double* in, double* out,
    int rate, double dettau, double tattack, double tdecay, double thold,
    double exp_ratio, double hyst_ratio, double attack_thresh, int nc,
    int wtype, double lowcut, double highcut, int run_filt, int run_vox,
    int run_audelay, double audelay,
    void(__stdcall* pushvox)(int id, int active), int antivox_run,
    int antivox_size, int antivox_rate, double antivox_gain,
    double antivox_tau) {
...
enum _dexpstate { DEXP_LOW, DEXP_ATTACK, DEXP_HIGH, DEXP_HOLD, DEXP_DECAY };

PORT void xdexp(int id) {
    DEXP a = pdexp[id];
    int i;
    ...
    if (a->state == DEXP_LOW && a->antivox_new != 0) {
        // if VOX is currently NOT triggered, and, if we have new antivox data
        // to process
        for (i = 0; i < a->antivox_size; i++) {
            sig = sqrt(a->antivox_data[2 * i + 0] * a->antivox_data[2 * i + 0]
                + a->antivox_data[2 * i + 1] * a->antivox_data[2 * i + 1]);
            a->antivox_level = a->antivox_mult * a->antivox_level
                + a->antivox_onemmult * sig;
        }
    ...
    for (i = 0; i < a->size; i++) {
        sig = sqrt(a->trigsig[2 * i + 0] * a->trigsig[2 * i + 0]
            + a->trigsig[2 * i + 1] * a->trigsig[2 * i + 1]);
        ...
        switch (a->state) {
            case DEXP_LOW:
                ...
                if (a->run_vox && a->state == DEXP_ATTACK) {
                    (a->pushvox)(a->id, 1);
                    // Set vox_count for delay IF the audio delay is also
                    // enabled.
                    if (a->run_audelay)
                        a->vox_count = (int)(a->audelay * a->rate);
                    else
                ...
                // If we're sitting in this state and the delayed vox count
                // expires, turn OFF VOX.
            ...
            case DEXP_ATTACK:
                ...
                if (a->count-- == 0) a->state = DEXP_HIGH;
                break;
            case DEXP_HIGH:
...
PORT void SendCBPushDexpVox(
    int id, void(__stdcall* pushvox)(int id, int active)) {
    // Call to set the address of the callback to operate VOX.
    DEXP a = pdexp[id];
    a->pushvox = pushvox;
}

PORT void SetDEXPRun(int id, int run) {
...
PORT void SetDEXPSize(int id, int size) {
    // There are some constraints on the input/output buffer sizes.
    //   * must be a power-of-two
    //   * must be less than or equal to 'nc', the number of filter
    //   coefficients, which is also a power-of-two
...
PORT void SetDEXPIOBuffers(int id, double* in, double* out) {
...
PORT void SetDEXPRate(int id, double rate) {
    // Sets the sample rate.
    // This is used for timing and filter calculations as well as sizing
    // 'audring'.
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    decalc_filter(a);
...
PORT void SetDEXPDetectorTau(int id, double tau) {
...
PORT void SetDEXPAttackTime(int id, double time) {
...
PORT void SetDEXPReleaseTime(int id, double time) {
...
PORT void SetDEXPHoldTime(int id, double time) {
...
PORT void SetDEXPExpansionRatio(int id, double ratio) {
...
PORT void SetDEXPHysteresisRatio(int id, double ratio) {
...
PORT void SetDEXPAttackThreshold(int id, double thresh) {
...
PORT void SetDEXPFilterTaps(int id, int taps) {
    // Set number of taps.  Must be a power of two and an even multiple of
    // 'size'.
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    decalc_filter(a);
...
PORT void SetDEXPWindowType(int id, int type) {
...
PORT void SetDEXPLowCut(int id, double lowcut) {
...
PORT void SetDEXPHighCut(int id, double highcut) {
...
PORT void SetDEXPRunSideChannelFilter(int id, int run) {
...
PORT void SetDEXPRunVox(int id, int run) {
...
PORT void SetDEXPRunAudioDelay(int id, int run) {
...
PORT void SetDEXPAudioDelay(int id, double delay) {
...
PORT void SetAntiVOXRun(int id, int run) {
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    a->antivox_run = run;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetAntiVOXSize(int id, int size) {
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    decalc_antivox(a);
    a->antivox_size = size;
...
PORT void SetAntiVOXRate(int id, double rate) {
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    decalc_antivox(a);
    a->antivox_rate = rate;
...
PORT void SetAntiVOXGain(int id, double gain) {
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    a->antivox_gain = gain;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetAntiVOXDetectorTau(int id, double tau) {
    DEXP a = pdexp[id];
    EnterCriticalSection(&a->cs_update);
    decalc_antivox(a);
    a->antivox_tau = tau;
...
PORT void SendAntiVOXData(int id, int nsamples, double* data) {
