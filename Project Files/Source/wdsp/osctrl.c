...
OSCTRL create_osctrl(int run, int size, double* inbuff, double* outbuff,
    int rate, double osgain) {
...
void xosctrl(OSCTRL a) {
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            a->dl[2 * a->in_idx + 0]
                = a->inbuff[2 * i + 0]; // put sample in delay line
            a->dl[2 * a->in_idx + 1] = a->inbuff[2 * i + 1];
            a->env_out = a->dlenv[a->in_idx]; // take env out of delay line
            a->dlenv[a->in_idx] = sqrt(a->inbuff[2 * i + 0]
                    * a->inbuff[2 * i + 0] // put env in delay line
                + a->inbuff[2 * i + 1] * a->inbuff[2 * i + 1]);
            if (a->dlenv[a->in_idx] > a->max_env)
                a->max_env = a->dlenv[a->in_idx];
            ...
            if (a->max_env > 1.0)
                divisor = 1.0 + a->osgain * (a->max_env - 1.0);
            else
                divisor = 1.0;
            a->outbuff[2 * i + 0]
                = a->dl[2 * a->out_idx + 0] / divisor; // output sample
