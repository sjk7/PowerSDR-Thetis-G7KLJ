...
void downslew2(IOB a, OUTREAL* pIout, OUTREAL* pQout) {
    ...
    for (i = 0; i < a->out_size; i++) {
        ...
        switch (a->slew.dstate) {
            ...
            case DOWNSLEW:
                pIout[i] = (OUTREAL)(I
                    * a->slew.cdown[a->slew.ntdown - a->slew.dcount]);
                pQout[i] = (OUTREAL)(Q
                    * a->slew.cdown[a->slew.ntdown - a->slew.dcount]);
                if (a->slew.dcount-- == 0) {
                    a->slew.dstate = ZERO;
...
void create_iobuffs(int channel) {
    ...
    ch[channel].iob.pc = ch[channel].iob.pd = ch[channel].iob.pe
        = ch[channel].iob.pf = a;
    a->channel = channel;
    ...
    if (_InterlockedAnd(&ch[channel].exchange, 1)) {
        ...
        if (a->bfo || doit) {
            if (_InterlockedAnd(&a->slew.downflag, 1)) {
                ...
                for (i = 0; i < a->out_size; i++) {
                    Iout[i] = (OUTREAL)((
                        a->r2_baseptr + 2 * a->r2_outidx)[2 * i + 0]);
                    Qout[i] = (OUTREAL)((
                        a->r2_baseptr + 2 * a->r2_outidx)[2 * i + 1]);
                }
        } else {
