...
// MODIFIED BESSEL FUNCTIONS OF THE 0TH AND 1ST ORDERS, Polynomial
// Approximations M. Abramowitz and I. Stegun, Eds., "Handbook of Mathematical
// Functions."  Washington, DC:  National
...
double bessI0(double x) {
    ...
    if (x == 0.0)
        ...
        if (x <= 3.75) {
            p = x / 3.75;
            p = p * p;
            res = (((((0.0045813 * p + 0.0360768) * p + 0.2659732) * p
                        + 1.2067492)
                           * p
                       + 3.0899424)
                          * p
            ...
            p = 3.75 / x;
            res = exp(x) / sqrt(x)
                * ((((((((+0.00392377 * p - 0.01647633) * p + 0.02635537) * p
                           - 0.02057706)
                              * p
                          + 0.00916281)
                             * p
...
double bessI1(double x) {
    ...
    if (x == 0.0)
        ...
        if (x <= 3.75) {
            ...
            p = p * p;
            res = x
                * ((((((0.00032411 * p + 0.00301532) * p + 0.02658733) * p
                         + 0.15084934)
                            * p
                        + 0.51498869)
                           * p
            ...
            p = 3.75 / x;
            res = exp(x) / sqrt(x)
                * ((((((((-0.00420059 * p + 0.01787654) * p - 0.02895312) * p
                           + 0.02282967)
                              * p
                          - 0.01031555)
                             * p
...
double e1xb(double x) {
    ...
    if (x == 0.0)
        ...
        for (k = 1; k <= 25; k++) {
            r = -r * k * x / ((k + 1.0) * (k + 1.0));
            e1 = e1 + r;
            if (fabs(r) <= fabs(e1) * 1.0e-15) break;
        }
        ...
        m = 20 + (int)(80.0 / x);
        t0 = 0.0;
        for (k = m; k >= 1; k--) t0 = (double)k / (1.0 + k / (x + t0));
        t = 1.0 / (x + t0);
        e1 = exp(-x) * t;
    }
    return e1;
...
void calc_window(EMNR a) {
    ...
    switch (a->wintype) {
        case 0:
            ...
            for (i = 0; i < a->fsize; i++) a->window[i] *= inv_coherent_gain;
            break;
    }
}
...
void calc_emnr(EMNR a) {
    int i;
    double Dvals[18] = {1.0, 2.0, 5.0, 8.0, 10.0, 15.0, 20.0, 30.0, 40.0, 60.0,
        80.0, 120.0, 140.0, 160.0, 180.0, 220.0, 260.0, 300.0};
    double Mvals[18] = {0.000, 0.260, 0.480, 0.580, 0.610, 0.668, 0.705, 0.762,
        0.800, 0.841, 0.865, 0.890, 0.900, 0.910, 0.920, 0.930, 0.935, 0.940};
    double Hvals[18] = {0.000, 0.150, 0.480, 0.780, 0.980, 1.550, 2.000, 2.300,
        2.520, 3.100, 3.380, 4.150, 4.350, 4.250, 3.900, 4.100, 4.700, 5.000};
    ...
    a->nsamps = 0;
    a->saveidx = 0;
    a->Rfor = fftw_plan_dft_r2c_1d(
        a->fsize, a->forfftin, (fftw_complex*)a->forfftout, FFTW_ESTIMATE);
    a->Rrev = fftw_plan_dft_c2r_1d(
        a->fsize, (fftw_complex*)a->revfftin, a->revfftout, FFTW_ESTIMATE);
    calc_window(a);
    ...
    if ((a->np.U
            = (int)(0.5 + (a->np.Dtime * a->np.rate / (a->np.V * a->np.incr))))
        < 1)
        a->np.U = 1;
    ...
    a->np.invQbar_points[3] = 1.0e300;
    {
        ...
        a->np.nsmax[0]
            = pow(10.0, db / 10.0 * a->np.V * a->np.incr / a->np.rate);
...

    {
        ...
        for (k = 0; k < a->np.msize; k++) {
            ...
            for (ku = 0; ku < a->np.U; ku++) a->np.actminbuff[ku][k] = 1.0e300;
        }
...
void decalc_emnr(EMNR a) {
    ...
    for (i = 0; i < a->np.U; i++) _aligned_free(a->np.actminbuff[i]);
    ...
    for (i = 0; i < a->ovrlp; i++) _aligned_free(a->save[i]);
...
EMNR create_emnr(int run, int position, int size, double* in, double* out,
    int fsize, int ovrlp, int rate, int wintype, double gain, int gain_method,
    int npe_method, int ae_run) {
...
void LambdaD(EMNR a) {
    ...
    a->np.alphaC = a->np.alphaCsmooth * a->np.alphaC
        + (1.0 - a->np.alphaCsmooth) * max(alphaCtilda, a->np.alphaCmin);
    f2 = a->np.alphaMax * a->np.alphaC;
    for (k = 0; k < a->np.msize; k++)
        a->np.alphaHat[k] = f2 * a->np.alphaOptHat[k];
    for (k = 0; k < a->np.msize; k++)
        a->np.p[k] = a->np.alphaHat[k] * a->np.p[k]
            + (1.0 - a->np.alphaHat[k]) * a->np.lambda_y[k];
    invQbar = 0.0;
    for (k = 0; k < a->np.msize; k++) {
        beta = min(a->np.betamax, a->np.alphaHat[k] * a->np.alphaHat[k]);
        a->np.pbar[k] = beta * a->np.pbar[k] + (1.0 - beta) * a->np.p[k];
        a->np.p2bar[k]
            = beta * a->np.p2bar[k] + (1.0 - beta) * a->np.p[k] * a->np.p[k];
    ...
    if (a->np.subwc == a->np.V) {
        ...
        for (k = 0; k < a->np.msize; k++) {
            int ku;
            double min;
            if (a->np.k_mod[k]) a->np.lmin_flag[k] = 0;
        ...
        if (a->np.subwc > 1) {
            for (k = 0; k < a->np.msize; k++) {
                if (a->np.k_mod[k]) {
                    a->np.lmin_flag[k] = 1;
                    a->np.sigma2N[k]
                        = min(a->np.actmin_sub[k], a->np.pmin_u[k]);
...
void LambdaDs(EMNR a) {
    ...
    for (k = 0; k < a->nps.msize; k++) {
        a->nps.PH1y[k] = 1.0
            / (1.0
                + (1.0 + a->nps.epsH1)
                    * exp(-a->nps.epsH1r * a->nps.lambda_y[k]
                        / a->nps.sigma2N[k]));
        a->nps.Pbar[k] = a->nps.alpha_Pbar * a->nps.Pbar[k]
            + (1.0 - a->nps.alpha_Pbar) * a->nps.PH1y[k];
        if (a->nps.Pbar[k] > 0.99) a->nps.PH1y[k] = min(a->nps.PH1y[k], 0.99);
        a->nps.EN2y[k] = (1.0 - a->nps.PH1y[k]) * a->nps.lambda_y[k]
            + a->nps.PH1y[k] * a->nps.sigma2N[k];
        a->nps.sigma2N[k] = a->nps.alpha_pow * a->nps.sigma2N[k]
            + (1.0 - a->nps.alpha_pow) * a->nps.EN2y[k];
    }
...
void aepf(EMNR a) {
    ...
    for (k = n; k < (a->ae.msize - n); k++) {
        a->ae.nmask[k] = 0.0;
        for (m = k - n; m <= (k + n); m++) a->ae.nmask[k] += a->mask[m];
...
void calc_gain(EMNR a) {
    ...
    for (k = 0; k < a->g.msize; k++) {
        a->g.lambda_y[k] = a->g.y[2 * k + 0] * a->g.y[2 * k + 0]
            + a->g.y[2 * k + 1] * a->g.y[2 * k + 1];
    }
    switch (a->g.npe_method) {
        case 0: LambdaD(a); break;
        case 1: LambdaDs(a); break;
    }
    switch (a->g.gain_method) {
        case 0: {
            ...
            for (k = 0; k < a->msize; k++) {
                gamma
                    = min(a->g.lambda_y[k] / a->g.lambda_d[k], a->g.gamma_max);
                eps_hat = a->g.alpha * a->g.prev_mask[k] * a->g.prev_mask[k]
                        * a->g.prev_gamma[k]
                    ...
                    * ((1.0 + v) * bessI0(0.5 * v) + v * bessI1(0.5 * v));
                {
                    ...
                    double eta = a->g.mask[k] * a->g.mask[k] * a->g.lambda_y[k]
                        / a->g.lambda_d[k];
                    double eps = eta / (1.0 - a->g.q);
                    double witchHat
                        = (1.0 - a->g.q) / a->g.q * exp(v2) / (1.0 + eps);
        ...
        case 1: {
            ...
            for (k = 0; k < a->g.msize; k++) {
                ...
                v = ehr * gamma;
                if ((a->g.mask[k] = ehr * exp(min(700.0, 0.5 * e1xb(v))))
                    > a->g.gmax)
                    a->g.mask[k] = a->g.gmax;
                if (a->g.mask[k] != a->g.mask[k]) a->g.mask[k] = 0.01;
                a->g.prev_gamma[k] = gamma;
        ...
        case 2: {
            ...
            for (k = 0; k < a->msize; k++) {
                ...
                a->g.mask[k] = getKey(a->g.GG, gamma, eps_hat)
                    * getKey(a->g.GGS, gamma, eps_p);
                a->g.prev_gamma[k] = gamma;
                a->g.prev_mask[k] = a->g.mask[k];
            }
            break;
...
void xemnr(EMNR a, int pos) {
    if (a->run && pos == a->position) {
        ...
        while (a->nsamps >= a->fsize) {
            for (i = 0, j = a->iaoutidx; i < a->fsize;
                 i++, j = (j + 1) % a->iasize)
            ...
            for (i = a->ovrlp; i > 0; i--) {
                ...
                for (j = sbegin, k = a->oainidx; j < a->incr + sbegin;
                     j++, k = (k + 1) % a->oasize) {
                    if (i == a->ovrlp)
...
PORT void SetRXAEMNRaeZetaThresh(int channel, double zetathresh) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].emnr.p->ae.zetaThresh = zetathresh;
    LeaveCriticalSection(&ch[channel].csDSP);
}

PORT void SetRXAEMNRaePsi(int channel, double psi) {
    EnterCriticalSection(&ch[channel].csDSP);
