...
void dR(int n, double* r, double* y) {
    ...
    for (k = 0; k < n - 1; k++) {
        ...
        gamma = 0.0;
        for (i = k + 1, j = 0; i > 0; i--, j++) gamma += r[i] * y[j];
        alpha = -(r[k + 2] + gamma) / beta;
        for (i = 0, j = k; i <= k; i++, j--) z[i] = y[i] + alpha * y[j];
...
void trI(int n, double* r, double* B) {
    int i, j, ni, nj;
    double gamma, t, scale, b;
    double* y = (double*)malloc0((n - 1) * sizeof(double));
    double* v = (double*)malloc0((n - 1) * sizeof(double));
    scale = 1.0 / r[0];
    for (i = 0; i < n; i++) r[i] *= scale;
    dR(n - 1, r, y);

    t = 0.0;
    for (i = 0; i < n - 1; i++) t += r[i + 1] * y[i];
    gamma = 1.0 / (1.0 + t);
    for (i = 0, j = n - 2; i < n - 1; i++, j--) v[i] = gamma * y[j];
    B[0] = gamma;
    for (i = 1, j = n - 2; i < n; i++, j--) B[i] = v[j];
    for (i = 1; i <= (n - 1) / 2; i++)
        for (j = i; j < n - i; j++)
            B[i * n + j] = B[(i - 1) * n + (j - 1)]
                + (v[n - j - 1] * v[n - i - 1] - v[i - 1] * v[j - 1]) / gamma;
    for (i = 0; i <= (n - 1) / 2; i++)
        for (j = i; j < n - i; j++) {
            b = B[i * n + j] *= scale;
            B[j * n + i] = b;
...
void asolve(int xsize, int asize, double* x, double* a) {
    ...
    for (i = 0; i <= asize; i++) {
        for (j = 0; j < xsize; j++) r[i] += x[j] * x[j - i];
    }
    z[0] = 1.0;
    beta = r[0];
    for (k = 0; k < asize; k++) {
        alpha = 0.0;
        for (j = 0; j <= k; j++) alpha -= z[j] * r[k + 1 - j];
        alpha /= beta;
        for (i = 0; i <= (k + 1) / 2; i++) {
            t = z[k + 1 - i] + alpha * z[i];
