...
PORT void* NewCriticalSection() { // used by VAC
    LPCRITICAL_SECTION cs_ptr;
    cs_ptr = (LPCRITICAL_SECTION)calloc(1, sizeof(CRITICAL_SECTION));
    return (void*)cs_ptr;
}

PORT void DestroyCriticalSection(LPCRITICAL_SECTION cs_ptr) { // used by VAC
    free((char*)cs_ptr);
}
...
void print_impulse(
    const char* filename, int N, double* impulse, int rtype, int pr_mode) {
    int i;
    FILE* file;
    if (pr_mode == 0)
    ...
    if (rtype == 0)
        for (i = 0; i < N; i++) fprintf(file, "%.17e\n", impulse[i]);
    else
        for (i = 0; i < N; i++)
            fprintf(
...
void print_peak_env(const char* filename, int N, double* buff, double thresh) {
    ...
    FILE* file;
    for (i = 0; i < N; i++) {
        new_peak = sqrt(buff[2 * i + 0] * buff[2 * i + 0]
            + buff[2 * i + 1] * buff[2 * i + 1]);
...
void print_peak_env_f2(
    const char* filename, int N, float* Ibuff, float* Qbuff) {
    int i;
    double peak = 0.0;
    double new_peak;
...
void print_iqc_values(const char* filename, int state, double env_in, double I,
    double Q, double ym, double yc, double ys, double thresh) {
    static unsigned int seqnum;
    double env_out;
    FILE* file;
...
void print_meter(const char* filename, double* meter, int enum_av, int enum_pk,
    int enum_gain) {
    FILE* file = fopen(filename, "a");
    if (enum_gain >= 0)
...
void print_message(
    const char* filename, const char* message, int p0, int p1, int p2) {
    FILE* file = fopen(filename, "a");
...
void print_window_gain(const char* filename, int wintype,
    double inv_coherent_gain, double inherent_power_gain) {
    FILE* file = fopen(filename, "a");
...
void __cdecl CalccPrintSamples(void* pargs) {
    ...
    for (i = 0; i < a->nsamps; i++) {
        env_tx = sqrt(a->txs[2 * i + 0] * a->txs[2 * i + 0]
            + a->txs[2 * i + 1] * a->txs[2 * i + 1]);
        env_rx = sqrt(a->rxs[2 * i + 0] * a->rxs[2 * i + 0]
            + a->rxs[2 * i + 1] * a->rxs[2 * i + 1]);
        fprintf(file, "%.12f  %.12f  %.12f      %.12f  %.12f  %.12f\n",
            a->txs[2 * i + 0], a->txs[2 * i + 1], env_tx, a->rxs[2 * i + 0],
            a->rxs[2 * i + 1], env_rx);
    }
    fflush(file);
    fclose(file);
...
void WriteAudioWDSP(
    double seconds, int rate, int size, double* indata, int mode, double gain) {
...
void WriteScaledAudioFile(void* arg) {
    ...
    for (i = 0; i < dstruct->n; i++)
        idata[i] = dstruct->ddata[i] >= 0.0
            ? (int)floor(conv * dstruct->ddata[i] / max + 0.5)
            : (int)ceil(conv * dstruct->ddata[i] / max - 0.5);
...
void WriteScaledAudio(double seconds, // number of seconds of audio to record
