...
void create_rxa(int channel) {
    ...
    // signal generator
    rxa[channel].gen0.p = create_gen(0, // run
    ...
    // adc (input) meter
    rxa[channel].adcmeter.p = create_meter(1, // run
    ...
    // notch database
    rxa[channel].ndb.p = create_notchdb(0, // master run for all nbp's
        1024); // max number of notches

    // notched bandpass
    rxa[channel].nbp0.p = create_nbp(1, // run, always runs
        0, // run the notches
        0, // position
        ch[channel].dsp_size, // buffer size
    ...
    // bandpass for snba
    rxa[channel].bpsnba.p = create_bpsnba(0, // bpsnba run flag
        0, // run the notches
        0, // position
        ch[channel].dsp_size, // size
    ...
    rxa[channel].sender.p = create_sender(channel == 0, // run
        0, // flag
        0, // mode
        ch[channel].dsp_size, // size
        rxa[channel].midbuff, // pointer to input buffer
    ...
    // S-meter
    rxa[channel].smeter.p = create_meter(1, // run
        0, // optional pointer to another 'run'
        ch[channel].dsp_size, // size
        rxa[channel].midbuff, // pointer to buffer
        ch[channel].dsp_rate, // samplerate
    ...
    // AM squelch
    rxa[channel].amsq.p = create_amsq(0, // run
        ch[channel].dsp_size, // buffer size
    ...
    // AM demod
    rxa[channel].amd.p = create_amd(0, // run - OFF by default
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
        0, // mode:  0->AM, 1->SAM
    ...
    // FM demod
    rxa[channel].fmd.p = create_fmd(0, // run
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
        ch[channel].dsp_rate, // sample rate
    ...
    // FM squelch
    rxa[channel].fmsq.p = create_fmsq(0, // run
        ch[channel].dsp_size, // buffer size
        ...
        &rxa[channel]
             .fmd.p->pllpole, // pointer to pole frequency of the fmd pll (Hz)
    ...
    // snba
    rxa[channel].snba.p = create_snba(0, // run
        rxa[channel].midbuff, // input buffer
        rxa[channel].midbuff, // output buffer
    ...
    // EQ
    {
        ...
        rxa[channel].eqp.p = create_eqp(0, // run - OFF by default
            ch[channel].dsp_size, // buffer size
            2048, // number of filter coefficients
            0, // minimum phase flag
            rxa[channel].midbuff, // pointer to input buffer
    ...
    // ANF
    rxa[channel].anf.p = create_anf(0, // run - OFF by default
        0, // position
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
    ...
    // ANR
    rxa[channel].anr.p = create_anr(0, // run - OFF by default
        0, // position
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
    ...
    // EMNR
    rxa[channel].emnr.p = create_emnr(0, // run
        0, // position
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // input buffer
        rxa[channel].midbuff, // output buffer
    ...
    // AGC
    rxa[channel].agc.p = create_wcpagc(1, // run
        3, // mode
        1, // peakmode = envelope
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
    ...
    // agc gain meter
    rxa[channel].agcmeter.p = create_meter(1, // run
        0, // optional pointer to another 'run'
        ch[channel].dsp_size, // size
        rxa[channel].midbuff, // pointer to buffer
        ch[channel].dsp_rate, // samplerate
    ...
    // bandpass filter
    rxa[channel].bp1.p
        = create_bandpass(1, // run - used only with ( AM || ANF || ANR || EMNR)
            0, // position
            ch[channel].dsp_size, // buffer size
            2048, // number of coefficients
    ...
    rxa[channel].sip1.p
        = create_siphon(1, // run - needed only for phase display
            0, // position
            0, // mode
            0, // disp
    ...
    // carrier block
    rxa[channel].cbl.p = create_cbl(0, // run - needed only if set to ON
        ch[channel].dsp_size, // buffer size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
        0, // mode
    ...
    // peaking filter
    rxa[channel].speak.p = create_speak(0, // run
        ch[channel].dsp_size, // buffer size,
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
        ch[channel].dsp_rate, // sample rate
    ...
    // multiple peak filter
    {
        ...
        rxa[channel].mpeak.p = create_mpeak(0, // run
            ch[channel].dsp_size, // size
            rxa[channel].midbuff, // pointer to input buffer
            rxa[channel].midbuff, // pointer to output buffer
            ch[channel].dsp_rate, // sample rate
    ...
    // patchpanel
    rxa[channel].panel.p = create_panel(channel, // channel number
        1, // run
        ch[channel].dsp_size, // size
        rxa[channel].midbuff, // pointer to input buffer
        rxa[channel].midbuff, // pointer to output buffer
...
void setInputSamplerate_rxa(int channel) {
    ...
    // input resampler
    setBuffers_resample(
        rxa[channel].rsmpin.p, rxa[channel].inbuff, rxa[channel].midbuff);
...
void setDSPSamplerate_rxa(int channel) {
    ...
    setBuffers_fmsq(rxa[channel].fmsq.p, rxa[channel].midbuff,
        rxa[channel].midbuff, rxa[channel].fmd.p->audio);
...
void setDSPBuffsize_rxa(int channel) {
    ...
    // dsp_size blocks
    setBuffers_gen(
        rxa[channel].gen0.p, rxa[channel].midbuff, rxa[channel].midbuff);
    ...
    setBuffers_nbp(
        rxa[channel].nbp0.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_nbp(rxa[channel].nbp0.p, ch[channel].dsp_size);
    setBuffers_bpsnba(
        rxa[channel].bpsnba.p, rxa[channel].midbuff, rxa[channel].midbuff);
    ...
    setBuffers_amsq(rxa[channel].amsq.p, rxa[channel].midbuff,
        rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_amsq(rxa[channel].amsq.p, ch[channel].dsp_size);
    setBuffers_amd(
        rxa[channel].amd.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_amd(rxa[channel].amd.p, ch[channel].dsp_size);
    setBuffers_fmd(
        rxa[channel].fmd.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_fmd(rxa[channel].fmd.p, ch[channel].dsp_size);
    setBuffers_fmsq(rxa[channel].fmsq.p, rxa[channel].midbuff,
        rxa[channel].midbuff, rxa[channel].fmd.p->audio);
    setSize_fmsq(rxa[channel].fmsq.p, ch[channel].dsp_size);
    setBuffers_snba(
        rxa[channel].snba.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_snba(rxa[channel].snba.p, ch[channel].dsp_size);
    setBuffers_eqp(
        rxa[channel].eqp.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_eqp(rxa[channel].eqp.p, ch[channel].dsp_size);
    setBuffers_anf(
        rxa[channel].anf.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_anf(rxa[channel].anf.p, ch[channel].dsp_size);
    setBuffers_anr(
        rxa[channel].anr.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_anr(rxa[channel].anr.p, ch[channel].dsp_size);
    setBuffers_emnr(
        rxa[channel].emnr.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_emnr(rxa[channel].emnr.p, ch[channel].dsp_size);
    setBuffers_bandpass(
        rxa[channel].bp1.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_bandpass(rxa[channel].bp1.p, ch[channel].dsp_size);
    setBuffers_wcpagc(
        rxa[channel].agc.p, rxa[channel].midbuff, rxa[channel].midbuff);
    ...
    setBuffers_cbl(
        rxa[channel].cbl.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_cbl(rxa[channel].cbl.p, ch[channel].dsp_size);
    setBuffers_speak(
        rxa[channel].speak.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_speak(rxa[channel].speak.p, ch[channel].dsp_size);
    setBuffers_mpeak(
        rxa[channel].mpeak.p, rxa[channel].midbuff, rxa[channel].midbuff);
    setSize_mpeak(rxa[channel].mpeak.p, ch[channel].dsp_size);
    setBuffers_panel(
        rxa[channel].panel.p, rxa[channel].midbuff, rxa[channel].midbuff);
...
PORT void SetRXAMode(int channel, int mode) {
    if (rxa[channel].mode != mode) {
        ...
        RXAbp1Check(channel, amd_run, rxa[channel].snba.p->run,
            rxa[channel].emnr.p->run, rxa[channel].anf.p->run,
            rxa[channel].anr.p->run);
...
void RXAbp1Check(int channel, int amd_run, int snba_run, int emnr_run,
    int anf_run, int anr_run) {
    BANDPASS a = rxa[channel].bp1.p;
    double gain;
    if (amd_run || snba_run || emnr_run || anf_run || anr_run)
        gain = 2.0;
    else
        gain = 1.0;
    if (a->gain != gain) setGain_bandpass(a, gain, 0);
}

void RXAbp1Set(int channel) {
    BANDPASS a = rxa[channel].bp1.p;
    int old = a->run;
    if ((rxa[channel].amd.p->run == 1) || (rxa[channel].snba.p->run == 1)
        || (rxa[channel].emnr.p->run == 1) || (rxa[channel].anf.p->run == 1)
...
void RXAbpsnbaCheck(int channel, int mode, int notch_run) {
    ...
    // 'run' and 'position' are examined at run time; no filter changes
    // required. Recalculate filter if frequencies OR 'run_notches' changed.
    if ((a->f_low != f_low) || (a->f_high != f_high)
...
PORT void RXASetMP(int channel, int mp) {
