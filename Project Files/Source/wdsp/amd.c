...
AMD create_amd(int run, int buff_size, double* in_buff, double* out_buff,
    ...
    double fmax, double zeta, double omegaN, double tauR, double tauI) {
...
void init_amd(AMD a) {
    ...
    a->g2 = -a->g1
        + 2.0
            * (1
                - exp(-a->omegaN * a->zeta / a->sample_rate)
                    * cos(a->omegaN / a->sample_rate
                        * sqrt(1.0 - a->zeta * a->zeta)));
    a->phs = 0.0;
    a->fil_out = 0.0;
    a->omega = 0.0;
        ...
        switch (a->mode) {
            ...
            case 0: // AM Demodulator
            {
                for (i = 0; i < a->buff_size; i++) {
                    audio = sqrt(a->in_buff[2 * i + 0] * a->in_buff[2 * i + 0]
                        + a->in_buff[2 * i + 1] * a->in_buff[2 * i + 1]);
                    if (a->levelfade) {
                        a->dc = a->mtauR * a->dc + a->onem_mtauR * audio;
                        a->dc_insert
                            = a->mtauI * a->dc_insert + a->onem_mtauI * audio;
            ...
            case 1: // Synchronous AM Demodulator with Sideband Separation
            {
                for (i = 0; i < a->buff_size; i++) {
                    ...
                    if (a->sbmode != 0) {
                        ...
                        for (j = 0; j < STAGES; j++) {
                            k = 3 * j;
                            a->a[k + 3] = a->c0[j] * (a->a[k] - a->a[k + 5])
                                + a->a[k + 2];
                            a->b[k + 3] = a->c1[j] * (a->b[k] - a->b[k + 5])
                                + a->b[k + 2];
                            a->c[k + 3] = a->c0[j] * (a->c[k] - a->c[k + 5])
                                + a->c[k + 2];
                            a->d[k + 3] = a->c1[j] * (a->d[k] - a->d[k + 5])
                                + a->d[k + 2];
                        }
                        ai_ps = a->a[OUT_IDX];
                        bi_ps = a->b[OUT_IDX];
                    ...
                    if (a->levelfade) {
                        a->dc = a->mtauR * a->dc + a->onem_mtauR * audio;
                        a->dc_insert
                            = a->mtauI * a->dc_insert + a->onem_mtauI * corr[0];
                        audio += a->dc_insert - a->dc;
                    }
                    a->out_buff[2 * i + 0] = audio;
                    a->out_buff[2 * i + 1] = audio;
...
PORT void SetRXAAMDRun(int channel, int run) {
    AMD a = rxa[channel].amd.p;
    if (a->run != run) {
        RXAbp1Check(channel, run, rxa[channel].snba.p->run,
...
PORT void SetRXAAMDFadeLevel(int channel, int levelfade) {
    EnterCriticalSection(&ch[channel].csDSP);
