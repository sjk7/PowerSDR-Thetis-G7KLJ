...
ANR create_anr(int run, int position, int buff_size, double* in_buff,
...
PORT void SetRXAANRTaps(int channel, int taps) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anr.p->n_taps = taps;
    flush_anr(rxa[channel].anr.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANRDelay(int channel, int delay) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anr.p->delay = delay;
    flush_anr(rxa[channel].anr.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANRGain(int channel, double gain) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anr.p->two_mu = gain;
    flush_anr(rxa[channel].anr.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANRLeakage(int channel, double leakage) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].anr.p->gamma = leakage;
    flush_anr(rxa[channel].anr.p);
    LeaveCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXAANRPosition(int channel, int position) {
    EnterCriticalSection(&ch[channel].csDSP);
