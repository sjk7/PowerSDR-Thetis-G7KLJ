...
#define MAX_ADVTIME                                                            \
    (0.002) // maximum deadtime (zero output) in advance of detected noise
...
PORT ANB create_anb(int run, int buffsize, double* in, double* out,
    double samplerate, double tau, double hangtime, double advtime,
...
PORT void flush_anb(ANB a) {
    EnterCriticalSection(&a->cs_update);
    initBlanker(a);
    LeaveCriticalSection(&a->cs_update);
}

PORT void xanb(ANB a) {
    double scale;
    double mag;
    int i;
    if (a->run) {
        ...
        for (i = 0; i < a->buffsize; i++) {
            mag = sqrt(a->in[2 * i + 0] * a->in[2 * i + 0]
                + a->in[2 * i + 1] * a->in[2 * i + 1]);
            ...
            switch (a->state) {
                ...
                case 2:
                    a->out[2 * i + 0] = 0.0;
                    a->out[2 * i + 1] = 0.0;
                    if (++a->atime > a->adv_count) a->state = 3;
                    break;
                case 3:
                    if (a->count > 0) a->htime = -a->count;
...
// The following is an example to follow for properties in the event that this
// function is used inside a channel.
...
PORT void pSetRCVRANBRun(ANB a, int run) {
    EnterCriticalSection(&a->cs_update);
    a->run = run;
    LeaveCriticalSection(&a->cs_update);
}

PORT void pSetRCVRANBBuffsize(ANB a, int size) {
    EnterCriticalSection(&a->cs_update);
    a->buffsize = size;
    LeaveCriticalSection(&a->cs_update);
}

PORT void pSetRCVRANBSamplerate(ANB a, int rate) {
    EnterCriticalSection(&a->cs_update);
...
PORT void pSetRCVRANBTau(ANB a, double tau) {
    EnterCriticalSection(&a->cs_update);
    a->tau = tau;
    initBlanker(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRANBHangtime(ANB a, double time) {
    EnterCriticalSection(&a->cs_update);
    a->hangtime = time;
    initBlanker(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRANBAdvtime(ANB a, double time) {
    EnterCriticalSection(&a->cs_update);
    a->advtime = time;
    initBlanker(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRANBBacktau(ANB a, double tau) {
    EnterCriticalSection(&a->cs_update);
    a->backtau = tau;
    initBlanker(a);
    LeaveCriticalSection(&a->cs_update);
...
PORT void pSetRCVRANBThreshold(ANB a, double thresh) {
    EnterCriticalSection(&a->cs_update);
    a->threshold = thresh;
    LeaveCriticalSection(&a->cs_update);
}
...
PORT void create_anbEXT(int id, int run, int buffsize, double samplerate,
    double tau, double hangtime, double advtime, double backtau,
    double threshold) {
    panb[id] = create_anb(run, buffsize, 0, 0, samplerate, tau, hangtime,
        advtime, backtau, threshold);
}

PORT void destroy_anbEXT(int id) {
    destroy_anb(panb[id]);
}

PORT void flush_anbEXT(int id) {
    flush_anb(panb[id]);
}

PORT void xanbEXT(int id, double* in, double* out) {
    ANB a = panb[id];
    a->in = in;
    a->out = out;
    xanb(a);
...
PORT void SetEXTANBRun(int id, int run) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->run = run;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetEXTANBBuffsize(int id, int size) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->buffsize = size;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetEXTANBSamplerate(int id, int rate) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->samplerate = (double)rate;
    initBlanker(a);
...
PORT void SetEXTANBTau(int id, double tau) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->tau = tau;
    initBlanker(a);
...
PORT void SetEXTANBHangtime(int id, double time) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->hangtime = time;
    initBlanker(a);
...
PORT void SetEXTANBAdvtime(int id, double time) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->advtime = time;
    initBlanker(a);
...
PORT void SetEXTANBBacktau(int id, double tau) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->backtau = tau;
    initBlanker(a);
...
PORT void SetEXTANBThreshold(int id, double thresh) {
    ANB a = panb[id];
    EnterCriticalSection(&a->cs_update);
    a->threshold = thresh;
    LeaveCriticalSection(&a->cs_update);
...
PORT void xanbEXTF(int id, float* I, float* Q) {
    int i;
    ANB a = panb[id];
    a->in = a->legacy;
    a->out = a->legacy;
