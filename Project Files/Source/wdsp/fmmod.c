...
FMMOD create_fmmod(int run, int size, double* in, double* out, int rate,
    double dev, double f_low, double f_high, int ctcss_run, double ctcss_level,
    double ctcss_freq, int bp_run, int nc, int mp) {
    ...
    a->mp = mp;
    calc_fmmod(a);
    impulse = fir_bandpass(
        a->nc, -a->bp_fc, +a->bp_fc, a->samplerate, 0, 1, 1.0 / (2 * a->size));
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            if (a->ctcss_run) {
                ...
                a->out[2 * i + 0] = a->tscale
                    * (a->in[2 * i + 0] + a->ctcss_level * cos(a->tphase));
            }
...
PORT void SetTXAFMDeviation(int channel, double deviation) {
    ...
    double* impulse = fir_bandpass(
        a->nc, -bp_fc, +bp_fc, a->samplerate, 0, 1, 1.0 / (2 * a->size));
...
PORT void SetTXACTCSSFreq(int channel, double freq) {
    FMMOD a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetTXAFMNC(int channel, int nc) {
    FMMOD a;
    double* impulse;
    EnterCriticalSection(&ch[channel].csDSP);
    a = txa[channel].fmmod.p;
    if (a->nc != nc) {
        a->nc = nc;
        impulse = fir_bandpass(a->nc, -a->bp_fc, +a->bp_fc, a->samplerate, 0, 1,
            1.0 / (2 * a->size));
...
PORT void SetTXAFMMP(int channel, int mp) {
    FMMOD a;
    a = txa[channel].fmmod.p;
    if (a->mp != mp) {
        a->mp = mp;
