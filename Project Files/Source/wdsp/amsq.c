...
AMSQ create_amsq(int run, int size, double* in, double* out, double* trigger,
    int rate, double avtau, double tup, double tdown, double tail_thresh,
    double unmute_thresh, double min_tail, double max_tail, double muted_gain) {
...
enum _amsqstate { MUTED, INCREASE, UNMUTED, TAIL, DECREASE };
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            ...
            switch (a->state) {
                ...
                case INCREASE:
                    a->out[2 * i + 0]
                        = a->in[2 * i + 0] * a->cup[a->ntup - a->count];
                    a->out[2 * i + 1]
                        = a->in[2 * i + 1] * a->cup[a->ntup - a->count];
                    if (a->count-- == 0) a->state = UNMUTED;
                    break;
                case UNMUTED:
                    if (a->avsig < a->tail_thresh) {
                        a->state = TAIL;
                        if ((siglimit = a->avsig) > 1.0) siglimit = 1.0;
                        a->count = (int)((a->min_tail
                                             + (a->max_tail - a->min_tail)
                                                 * (1.0 - siglimit))
                            * a->rate);
                    }
                ...
                case DECREASE:
                    a->out[2 * i + 0]
                        = a->in[2 * i + 0] * a->cdown[a->ntdown - a->count];
                    a->out[2 * i + 1]
                        = a->in[2 * i + 1] * a->cdown[a->ntdown - a->count];
                    if (a->count-- == 0) a->state = MUTED;
                    break;
            }
        }
    } else if (a->in != a->out)
...
PORT void SetRXAAMSQMaxTail(int channel, double tail) {
    AMSQ a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetTXAAMSQMutedGain(
    int channel, double dBlevel) { // dBlevel is negative
    AMSQ a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetTXAAMSQThreshold(int channel, double threshold) {
    double thresh = pow(10.0, threshold / 20.0);
    EnterCriticalSection(&ch[channel].csDSP);
