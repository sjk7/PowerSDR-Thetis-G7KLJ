...
void xmav(MAV a, int input, double* output) {
    if (a->load >= a->ringmax) a->sum -= a->ring[a->i];
...
void xaamav(AAMAV a, int input, double* output) {
    ...
    if (a->load >= a->ringmin) {
        ...
        *output = (1.0 - frac) * a->nom_ratio
            + frac * ((double)a->neg / (double)a->pos);
    } else {
        *output = a->nom_ratio;
    }
    a->i = (a->i + 1) & a->mask;
...
void calc_rmatch(RMATCH a) {
    ...
    a->pr_gain = a->prop_gain * 48000.0
        / (double)a->nom_outrate; // adjust gain for rate
...
RMATCH create_rmatch(
    ...
    double startup_delay, // time (seconds) to delay before beginning
                          // measurements to control variable resampler
    int auto_ringsize, // 0 specified ringsize is used; 1 ringsize is
                       // auto-optimized - FEATURE NOT IMPLEMENTED!!
    ...
    int ffmav_min, // minimum feed-forward moving average size to put full
                   // weight on data in the ring
    int ffmav_max, // maximum feed-forward moving average size - MUST BE A POWER
                   // OF TWO!
    ...
    int prop_ringmax, // proportional feedback max moving average ringsize -
                      // MUST BE A POWER OF TWO!
    double prop_gain, // proportional feedback gain factor
    int varmode, // 0 - use same var for all samples of the buffer; 1 -
                 // interpolate from old_var to this var
...
void control(RMATCH a, int change) {
    {
        ...
        a->feed_forward = a->ff_alpha * current_ratio
            + (1.0 - a->ff_alpha) * a->feed_forward;
    }
    {
...
void blend(RMATCH a) {
    ...
    for (i = 0, j = a->iout; i <= a->ntslew; i++, j = (j + 1) % a->rsize) {
        a->ring[2 * j + 0] = a->cslew[i] * a->ring[2 * j + 0]
            + (1.0 - a->cslew[i]) * a->baux[2 * i + 0];
        a->ring[2 * j + 1] = a->cslew[i] * a->ring[2 * j + 1]
            + (1.0 - a->cslew[i]) * a->baux[2 * i + 1];
    }
}
...
PORT void xrmatchIN(void* b, double* in) {
    ...
    if (InterlockedAnd(&a->run, 1)) {
        ...
        if (!a->control_flag) {
            ...
            if ((a->readsamps >= a->read_startup)
                && (a->writesamps >= a->write_startup))
                a->control_flag = 1;
        }
...
PORT void xrmatchOUT(void* b, double* out) {
    RMATCH a = (RMATCH)b;
    if (InterlockedAnd(&a->run, 1)) {
        int first, second;
        a->out = out;
...
PORT void getRMatchDiags(
    void* b, int* underflows, int* overflows, double* var, int* ringsize) {
    RMATCH a = (RMATCH)b;
...
PORT void resetRMatchDiags(void* b) {
    RMATCH a = (RMATCH)b;
    InterlockedExchange(&a->underflows, 0);
    InterlockedExchange(&a->overflows, 0);
}

PORT void forceRMatchVar(void* b, int force, double fvar) {
    RMATCH a = (RMATCH)b;
    EnterCriticalSection(&a->cs_var);
    a->force = force;
    a->fvar = fvar;
...
PORT void* create_rmatchV(
    int in_size, int out_size, int nom_inrate, int nom_outrate, int ringsize) {
    return (void*)create_rmatch(1, // run
...
PORT void destroy_rmatchV(void* ptr) {
    RMATCH a = (RMATCH)ptr;
    destroy_rmatch(a);
}

PORT void setRMatchInsize(void* ptr, int insize) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchOutsize(void* ptr, int outsize) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchNomInrate(void* ptr, int nom_inrate) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchNomOutrate(void* ptr, int nom_outrate) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchRingsize(void* ptr, int ringsize) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void* create_rmatchLegacyV(
    int in_size, int out_size, int nom_inrate, int nom_outrate, int ringsize) {
    return (void*)create_rmatch(1, // run
        0, // input buffer, stuffed in other calls
        0, // output buffer, stuffed in other calls
        ...
        4096, // proportional feedback min moving av ringsize
        16384, // proportional feedback max moving av ringsize - POWER OF TWO!
        1.0e-06, // proportional feedback gain  ***W4WMT - reduce loop gain a
                 // bit for PowerSDR to help Primary buffers > 512
        0, // linearly interpolate cvar by sample  ***W4WMT - set varmode = 0
           // for PowerSDR (doesn't work otherwise!?!)
        0.003); // slew time (seconds)
}
