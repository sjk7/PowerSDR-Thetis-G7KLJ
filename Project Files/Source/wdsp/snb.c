...
void calc_snba(SNBA d) {
    ...
    if (d->inrate != d->internalrate)
        ...
        d->resamprun = 0;
    d->inresamp = create_resample(d->resamprun, d->bsize, d->in, d->inbuff,
        d->inrate, d->internalrate, 0.0, 0, 2.0);
    setFCLow_resample(d->inresamp, 250.0);
    d->outresamp = create_resample(d->resamprun, d->isize, d->outbuff, d->out,
        d->internalrate, d->inrate, 0.0, 0, 2.0);
...
SNBA create_snba(int run, double* in, double* out, int inrate, int internalrate,
    int bsize, int ovrlp, int xsize, int asize, int npasses, double k1,
    double k2, int b, int pre, int post, double pmultmin, double out_low_cut,
    double out_high_cut) {
...
void ATAc0(int n, int nr, double* A, double* r) {
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < nr; j++) r[i] += A[j * n + i] * A[j * n + 0];
}
...
void multXKE(double* a, double* xk, int m, int q, int p, double* vout) {
    ...
    for (i = 0; i < m; i++) {
        for (k = i; k < p; k++) vout[i] += a[i * q + k] * xk[k];
        for (k = q - p; k <= q - m + i; k++) vout[i] += a[i * q + k] * xk[k];
    }
}
...
void multAv(double* a, double* v, int m, int q, double* vout) {
    ...
    memset(vout, 0, m * sizeof(double));
    for (i = 0; i < m; i++) {
        for (k = 0; k < q; k++) vout[i] += a[i * q + k] * v[k];
    }
}
...
void xHat(int xusize, int asize, double* xk, double* a, double* xout) {
    ...
    for (i = 0; i < xusize; i++) {
        A1[i * xusize + i] = 1.0;
        k = i + 1;
        for (j = k; j < k + asize; j++) A1[j * xusize + i] = -a[j - k];
    }
...
void invf(int xsize, int asize, double* a, double* x, double* v) {
    ...
    for (i = xsize - asize; i < xsize; i++) {
        for (j = 0; j < asize; j++) v[i] += a[j] * x[i - 1 - j];
        v[i] = x[i] - v[i];
    }
}
        ...
        switch (bstate) {
            ...
            case 2:
                ...
                if (bcount > d->sdet.b)
                    ...
                    for (j = bsamp; j < bsamp + bcount - 1; j++) detout[j] = 1;
                    bstate = 1;
                }
                break;
        }
...
int scanFrame(int xsize, int pval, double pmultmin, int* det, int* bimp,
    int* limp, int* befimp, int* aftimp, int* p_opt, int* next) {
    int inflag = 0;
    ...
    for (i = 0; i < nimp; i++) {
        if (befimp[i] < aftimp[i])
            ...
            p_opt[i] = aftimp[i];
        if (p_opt[i] > pval) p_opt[i] = pval;
        if (p_opt[i] < (int)(pmultmin * limp[i])) p_opt[i] = -1;
    }
...
void execFrame(SNBA d, double* x) {
    ...
    for (i = 0; i < d->xsize; i++) {
        if (d->exec.detout[i] != 0) x[i] = 0.0;
    }
    nimp = scanFrame(d->xsize, d->exec.asize, d->scan.pmultmin, d->exec.detout,
        bimp, limp, befimp, aftimp, p_opt, &next);
    for (pass = 0; pass < d->exec.npasses; pass++) {
        memcpy(d->exec.unfixed, d->exec.detout, d->xsize * sizeof(int));
        for (k = 0; k < nimp; k++) {
            if (k > 0)
                scanFrame(d->xsize, d->exec.asize, d->scan.pmultmin,
                    d->exec.unfixed, bimp, limp, befimp, aftimp, p_opt, &next);

            if ((p = p_opt[next]) > 0) {
                asolve(d->xsize, p, x, d->exec.a);
                xHat(limp[next], p, &x[bimp[next] - p], d->exec.a,
                    d->exec.xHout);
                memcpy(
                    &x[bimp[next]], d->exec.xHout, limp[next] * sizeof(double));
                memset(
                    &d->exec.unfixed[bimp[next]], 0, limp[next] * sizeof(int));
            } else {
                memcpy(&x[bimp[next]], &d->exec.savex[bimp[next]],
                    limp[next] * sizeof(double));
            }
        }
    }
}
...
void xsnba(SNBA d) {
    if (d->run) {
        ...
        while (d->nsamps >= d->incr) {
            memcpy(&d->xaux[d->xsize - d->incr], &d->inaccum[d->iaoutidx],
                d->incr * sizeof(double));
            execFrame(d, d->xaux);
            ...
            memmove(d->xbase, &d->xbase[d->incr],
                (2 * d->xsize - d->incr) * sizeof(double));
        }
        for (i = 0; i < d->isize; i++) {
...
PORT void SetRXASNBARun(int channel, int run) {
    ...
    if (a->run != run) {
        RXAbpsnbaCheck(
            channel, rxa[channel].mode, rxa[channel].ndb.p->master_run);
        RXAbp1Check(channel, rxa[channel].amd.p->run, run,
...
// This is a thin wrapper for a notched-bandpass filter (nbp).  The basic
// difference is that it provides for its input and output to happen at
// different points in the processing pipeline.  This means it must include a
// buffer, 'buff'.  Its input and output are done via functions xbpshbain() and
// xbpshbaout().
...
BPSNBA create_bpsnba(int run, int run_notches, int position, int size, int nc,
    int mp, double* in, double* out, int rate, double abs_low_freq,
    double abs_high_freq, double f_low, double f_high, int wintype, double gain,
    int autoincr, int maxpb, NOTCHDB* ptraddr) {
...
void xbpsnbaout(BPSNBA a, int position) {
    if (a->run && a->position == position) xnbp(a->bpsnba, 0);
}
...
PORT void RXABPSNBASetNC(int channel, int nc) {
    BPSNBA a;
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void RXABPSNBASetMP(int channel, int mp) {
    BPSNBA a;
    a = rxa[channel].bpsnba.p;
    if (a->mp != mp) {
        a->mp = mp;
