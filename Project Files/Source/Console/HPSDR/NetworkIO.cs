...
partial class NetworkIO {
    ...
    // get the name of this PC and, using it, the IP address of the first
    // adapter
    // static string strHostName = Dns.GetHostName();
    // public static IPAddress[] addr =
    // Dns.GetHostEntry(Dns.GetHostName()).AddressList; get a socket to send and
    // receive on
    ...
    public static int initRadio() {
        ...
        // make a list of all the adapters that we found in
        // Dns.GetHostEntry(strHostName).AddressList
        foreach (IPAddress a in addr) {
            // make sure to get only IPV4 addresses!
            // test added because Erik Anderson noted an issue on Windows 7. May
            // have been in the socket construction or binding below.
        ...
        if (enableStaticIP) {
            ...
            IPEndPoint localEndPoint
                = QueryRoutingInterface(sock, remoteEndPoint);
            EthernetHostIPAddress
                = IPAddress.Parse(localEndPoint.Address.ToString()).ToString();

            sock.Close();
            sock = null;
            if (IPAddress.TryParse(EthernetHostIPAddress, out IPAddress hostIP)
                && IPAddress.TryParse(
                    HpSdrHwIpAddress, out IPAddress targetIP)) {
                System.Console.WriteLine(String.Format(
                    "Attempting connect to host adapter {0}, Static IP {1}",
                    EthernetHostIPAddress, HpSdrHwIpAddress));
                ...
                if (DiscoverRadioOnPort(ref hpsdrd, hostIP, targetIP)) {
                    ...
                    if (hpsdrd.Count > 0) {
                        ...
                        // clear the list and put our single element in it, if
                        // we found it.
                        hpsdrd.Clear();
                        if (m2 != null) {
                            hpsdrd.Add(m2);
        ...
        if (FastConnect && (EthernetHostIPAddress.Length > 0)
            && (HpSdrHwIpAddress.Length > 0)) {
            if (IPAddress.TryParse(EthernetHostIPAddress, out IPAddress hostIP)
                && IPAddress.TryParse(
                    HpSdrHwIpAddress, out IPAddress targetIP)) {
                System.Console.WriteLine(String.Format(
                    "Attempting fast re-connect to host adapter {0}, IP {1}",
                    EthernetHostIPAddress, HpSdrHwIpAddress));
        ...
        EthernetHostIPAddress
            = hpsdrd[chosenDevice].hostPortIPAddress.ToString();
        ...
        rc = nativeInitMetis(HpSdrHwIpAddress, EthernetHostIPAddress,
            EthernetHostPort, (int)CurrentRadioProtocol);
        return -rc;
    }
    ...
    // checks if the firmware versions are consistent - returns false if they
    // are not and set fwVersionmsg to point to an appropriate message
    private static bool fwVersionsGood() { return true; }
    ...
    unsafe public static int GetDotDashPTT() {
        ...
        if (legacyDotDashPTT) // old style dot and ptt overloaded on 0x1 bit,
                              // new style dot on 0x4, ptt on 0x1
        {
            if ((bits & 0x1) != 0) {
                bits |= 0x4;
            }
    ...
    public static void freqCorrectionChanged() {
        if (!Console.FreqCalibrationRunning) // we can't be applying freq
                                             // correction when cal is running
        {
    ...
    private static double[][] lastVFOfreq = new double
        [2][] { new double[] { 0.0, 0.0, 0.0, 0.0 }, new double[] { 0.0 } };
    unsafe public static void VFOfreq(int id, double f, int tx) {
        ...
        if (f_freq >= 0)
            if (CurrentRadioProtocol == RadioProtocol.USB)
                ...
                SetVFOfreq(
                    id, Freq2PW(f_freq), tx); // sending phaseword to firmware
    }
    ...
    public static string Network_interfaces
        = null; // holds a list with the description of each Network Adapter
    public static int adapterSelected
        = 1; // from Setup form, the number of the Network Adapter to use

    public static void GetNetworkInterfaces() {
        // creat a string that contains the name and speed of each Network
        // adapter
        ...
        foreach (var netInterface in nics) {
            if ((netInterface.OperationalStatus == OperationalStatus.Up
                ...
                foreach (var addrInfo in netInterface.GetIPProperties()
                             .UnicastAddresses) {
                    if (addrInfo.Address.AddressFamily
                        == AddressFamily.InterNetwork) {
                        NicProperties np = new NicProperties { ipv4Address
                            = addrInfo.Address,
                            ipv4Mask = addrInfo.IPv4Mask };
                        nicProperties.Add(np);
                    }
                }
            }

            // if the length of the network adapter name is > 31 characters then
            // trim it, if shorter then pad to 31. Need to use fixed width font
            // - Courier New
            string speed
                = "  " + (netInterface.Speed / 1000000).ToString() + "T";
            if (netInterface.Description.Length > 31) {
                Network_interfaces += adapterNumber++.ToString() + ". "
                    + netInterface.Description.Remove(31) + speed + "\n";
            } else {
                Network_interfaces += adapterNumber++.ToString() + ". "
                    + netInterface.Description.PadRight(31, ' ') + speed + "\n";
            }
    ...
    private static bool DiscoverRadioOnPort(ref List<HPSDRDevice> hpsdrdList,
        IPAddress HostIP, IPAddress targetIP) {
        ...
        // Listen to data on this PC's IP address. Allow the program to allocate
        // a free port.
        iep = new IPEndPoint(
            HostIP, LocalPort); // was iep = new IPEndPoint(ipa, 0);

        try {
            // bind to socket and Port
            socket.Bind(iep);
            //   socket.ReceiveBufferSize = 0xFFFFF;   // no lost frame counts
            //   at 192kHz with this setting
            socket.Blocking = true;

            IPEndPoint localEndPoint = (IPEndPoint)socket.LocalEndPoint;
            System.Console.WriteLine(
                "Looking for radio using host adapter IP {0}, port {1}",
            ...
            System.Console.WriteLine(
                "Caught an exception while binding a socket to endpoint {0}.  Exception was: {1} ",
                iep.ToString(), ex.ToString());
            result = false;
        } finally {
            socket.Close();
            socket = null;
    ...
    private static bool Discovery(
        ref List<HPSDRDevice> hpsdrdList, IPEndPoint iep, IPAddress targetIP) {
        ...
        string receivedIP; // the IP address Metis obtains; assigned, from DHCP
                           // or APIPA (169.254.x.y)
        ...
        // send every second until we either find a radio or exceed the number
        // of attempts
        while (!radio_found) // #### djm should loop for a while in case there
                             // are multiple radios
        {
            ...
            if (enableStaticIP && static_ip_ok)
                ...
                broadcast
                    = new IPEndPoint(IPAddressExtensions.GetBroadcastAddress(
                                         hostPortIPAddress, hostPortMask),
                        DiscoveryPort);
            ...
            do {
                ...
                data_available = socket.Poll(100000,
                    SelectMode.SelectRead); // wait 100 msec  for time out

                if (data_available) {
                    EndPoint remoteEP = new IPEndPoint(IPAddress.None, 0);
                    recv = socket.ReceiveFrom(data,
                    ...
                    // IPEndPoint remoteIpEndPoint = socket.RemoteEndPoint as
                    // IPEndPoint; System.Console.WriteLine(" Remote Port # = ",
                    // remoteIpEndPoint.Port);

                    string junk
                        = Convert.ToString(remoteEP); // see code in DataLoop
                    ...
                    // check for HPSDR frame ID and type 2 (not currently
                    // streaming data, which also means 'not yet in use')
                    // changed to filter a proper discovery packet from the
                    // radio, even if already in use!  This prevents the need to
                    // power-cycle the radio.
                    if (((data[0] == 0xef) && // Protocol-USB (P1) Busy
                            (data[1] == 0xfe) && (data[2] == 0x3))
                    ...
                    if (((data[0] == 0xef) && // Protocol-USB (P1)
                            (data[1] == 0xfe) && (data[2] == 0x2))
                        || ((data[0] == 0x0) && // Protocol-ETH (P2)
                            (data[1] == 0x0) && (data[2] == 0x0)
                            && (data[3] == 0x0) && (data[4] == 0x2))) {
                        if (data[2] == 0x2)
                        ...
                        // get IP address from the IPEndPoint passed to
                        // ReceiveFrom.
                        ...
                        System.Console.WriteLine(
                            "IP from IP Header = " + receivedIP);
                        System.Console.WriteLine(
                            "MAC address from payload = " + MAC);

                        if (!SameSubnet(receivedIPAddr, hostPortIPAddress,
                                hostPortMask)) {
                            // device is NOT on the subnet that this port
                            // actually services.  Do NOT add to list!
                            System.Console.WriteLine(
                                "Not on subnet of host adapter! Adapter IP {0}, Adapter mask {1}",
                                hostPortIPAddress.ToString(),
                                hostPortMask.ToString());
                        } else if (MAC.Equals("00-00-00-00-00-00")) {
                            System.Console.WriteLine(
                                "Rejected: contains bogus MAC address of all-zeroes");
                        } else {
                            HPSDRDevice hpsdrd = new HPSDRDevice {
                                IPAddress = receivedIP, MACAddress = MAC,
                                deviceType
                                = CurrentRadioProtocol == RadioProtocol.USB
                                    ? (HPSDRHW)data[10]
                                    : (HPSDRHW)data[11],
                                ...
                                MetisVersion = data[19], numRxs = data[20],
                            ...
                            if (targetIP != null) {
                                // if
                                // (hpsdrd.IPAddress.CompareTo(targetIP.ToString())
                                // == 0)
                    ...
                    if ((++time_out) > 5) {
                        ...
                        if (RadioProtocolSelected != RadioProtocol.Auto)
                            RadioProtocolSelected
                                = RadioProtocol
                                      .Auto; // you might succeed next time
                        return false;
                    }
                    static_ip_ok = false;
                }
    ...
    /// <summary>
    /// Determines whether the board and hostAdapter IPAddresses are on the same
    /// subnet, using subnetMask to make the determination.  All addresses are
    /// IPV4 addresses
    /// </summary>
    /// <param name="board">IP address of the remote device</param>
    /// <param name="hostAdapter">IP address of the ethernet adapter</param>
    /// <param name="subnetMask">subnet mask to use to determine if the above 2
    /// IPAddresses are on the same subnet</param> <returns>true if same subnet,
    /// false otherwise</returns>
    public static bool SameSubnet(
        IPAddress board, IPAddress hostAdapter, IPAddress subnetMask) {
        ...
        for (int i = 0; i < boardBytes.Length; ++i) {
            ...
            byte hostAdapterByte
                = (byte)(hostAdapterBytes[i] & subnetMaskBytes[i]);
    ...
    private static IPEndPoint QueryRoutingInterface(
        Socket socket, IPEndPoint remoteEndPoint) {
        ...
        socket.IOControl(
            IOControlCode.RoutingInterfaceQuery, remoteAddrBytes, outBytes);
        for (int i = 0; i < address.Size; i++) {
...
public static class IPAddressExtensions {
    public static IPAddress GetBroadcastAddress(
        this IPAddress address, IPAddress subnetMask) {
        ...
        for (int i = 0; i < broadcastAddress.Length; i++) {
            broadcastAddress[i]
                = (byte)(ipAdressBytes[i] | (subnetMaskBytes[i] ^ 255));
        }
