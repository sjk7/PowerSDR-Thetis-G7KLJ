...
#region Constructors
    ...
    // Commands getting this far have been checked for a valid prefix, a correct
    // suffix length, and a terminator.  All we need to do in this class is to
    // decide what kind of command (read or set) and execute it.  Only read
    // commands generate answers.

#region Standard CAT Methods A - F

    // Sets or reads the Audio Gain control
    public string AG(string s) {
        if (s.Length == parser.nSet) // if the length of the parameter legal for
                                     // setting this prefix
        {
            int raw = Convert.ToInt32(s.Substring(1));
            int af = (int)Math.Round(
                raw / 2.55, 0); // scale 255:100 (Kenwood vs SDR)
        ...
        } else if (s.Length == parser.nGet) // if this is a read command
        {
            ...
            //				return AddLeadingZeros(console.AF);		// Get the console
            //setting
    ...
    public string CN(string s) { return ZZTB(s); }

    // Reads or sets the CTCSS enable button
    public string CT(string s) { return ZZTA(s); }

    // Moves the VFO A frequency by the step size set on the console
    public string DN() {
        //			int step = console.StepSize;
        //			double[] wheel_tune_list;
        //			wheel_tune_list = new double[13];		// initialize wheel tuning
        //list array 			wheel_tune_list[0]  =  0.000001; 			wheel_tune_list[1]  =
        //0.000010; 			wheel_tune_list[2]  =  0.000050; 			wheel_tune_list[3]  =
        //0.000100; 			wheel_tune_list[4]  =  0.000250; 			wheel_tune_list[5]  =
        //0.000500; 			wheel_tune_list[6]  =  0.001000; 			wheel_tune_list[7]  =
        //0.005000; 			wheel_tune_list[8]  =  0.009000; 			wheel_tune_list[9]  =
        //0.010000; 			wheel_tune_list[10] =  0.100000; 			wheel_tune_list[11]
        //=  1.000000; 			wheel_tune_list[12] = 10.000000;
        //
    ...
    public string FA(string s) {
        //			if(s.Length == parser.nSet)
        //			{
        //				s = s.Insert(5,separator);		//reinsert the global numeric
        //separator 				console.VFOAFreq = double.Parse(s); 				return "";
        //			}
        //			else if(s.Length == parser.nGet)
        //				return StrVFOFreq("A");
        //			else
...
#region Standard CAT Methods G - M
    ...
    public string IF() {
        ...
        if (retval) split = "1";
        // Get the mode
        //			temp = Mode2KString(console.RX1DSPMode);   //possible fix for
        //SAM problem 			if(temp == parser.Error1) 				temp = " ";
        ...
        rtn += f;
        //			rtn += StrVFOFreq("A");						// VFO A frequency			11
        //bytes
        ...
        rtn += tx; // tx-rx status				 1 byte
                   //			rtn += temp;
                   //			rtn += Mode2KString(console.RX1DSPMode);	//
                   //current mode			 1 bytes
        ...
        //			if(ITValue < 0)
        //				cmd_string +=
        //"-"+Convert.ToString(Math.Abs(ITValue)).PadLeft(5,'0'); 			else
        //				cmd_string +=
        //"+"+Convert.ToString(Math.Abs(ITValue)).PadLeft(5,'0');
        //
        //			// Add the RIT/XIT status bits (P4 and P5, one byte each)
        //			cmd_string += rit+xit;
        //
        //			// Skip the memory channel stuff, the SDR1K doesn't use banks
        //and channels per se
        //			// (P6 - 1 byte, P7 - 2 bytes)
        //			cmd_string += "000";
        //
        //			// Set the current MOX state (P8 - 1 byte)(what the heck is this
        //for?) 			if(console.MOX) 				cmd_string += "1"; 			else 				cmd_string += "0";
        //
        //			// Get the SDR mode.  (P9 - 1 byte)
        //			temp = Mode2KString(console.RX1DSPMode);
        //			if(temp.Length == 1)	// if the answer is not an error message
        //?; 				cmd_string += temp; 			else 				cmd_string += " ";	// return a blank if
        //it's an error
        //
        //			// Set the FR/FT commands which determines the transmit and
        //receive
        ...
        //			// The Scan code might be implemented but the frequency range
        //would
        ...
        //			cmd_string += ZZSP("");
        //
        //			// Set the remaining CTCSS tone and shift bits to 0 (P13 - P15,
        //4 bytes) 			cmd_string += "0000";
        //
        //			return cmd_string;
    }
    ...
    public string KY(string s) {
        ...
        switch (console.RX1DSPMode) {
            ...
            case DSPMode.LSB:
            case DSPMode.USB:
                if (console.RX1Band >= Band.B160M
                    && console.RX1Band <= Band.B40M)
            ...
            default: console.RX1DSPMode = DSPMode.CWU; break;
        }
    ...
    public string MG(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            n = Math.Min(100, n);
            int mg
                = (int)Math.Round(n / 1.43, 0); // scale 100:70 (Kenwood vs SDR)
...
#region Standard CAT Methods N - Q
    ...
    public string OF(string s) { return ZZOT(s); }

    // Sets or reads the repeater offset direction
    public string OS(string s) { return ZZOS(s); }
...
#region Standard CAT Methods R - Z
    ...
    // Decrements RIT
    public string RD(string s) { return ZZRD(s); }
    ...
    // Increments RIT
    public string RU(string s) { return ZZRU(s); }
    ...
    public string SH(string s) {
        if (s.Length == parser.nSet) {
            ...
            switch (console.RX1DSPMode) {
                ...
                default: return Frequency2Code(console.RX1FilterHigh, "SH");
            }
        } else {
            return parser.Error1;
        }
    ...
    public string SL(string s) {
        if (s.Length == parser.nSet) {
            ...
            switch (console.RX1DSPMode) {
                ...
                default: return Frequency2Code(console.RX1FilterLow, "SL");
            }
        } else {
            return parser.Error1;
        }
    ...
    public string SQ(string s) {
        ...
        // convert to a double and add the scale factor (160 = 255)
        {
            ...
            int isquelch = Convert.ToInt32(
                (1.0 - (Math.Abs(console.Squelch / 160.0))) * 255.0);
...
#region Extended CAT Methods ZZA - ZZF
    ...
    public string ZZAE(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            if (numsteps >= 0 && step <= 99) {
                console.VFOAFreq
                    = console.CATVFOA - stepFreq * (double)numsteps;
                return "";
            } else
                return parser.Error1;
        } else
    ...
    public string ZZBE(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            if (numsteps >= 0 && step <= 99) {
                console.VFOBFreq
                    = console.CATVFOB - stepFreq * (double)numsteps;
                return "";
            } else
                return parser.Error1;
        } else
    ...
    public string ZZBS(string s) { return GetBand(s); }
    ...
    public string ZZBT(string s) {
        if (s.Length == parser.nGet) {
            ...
            console.SetupBand(s); // MW0LGE force the band to be setup, TODO
    ...
    public string ZZCD(string s) {
        ...
        if (s != null && s != "") n = Convert.ToInt32(s);
        n = Math.Max(150, n);
        n = Math.Min(5000, n);
    ...
    public string ZZCU() {
        ...
        if (console.cpu_usage != null && console.cpu_init_complete) {
    ...
    public string ZZDD(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            console.TitleBarEncoderString
                = "Diversity Phase = " + phase + "degrees";
    ...
    public string ZZDM(string s) {
        ...
        if (s.Length == parser.nSet) {
            n = Convert.ToInt32(s);
            switch (n) {
                case 0: console.DisplayModeText = "Spectrum"; break;
                case 1: console.DisplayModeText = "Panadapter"; break;
                case 2: console.DisplayModeText = "Scope"; break;
                case 3: console.DisplayModeText = "Phase"; break;
                case 4: console.DisplayModeText = "Phase2"; break;
                case 5: console.DisplayModeText = "Waterfall"; break;
                case 6: console.DisplayModeText = "Histogram"; break;
                case 7: console.DisplayModeText = "Panafall"; break;
                case 8: console.DisplayModeText = "Panascope"; break;
                case 9: console.DisplayModeText = "Off"; break;
            }
    ...
    public string ZZEA(string s) {
        if (s.Length == parser.nSet) {
            ...
            int nb = console.EQForm
                         .NumBands; // Get the number of bands in the array
            int val; // Holds a temporary value
            string ans = nb.ToString().PadLeft(
                3, '0'); // The return string with the number of bands added

            for (int x = 0; x <= nb; x++) // Loop thru the array
            {
                if (eqarray[x] < 0) {
                    val = Math.Abs(eqarray[x]); // If the value is negative,
                                                // format the answer
    ...
    public string ZZFA(string s) {
        if (s.Length == parser.nSet) {
            ...
            if (!isMidi
                && console.CATChangesCenterFreq) // MW0LGE changed to take into
                                                 // consideration the flag
    ...
    public string ZZFM() {
        ...
        else if (radio == "ANAN100" || radio == "ANAN100B"
            || radio == "ANAN100D" || radio == "ANAN200D"
            || radio == "ANAN8000D")
            return "1";
        else
            return parser.Error1;
    ...
    public string ZZFV(string s) {
        if (s.Length == parser.nGet) {
            ...
            if (!sfxpattern.IsMatch(s)) return parser.Error1;
...
#region Extended CAT Methods ZZG - ZZM
    ...
    public string ZZGT(string s) {
        if (s.Length == parser.nSet) {
            if ((Convert.ToInt32(s) > (int)AGCMode.FIRST
                    && Convert.ToInt32(s) < (int)AGCMode.LAST))
    ...
    public string ZZIF(string s) {
        ...
        rtn += tx; // tx-rx status				 1 byte
        rtn += Mode2String(
            console.RX1DSPMode); // current mode				 2 bytes
        rtn += "0"; // dummy for FR/FT			 1 byte
        rtn += "0"; // dummy for scan status	 1 byte
        rtn += split; // VFO Split status			 1 byte
        rtn += "0000"; // dummy for the balance	 4 bytes
    ...
    public string ZZIU() {
        console.CATFilterShiftReset
            = 1; // Fixed XML entry 4/2/2007 to prevent return value.  BT
        return "";
    }
    ...
    public string ZZLB(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            if (n == 50)
                ...
                console.TitleBarEncoderString
                    = "RX1 Stereo Balance = left " + 2 * (50 - n) + "%";
            else
                console.TitleBarEncoderString
                    = "RX1 Stereo Balance = right " + 2 * (n - 50) + "%";
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    public string ZZLD(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            if (n == 50)
                ...
                console.TitleBarEncoderString
                    = "sub-RX Stereo Balance = left " + 2 * (50 - n) + "%";
            else
                console.TitleBarEncoderString
                    = "sub-RX Stereo Balance = right " + 2 * (n - 50) + "%";
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    public string ZZLF(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            if (n == 50)
                ...
                console.TitleBarEncoderString
                    = "RX2 Stereo Balance = left " + 2 * (50 - n) + "%";
            else
                console.TitleBarEncoderString
                    = "RX2 Stereo Balance = right " + 2 * (n - 50) + "%";
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    public string ZZME(string s) {
        if (s.Length == parser.nGet) {
            ...
            switch (s) {
                case "00": console.RX2DSPMode = DSPMode.LSB; break;
                case "01": console.RX2DSPMode = DSPMode.USB; break;
                case "02": console.RX2DSPMode = DSPMode.DSB; break;
                case "03": console.RX2DSPMode = DSPMode.CWL; break;
                case "04": console.RX2DSPMode = DSPMode.CWU; break;
                case "05": console.RX2DSPMode = DSPMode.FM; break;
                case "06": console.RX2DSPMode = DSPMode.AM; break;
                case "07": console.RX2DSPMode = DSPMode.DIGU; break;
                case "09": console.RX2DSPMode = DSPMode.DIGL; break;
                case "10": console.RX2DSPMode = DSPMode.SAM; break;
                case "11": console.RX2DSPMode = DSPMode.DRM; break;
            }
            return "";
        } else
            return parser.Error1;
    ...
    // cc are 15 pairs of digits 0-99 each making up an ASCII code -32 (so  'A'
    // is 33 for example)
        ...
        if (s.Length == parser.nSet) {
            ...
            for (cntr = 0; cntr < parser.nSet / 2; cntr++) {
                code = Convert.ToInt32(
                    s.Substring(2 * cntr, 2), 10); // get ascii code
                msg += Char.ConvertFromUtf32(code + 32);
            }
            console.TitleBarMultifunctionString
                = "   multifunction encoder = " + msg;
            return "";
        }
    ...
    public string ZZMG(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            // we have to remove the leading zero and replace it with the sign.
            return sign
                + (AddLeadingZeros(Math.Abs(console.CATMIC))).Substring(1);
        } else
            return parser.Error1;
    }
        ...
        try {
            ...
            for (n = 0; values[n] != DSPMode.LAST; n++) {
                if (values[n] != DSPMode.FIRST && values[n] != DSPMode.LAST) {
                    if ((int)values[n] <= 9)
                        thisMode
                            = values[n].ToString() + "0" + (int)values[n] + ":";
                    else
    ...
    public string ZZMT(string s) {
        ...
        if (s.Length == parser.nSet
            ...
            return TXMeter2String().PadLeft(2, '0'); // Added padleft 4/2/2007
                                                     // BT
        } else {
            return parser.Error1;
        }
    ...
    public string ZZMW(string s) {
        try {
            ...
            if (rec.Comments.StartsWith(s)) console.MemoryList.List.Remove(rec);
            return "";
        } catch {
            parser.Verbose_Error_Code = 4;
            return parser.Error1;
    ...
    public string ZZMY() {
        try {
            ...
            console.MemoryList.List.Add(
                new MemoryRecord("", console.VFOAFreq, "", console.RX1DSPMode,
                    true, console.TuneStepList[console.TuneStepIndex].Name,
                    console.CurrentFMTXMode, console.FMTXOffsetMHz,
                    console.radio.GetDSPTX(0).CTCSSFlag,
                    console.radio.GetDSPTX(0).CTCSSFreqHz, console.PWR,
                    (int)console.radio.GetDSPTX(0).TXFMDeviation,
                    console.VFOSplit, console.TXFreq, console.RX1Filter,
                    console.RX1FilterLow, console.RX1FilterHigh, newCh + ":",
                    console.radio.GetDSPRX(0, 0).RXAGCMode, console.RF,

                    DateTime.Now, false, 0, false, false, false,
                    0 // ke9ns add for freq scheduler
    ...
    public string ZZMZ(string s) {
        if (GetIndex(s) >= 0) {
            try {
                ...
                MemoryRecord newrec = new MemoryRecord { Group = oldrec.Group,
                    RXFreq = console.VFOAFreq, Name = oldrec.Name,
                    DSPMode = console.RX1DSPMode, Scan = oldrec.Scan,
                    ...
                    Split = console.VFOSplit, TXFreq = console.TXFreq,
...
#region Extended CAT Methods ZZN - ZZQ
    ...
    public string ZZPA(string s) {
        ...
        if (s.Length == parser.nSet) {
            if ((n > (int)PreampMode.FIRST && n < (int)PreampMode.LAST)) {
                switch (s) {
                    case "0": console.CATPreamp = PreampMode.HPSDR_OFF; break;
                    case "1": console.CATPreamp = PreampMode.HPSDR_ON; break;
                    case "2":
    ...
    public string ZZPY(string s) {
        ...
        if (s.Length == parser.nSet) {
            ...
            console.Zoom = level;
            console.TitleBarEncoderString
                = "Display zoom =" + Convert.ToInt32(level / 2.4) + "%";
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    public string ZZQM() { return StrVFOFreq("C"); }
...
#region Extended CAT Methods ZZR - ZZZ
    ...
    public string ZZRD(string s) {
        if (s.Length == parser.nSet) {
            ...
            == parser.nGet) // && console.RITOn)  //-W2PA Want to be able to
                            // change RIT value even if it's off
        {
            // switch(console.RX1DSPMode)
            //{
            //	case DSPMode.CWL:
            ...
            //             }
            console.RITValue
                -= 10; //-W2PA Changed to be same step in all modes.
            return "";
        } else
            return parser.Error1;
    }
        ...
        if (!console.MOX) {
            switch (s) {
                ...
                case "1":
                    ...
                    break;
                case "2": output = console.CATReadADC_L().PadLeft(20); break;
                case "3": output = console.CATReadADC_R().PadLeft(20); break;
            }
        } else {
            switch (s) {
                case "4": output = console.CATReadALC().PadLeft(20); break;
                case "5": output = console.CATReadFwdPwr().PadLeft(20); break;
                case "6": output = parser.Error1; break;
                case "7": output = console.CATReadRevPwr().PadLeft(20); break;
                case "8": output = console.CATReadSWR().PadLeft(20); break;
            }
        }
        return output;
    }
    ...
    public string ZZRX(string s) {
                                     ...
                                     // setting this prefix
        {
            ...
            att = Math.Max(0, att);
            att = Math.Min(31, att);
            console.SetupForm.HermesAttenuatorData
                = att; // Set the console control
        ...
        } else if (s.Length == parser.nGet) // if this is a read command
        {
            return AddLeadingZeros(
                console.SetupForm
                    .HermesAttenuatorData); // Get the console setting
        } else {
            return parser.Error1; // return a ?
        }
    }
            ...
            switch (console.CurrentHPSDRModel) {
                ...
                default:
                    if (s == "0") {
                        ...
                        num = num + console.RX2MeterCalOffset
    ...
    public string ZZTF(string s) {
        switch (console.RX1DSPMode) {
            case DSPMode.CWL:
            case DSPMode.CWU: return parser.Error1;
            default:
                if (s.Length == parser.nSet && (s == "0" || s == "1")) {
                    if (s == "1")
    ...
    public string ZZTV(string s) {
        if (ZZRS("") == "1" && (ZZSP("") == "1" || ZZMU("") == "1")) {
            if (s.Length == parser.nSet) {
                ...
                int f
                    = Convert.ToInt32(Math.Round(console.VFOASubFreq, 6) * 1e6);
                return AddLeadingZeros(f);
            } else
                return parser.Error1;
        } else {
    ...
    public string ZZUT(string s) //-W2PA  Toggle two tone test
    {
        if (s.Length == parser.nSet && (s == "0" || s == "1")) {
            switch (s) {
                case "0": console.CATTTTest = 0; break;
                case "1": console.CATTTTest = 1; break;
            }
    ...
    public string ZZVD(string s) {
        ...
        if (s.Length == parser.nSet) {
            n = Convert.ToInt32(s);
            switch (n) {
                case 0: console.VACSampleRate = "6000"; break;
                case 1: console.VACSampleRate = "8000"; break;
                case 2: console.VACSampleRate = "11025"; break;
                case 3: console.VACSampleRate = "12000"; break;
                case 4: console.VACSampleRate = "24000"; break;
                case 5: console.VACSampleRate = "22050"; break;
                case 6: console.VACSampleRate = "44100"; break;
                case 7: console.VACSampleRate = "48000"; break;
            }
            return "";
        } else if (s.Length == parser.nGet) {
            ...
            switch (rate) {
                case "6000": ans = "0"; break;
                case "8000": ans = "1"; break;
                case "11025": ans = "2"; break;
                case "12000": ans = "3"; break;
                case "24000": ans = "4"; break;
                case "22050": ans = "5"; break;
                case "44100": ans = "6"; break;
                case "48000": ans = "7"; break;
                case "96000": ans = "8"; break;
                case "192000": ans = "9"; break;
                default: ans = parser.Error1; break;
            }
            return ans;
        } else {
            return parser.Error1;
    ...
    public string ZZUX(string s) //-W2PA  Lock VFOA
    {
        if (s.Length == parser.nSet && (s == "0" || s == "1")) {
            ...
            if (console.VFOLock == CheckState.Checked
                || console.VFOALock == true || console.CATVFOLock)
                return "1";
            else
                return "0";
        } else {
    ...
    public string ZZUY(string s) //-W2PA  Lock VFOB
    {
        if (s.Length == parser.nSet && (s == "0" || s == "1")) {
            ...
            if (console.VFOBLock || console.VFOLock == CheckState.Indeterminate
                || console.CATVFOBLock)
                return "1";
            else
                return "0";
    ...
    public string ZZVU(string s) {
        ...
        if (s.Length == parser.nSet) {
            n = Convert.ToInt32(s);
            switch (n) {
                case 0: console.SetupForm.VAC2SampleRate = "6000"; break;
                case 1: console.SetupForm.VAC2SampleRate = "8000"; break;
                case 2: console.SetupForm.VAC2SampleRate = "11025"; break;
                case 3: console.SetupForm.VAC2SampleRate = "12000"; break;
                case 4: console.SetupForm.VAC2SampleRate = "24000"; break;
                case 5: console.SetupForm.VAC2SampleRate = "22050"; break;
                case 6: console.SetupForm.VAC2SampleRate = "44100"; break;
                case 7: console.SetupForm.VAC2SampleRate = "48000"; break;
                case 8: console.SetupForm.VAC2SampleRate = "96000"; break;
                case 9: console.SetupForm.VAC2SampleRate = "192000"; break;
            }
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    public string ZZVY(string s) {
        ...
        if (s.Length == parser.nSet) {
            n = Convert.ToInt32(s);
            switch (n) {
                case 0: console.SetupForm.VAC1BufferSize = "512"; break;
                case 1: console.SetupForm.VAC1BufferSize = "1024"; break;
                case 2: console.SetupForm.VAC1BufferSize = "2048"; break;
            }
            return "";
        } else if (s.Length == parser.nGet) {
            ...
            switch (rate) {
                case "512": ans = "0"; break;
                case "1024": ans = "1"; break;
                case "2048": ans = "2"; break;
                default: ans = parser.Error1; break;
            }
            return ans;
        } else {
    ...
    public string ZZVZ(string s) {
        ...
        if (s.Length == parser.nSet) {
            n = Convert.ToInt32(s);
            switch (n) {
                case 0: console.SetupForm.VAC2BufferSize = "512"; break;
                case 1: console.SetupForm.VAC2BufferSize = "1024"; break;
                case 2: console.SetupForm.VAC2BufferSize = "2048"; break;
            }
            return "";
        } else if (s.Length == parser.nGet) {
    ...
    // Sets or reads the F1500 FlexWire Out/F5K Mixer Line Out RCA Selected
    // Checkbox
    ...
    public string ZZXN(string s) {
        ...
        if (s.Length == parser.nGet) {
            ...
            n = m_agc + m_att;
            if (console.CATSquelch != 0) n += (1 << 6);
            if (console.CATNB1 != 0) n += (1 << 7);
            if (console.CATNB2 != 0) n += (1 << 8);
            if (console.CATNR != 0) n += (1 << 9);
            if (console.CATNR2 != 0) n += (1 << 10);
            if (console.CATSNB != 0) n += (1 << 11);
            if (console.CATANF != 0) n += (1 << 12);
            return AddLeadingZeros(n);
        } else {
            return parser.Error1;
        }
    ...
    public string ZZXO(string s) {
        ...
        if (s.Length == parser.nGet) {
            ...
            m_att = (m_att & 7) << 3; // 3 bits, moved left
            n = m_agc + m_att;
            if (console.CATSquelch2 == "1") n += (1 << 6);
            if (console.CATRX2NB1 != 0) n += (1 << 7);
            if (console.CATRX2NB2 != 0) n += (1 << 8);
            if (console.CATRX2NR != 0) n += (1 << 9);
            if (console.CATRX2NR2 != 0) n += (1 << 10);
            if (console.CATRX2SNB != 0) n += (1 << 11);
            if (console.CATRX2ANF != 0) n += (1 << 12);
            return AddLeadingZeros(n);
        } else {
            return parser.Error1;
        }
    ...
    public string ZZXV(string s) {
        int n = 0;
        if (s.Length == parser.nGet) {
            if (console.RITOn == true) n += 1;
            if (console.VFOLock == CheckState.Checked
                || console.VFOALock == true || console.CATVFOLock) // VFO A lock
                n += (1 << 1);
            if (console.VFOBLock || console.VFOLock == CheckState.Indeterminate
                || console.CATVFOBLock) // VFO B lock
                n += (1 << 2);
            if (console.VFOSplit == true) n += (1 << 3);
            if (console.CTuneDisplay == true) n += (1 << 4);
            if (console.CTuneRX2Display == true) n += (1 << 5);
            if ((console.CATPTT == true) || (console.MOX == true))
                n += (1 << 6);
            if (console.TUN == true) n += (1 << 7);
            if (console.XITOn == true) n += (1 << 8);
            if (console.VFOSync == true) n += (1 << 9);
            return AddLeadingZeros(n);
        } else {
            return parser.Error1;
        }
    ...
    public string ZZZB() {
        if (console.CATDisplayAvg == 1) console.CATZB = "1";
        return "";
    }
...
#region General Helpers
    ...
    private string AddLeadingZeros(int n) {
        ...
        while (num.Length < parser.nAns) num = num.Insert(0, "0");
...
#region Repeater Methods
    ...
    private string OffsetDirection2String() {
        ...
        switch (console.CurrentFMTXMode) {
            case FMTXMode.Simplex: rtn = "0"; break;
            case FMTXMode.High: rtn = "1"; break;
            case FMTXMode.Low: rtn = "2"; break;
            default: rtn = "0"; break;
        }
        return rtn;
    }
    ...
    private void String2OffsetDirection(string s) {
        switch (s) {
            case "0": console.CurrentFMTXMode = FMTXMode.Simplex; break;
            case "1": console.CurrentFMTXMode = FMTXMode.High; break;
            case "2": console.CurrentFMTXMode = FMTXMode.Low; break;
            default: console.CurrentFMTXMode = FMTXMode.Simplex; break;
        }
    }
    ...
    private double String2CTCSSFreq(string s) {
        double freq = 00.0;
        switch (s) {
            case "01": freq = 67.0; break;
            case "02": freq = 69.3; break;
            case "03": freq = 71.9; break;
            case "04": freq = 74.4; break;
            case "05": freq = 77.0; break;
            case "06": freq = 79.7; break;
            case "07": freq = 82.5; break;
            case "08": freq = 85.4; break;
            case "09": freq = 88.5; break;
            case "10": freq = 91.5; break;
            case "11": freq = 94.8; break;
            case "12": freq = 97.4; break;
            case "13": freq = 100.0; break;
            case "14": freq = 103.5; break;
            case "15": freq = 107.2; break;
            case "16": freq = 110.9; break;
            case "17": freq = 114.8; break;
            case "18": freq = 118.8; break;
            case "19": freq = 123.0; break;
            case "20": freq = 127.3; break;
            case "21": freq = 131.8; break;
            case "22": freq = 136.5; break;
            case "23": freq = 141.3; break;
            case "24": freq = 146.2; break;
            case "25": freq = 151.4; break;
            case "26": freq = 156.7; break;
            case "27": freq = 159.8; break;
            case "28": freq = 162.2; break;
            case "29": freq = 165.5; break;
            case "30": freq = 167.9; break;
            case "31": freq = 171.3; break;
            case "32": freq = 173.8; break;
            case "33": freq = 177.3; break;
            case "34": freq = 179.9; break;
            case "35": freq = 183.5; break;
            case "36": freq = 186.2; break;
            case "37": freq = 189.9; break;
            case "38": freq = 192.8; break;
            case "39": freq = 199.5; break;
            case "40": freq = 203.5; break;
            case "41": freq = 206.5; break;
            case "42": freq = 210.7; break;
            case "43": freq = 218.1; break;
            case "44": freq = 225.7; break;
            case "45": freq = 229.1; break;
            case "46": freq = 233.6; break;
            case "47": freq = 241.8; break;
            case "48": freq = 250.3; break;
            case "49": freq = 254.1; break;
            default: freq = 67.0; break;
        }
        return freq;
    }
    ...
    private string CTCSSFreq2String(int freq) {
        string ans = "";
        switch (freq) {
            case 670: ans = "01"; break;
            case 693: ans = "02"; break;
            case 719: ans = "03"; break;
            case 744: ans = "04"; break;
            case 770: ans = "05"; break;
            case 797: ans = "06"; break;
            case 825: ans = "07"; break;
            case 854: ans = "08"; break;
            case 885: ans = "09"; break;
            case 915: ans = "10"; break;
            case 948: ans = "11"; break;
            case 974: ans = "12"; break;
            case 1000: ans = "13"; break;
            case 1035: ans = "14"; break;
            case 1072: ans = "15"; break;
            case 1109: ans = "16"; break;
            case 1148: ans = "17"; break;
            case 1188: ans = "18"; break;
            case 1230: ans = "19"; break;
            case 1273: ans = "20"; break;
            case 1318: ans = "21"; break;
            case 1365: ans = "22"; break;
            case 1413: ans = "23"; break;
            case 1462: ans = "24"; break;
            case 1514: ans = "25"; break;
            case 1567: ans = "26"; break;
            case 1598: ans = "27"; break;
            case 1622: ans = "28"; break;
            case 1655: ans = "29"; break;
            case 1679: ans = "30"; break;
            case 1713: ans = "31"; break;
            case 1738: ans = "32"; break;
            case 1773: ans = "33"; break;
            case 1799: ans = "34"; break;
            case 1835: ans = "35"; break;
            case 1862: ans = "36"; break;
            case 1899: ans = "37"; break;
            case 1928: ans = "38"; break;
            case 1995: ans = "39"; break;
            case 2035: ans = "40"; break;
            case 2065: ans = "41"; break;
            case 2107: ans = "42"; break;
            case 2181: ans = "43"; break;
            case 2257: ans = "44"; break;
            case 2291: ans = "45"; break;
            case 2336: ans = "46"; break;
            case 2418: ans = "47"; break;
            case 2503: ans = "48"; break;
            case 2541: ans = "49"; break;
            default: ans = "01"; break;
        }
        return ans;
    }
    ...
    private SortableBindingList<MemoryRecord> GetMemoryList() {
        try {
            ...
            SortableBindingList<MemoryRecord> list
    ...
    private int GetIndex(string channel) {
        ...
        try {
            ...
            int n = 0;
            for (n = 0; n < list.Count; n++) {
                if (list[n].Comments.Substring(0, 4) == channel + ":") ndx = n;
            }
        } catch {
        }
        return ndx;
    ...
    private int GetNextChannelNumber() {
        ...
        for (n = 0; n < list.Count; n++) {
            if (list[n].Comments.Contains(":")) {
                int thisN = int.Parse(list[n].Comments.Substring(
                    0, list[n].Comments.IndexOf(":")));
                if (thisN > last) last = thisN;
            }
        }
        return last + 1;
    }
...
#region Filter Methods
    ...
    public string Filter2String(Filter f) {
        ...
        int retval = 0;
        switch (fw) {
            case "F6000": strfilt = "6000"; break;
            case "F4000": strfilt = "4000"; break;
            case "F2600": strfilt = "2600"; break;
            case "F2100": strfilt = "2100"; break;
            case "F1000": strfilt = "1000"; break;
            case "F500": strfilt = "0500"; break;
            case "F250": strfilt = "0250"; break;
            case "F100": strfilt = "0100"; break;
            case "F50": strfilt = "0050"; break;
            case "F25": strfilt = "0025"; break;
            case "VAR1":
    ...
    public Filter String2Filter(string f) {
        ...
        switch (f) {
            case "6000": filter = Filter.F1; break;
            case "4000": filter = Filter.F2; break;
            case "2600": filter = Filter.F3; break;
            case "2100": filter = Filter.F4; break;
            case "1000": filter = Filter.F5; break;
            case "0500": filter = Filter.F6; break;
            case "0250": filter = Filter.F7; break;
            case "0100": filter = Filter.F8; break;
            case "0050": filter = Filter.F9; break;
            case "0025": filter = Filter.F10; break;
            case "VAR1": filter = Filter.VAR1; break;
            case "VAR2": filter = Filter.VAR2; break;
        }
        return filter;
    }
    ...
    private void SetFilterCenterAndWidth(int center, int width) {
        ...
        // new_lo and new_hi calculated assuming a USB mode .. do the right
        // thing for lsb and other modes fixme -- needs more thinking
        switch (console.RX1DSPMode) {
            case DSPMode.LSB:
                int scratch = new_hi;
                new_hi = -new_lo;
            ...
            case DSPMode.AM:
            case DSPMode.SAM: new_lo = -new_hi; break;
        }
    ...
    private int Code2Frequency(string c, string n) {
        ...
        switch (console.RX1DSPMode) {
            ...
            case DSPMode.DRM:
            case DSPMode.FM:
            case DSPMode.SAM: mode = "DSB"; break;
        }
        ...
        switch (fgroup) {
            case 1: // SL SSB
                switch (c) {
                    case "00": freq = 0; break;
                    case "01": freq = 50; break;
                    case "02": freq = 100; break;
                    case "03": freq = 200; break;
                    case "04": freq = 300; break;
                    case "05": freq = 400; break;
                    case "06": freq = 500; break;
                    case "07": freq = 600; break;
                    case "08": freq = 700; break;
                    case "09": freq = 800; break;
                    case "10": freq = 900; break;
                    case "11": freq = 1000; break;
                }
                break;
            case 2: // SH SSB
                switch (c) {
                    case "00": freq = 1400; break;
                    case "01": freq = 1600; break;
                    case "02": freq = 1800; break;
                    case "03": freq = 2000; break;
                    case "04": freq = 2200; break;
                    case "05": freq = 2400; break;
                    case "06": freq = 2600; break;
                    case "07": freq = 2800; break;
                    case "08": freq = 3000; break;
                    case "09": freq = 3400; break;
                    case "10": freq = 4000; break;
                    case "11": freq = 5000; break;
                }
                break;
            case 3: // SL DSB
                switch (c) {
                    case "00": freq = 0; break;
                    case "01": freq = 100; break;
                    case "02": freq = 200; break;
                    case "03": freq = 500; break;
                }
                break;
            case 4: // SH DSB
                switch (c) {
                    case "00": freq = 2500; break;
                    case "01": freq = 3000; break;
                    case "02": freq = 4000; break;
                    case "03": freq = 5000; break;
                }
                break;
        }
        return freq;
    ...
    private void SetFilter(string c, string n) {
        ...
        switch (console.RX1DSPMode) {
            ...
            case DSPMode.CWU:
                ...
                if (n == "SH")
                    console.RX1FilterHigh
                        = freq; // split the bandwidth at the center frequency
                else
            ...
            case DSPMode.CWL:
                if (n == "SH") {
                    freq = Code2Frequency(c,
                        "SH"); // get the upper limit from the lower value set
                    console.RX1FilterLow
                        = -freq; // since we need the more positive value
                } // closest to the center freq in lsb modes
                else {
                    freq = Code2Frequency(c,
                        "SL"); // do the reverse here, the less positive value
                    console.RX1FilterHigh
                        = -freq; // is away from the center freq
                }
                break;
            case DSPMode.AM:
            case DSPMode.DRM:
            ...
            case DSPMode.SAM:
                if (n == "SH") {
                    ...
                    // reset the frequency to the nominal value (in case it's
                    // been changed)
                    ...
                    console.RX1FilterHigh = freq / 2;
                    console.RX1FilterLow = -freq / 2;
                    // subtract the SL value from the lower half of the
                    // bandwidth
...
#region Mode Methods
    ...
    public void String2Mode(string pIndex) {
        ...
        switch (s) {
            case "00": console.RX1DSPMode = DSPMode.LSB; break;
            case "01": console.RX1DSPMode = DSPMode.USB; break;
            case "02": console.RX1DSPMode = DSPMode.DSB; break;
            case "03": console.RX1DSPMode = DSPMode.CWL; break;
            case "04": console.RX1DSPMode = DSPMode.CWU; break;
            case "05": console.RX1DSPMode = DSPMode.FM; break;
            case "06": console.RX1DSPMode = DSPMode.AM; break;
            case "07": console.RX1DSPMode = DSPMode.DIGU; break;
            case "08": console.RX1DSPMode = DSPMode.SPEC; break;
            case "09": console.RX1DSPMode = DSPMode.DIGL; break;
            case "10": console.RX1DSPMode = DSPMode.SAM; break;
            case "11": console.RX1DSPMode = DSPMode.DRM; break;
        }
    }
    ...
    public string Mode2String(DSPMode pMode) {
        ...
        switch (s) {
            case DSPMode.LSB: retval = "00"; break;
            case DSPMode.USB: retval = "01"; break;
            case DSPMode.DSB: retval = "02"; break;
            case DSPMode.CWL: retval = "03"; break;
            case DSPMode.CWU: retval = "04"; break;
            case DSPMode.FM: retval = "05"; break;
            case DSPMode.AM: retval = "06"; break;
            case DSPMode.DIGU: retval = "07"; break;
            case DSPMode.SPEC: retval = "08"; break;
            case DSPMode.DIGL: retval = "09"; break;
            case DSPMode.SAM: retval = "10"; break;
            case DSPMode.DRM: retval = "11"; break;
            default: retval = parser.Error1; break;
        }
    ...
    public void KString2Mode(string pIndex) {
        ...
        switch (s) {
            ...
            case "2":
                ...
                break;
            case "3": console.RX1DSPMode = DSPMode.CWU; break;
            case "4": console.RX1DSPMode = DSPMode.FM; break;
            case "5": console.RX1DSPMode = DSPMode.AM; break;
            case "6": console.RX1DSPMode = DSPMode.DIGL; break;
            case "7": console.RX1DSPMode = DSPMode.CWL; break;
            case "9": console.RX1DSPMode = DSPMode.DIGU; break;
            default: console.RX1DSPMode = DSPMode.USB; break;
        }
    }
    ...
    public string Mode2KString(DSPMode pMode) {
        ...
        switch (s) {
            case DSPMode.LSB: retval = "1"; break;
            case DSPMode.USB: retval = "2"; break;
            case DSPMode.CWU: retval = "3"; break;
            case DSPMode.FM: retval = "4"; break;
            case DSPMode.AM:
                //				case DSPMode.SAM:		//possible fix for SAM
                //problem
                retval = "5";
                break;
            case DSPMode.DIGL:
                if (console.SetupForm.DigUIsUSB)
                    ...
                    retval = "6";
                break;
            case DSPMode.CWL: retval = "7"; break;
            case DSPMode.DIGU:
                if (console.SetupForm.DigUIsUSB)
                    retval = "2";
                else
...
#region Band Methods
    ...
    private string Band2String(Band pBand) {
        ...
        switch (band) {
            case Band.GEN: retval = "888"; break;
            case Band.B160M: retval = "160"; break;
            case Band.B60M: retval = "060"; break;
            case Band.B80M: retval = "080"; break;
            case Band.B40M: retval = "040"; break;
            case Band.B30M: retval = "030"; break;
            case Band.B20M: retval = "020"; break;
            case Band.B17M: retval = "017"; break;
            case Band.B15M: retval = "015"; break;
            case Band.B12M: retval = "012"; break;
            case Band.B10M: retval = "010"; break;
            case Band.B6M: retval = "006"; break;
            case Band.B2M: retval = "002"; break;
            case Band.WWV: retval = "999"; break;
            case Band.VHF0: retval = "V00"; break;
            case Band.VHF1: retval = "V01"; break;
            case Band.VHF2: retval = "V02"; break;
            case Band.VHF3: retval = "V03"; break;
            case Band.VHF4: retval = "V04"; break;
            case Band.VHF5: retval = "V05"; break;
            case Band.VHF6: retval = "V06"; break;
            case Band.VHF7: retval = "V07"; break;
            case Band.VHF8: retval = "V08"; break;
            case Band.VHF9: retval = "V09"; break;
            case Band.VHF10: retval = "V10"; break;
            case Band.VHF11: retval = "V11"; break;
            case Band.VHF12: retval = "V12"; break;
            case Band.VHF13: retval = "V13"; break;
            default: retval = "888"; break;
        }
        return retval;
    }
    ...
    private Band String2Band(string pBand) {
        ...
        switch (band) {
            case "888": retval = Band.GEN; break;
            case "160": retval = Band.B160M; break;
            case "060": retval = Band.B60M; break;
            case "080": retval = Band.B80M; break;
            case "040": retval = Band.B40M; break;
            case "030": retval = Band.B30M; break;
            case "020": retval = Band.B20M; break;
            case "017": retval = Band.B17M; break;
            case "015": retval = Band.B15M; break;
            case "012": retval = Band.B12M; break;
            case "010": retval = Band.B10M; break;
            case "006": retval = Band.B6M; break;
            case "002": retval = Band.B2M; break;
            case "999": retval = Band.WWV; break;
            case "V00": retval = Band.VHF0; break;
            case "V01": retval = Band.VHF1; break;
            case "V02": retval = Band.VHF2; break;
            case "V03": retval = Band.VHF3; break;
            case "V04": retval = Band.VHF4; break;
            case "V05": retval = Band.VHF5; break;
            case "V06": retval = Band.VHF6; break;
            case "V07": retval = Band.VHF7; break;
            case "V08": retval = Band.VHF8; break;
            case "V09": retval = Band.VHF9; break;
            case "V10": retval = Band.VHF10; break;
            case "V11": retval = Band.VHF11; break;
            case "V12": retval = Band.VHF12; break;
            case "V13": retval = Band.VHF13; break;
            default: retval = Band.GEN; break;
        }
        return retval;
    }
...
#region Step Methods
    ...
    private double Step2Freq(int step) {
        double freq = 0.0;
        switch (step) {
            case 0: freq = 0.000001; break;
            case 1: freq = 0.000010; break;
            case 2: freq = 0.000025; break;
            case 3: freq = 0.000050; break;
            case 4: freq = 0.000100; break;
            case 5: freq = 0.000250; break;
            case 6: freq = 0.000500; break;
            case 7: freq = 0.001000; break;
            case 8: freq = 0.005000; break;
            case 9: freq = 0.009000; break;
            case 10: freq = 0.010000; break;
            case 11: freq = 0.100000; break;
            case 12: freq = 0.250000; break;
            case 13: freq = 0.500000; break;
            case 14: freq = 1.000000; break;
            case 15: freq = 10.000000; break;
        }
        return freq;
    }

    private string Step2String(int pSize) {
        // Modified 2/25/07 to accomodate changes to console where odd step
        // sizes added.  BT
        string stepval = "";
        int step = pSize;
        switch (step) {
...
#region Rig ID Methods
    ...
    private string CAT2RigType() { return ""; }

    private string RigType2CAT() { return ""; }
...
#region DSP Filter Size Methods
    ...
    private string Width2Index(int txt) {
        string ans = "";
        switch (txt) {
            case 256: ans = "0"; break;
            case 512: ans = "1"; break;
            case 1024: ans = "2"; break;
            case 2048: ans = "3"; break;
            case 4096: ans = "4"; break;
            case 8192: ans = "5"; break;
            case 16384: ans = "6"; break;
            default: ans = "0"; break;
        }
        return ans;
    }
    ...
    private int Index2Width(string ndx) {
        int ans;
        switch (ndx) {
            case "0": ans = 256; break;
            case "1": ans = 512; break;
            case "2": ans = 1024; break;
            case "3": ans = 2048; break;
            case "4": ans = 4096; break;
            case "5": ans = 8192; break;
            case "6": ans = 16384; break;
            default: ans = 256; break;
        }
        return ans;
    }
