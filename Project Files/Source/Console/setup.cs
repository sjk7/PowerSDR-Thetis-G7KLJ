...
#region Constructor and Destructor
    ...
    public Setup(Console c) {
        Splash.SetStatus("Initialising setup ...");
        InitializeComponent();
        console = c;

        Splash.SetStatus("Initialising setup, step 2");
        ...
        // MW0LGE node: this will allways cause the change event to fire, as the
        ...
        Splash.SetStatus("Setting up keymap ...");
        ...
        Splash.SetStatus("Initialising audio tab ...");
        InitAudioTab();

        Splash.SetStatus("Setting setup defaults ...");
        ...
        Splash.SetStatus("Refreshing skins list...");
        RefreshSkinList(); // moved down the initialisation order, so we at
                           // least know if we are gdi or dx, only build the
                           // list

        // G7KLJ: I do not know if it's important to load this here, so I left
        // it. But i call it again later so the TX profile does not overwrite
        // the VAC settings
        Splash.SetStatus("Getting options ...");
        getOptions2();

        Splash.SetStatus("Selecting skin ...");
        selectSkin();

        Splash.SetStatus("Applying settings ...");
        if (comboDSPPhoneRXBuf.SelectedIndex < 0
            || comboDSPPhoneRXBuf.SelectedIndex
                >= comboDSPPhoneRXBuf.Items.Count)
            comboDSPPhoneRXBuf.SelectedIndex
                = comboDSPPhoneRXBuf.Items.Count - 1;
        if (comboDSPPhoneTXBuf.SelectedIndex < 0
            || comboDSPPhoneTXBuf.SelectedIndex
                >= comboDSPPhoneTXBuf.Items.Count)
            comboDSPPhoneTXBuf.SelectedIndex
                = comboDSPPhoneTXBuf.Items.Count - 1;
        if (comboDSPCWRXBuf.SelectedIndex < 0
            || comboDSPCWRXBuf.SelectedIndex >= comboDSPCWRXBuf.Items.Count)
            comboDSPCWRXBuf.SelectedIndex = comboDSPCWRXBuf.Items.Count - 1;
        if (comboDSPDigRXBuf.SelectedIndex < 0
            || comboDSPDigRXBuf.SelectedIndex >= comboDSPDigRXBuf.Items.Count)
            comboDSPDigRXBuf.SelectedIndex = comboDSPDigRXBuf.Items.Count - 1;
        if (comboDSPDigTXBuf.SelectedIndex < 0
            || comboDSPDigTXBuf.SelectedIndex >= comboDSPDigTXBuf.Items.Count)
        ...
        Splash.SetStatus("Force All Events ...");
        ForceAllEvents();
...
#region Init Routines
    ...
    private void InitGeneralTab() {
        ...
        chkWheelTunesOutsideSpectral.Checked
            = console.WheelTunesOutsideSpectral;
        chkAlsoUseSpecificMouseWheel.Checked
            = console.AlsoUseSpecificMouseWheel;
    ...
    private void InitDisplayTab() {
        ...
        udDisplayWaterfallLowLevel.Value
            = (decimal)Display.WaterfallLowThreshold;
        udDisplayWaterfallHighLevel.Value
            = (decimal)Display.WaterfallHighThreshold;
        udRX2DisplayWaterfallLowLevel.Value
            = (decimal)Display.RX2WaterfallLowThreshold;
        udRX2DisplayWaterfallHighLevel.Value
            = (decimal)Display.RX2WaterfallHighThreshold;
        ...
        // MW0LGE in the case where we don't have a setting in the db, this
        // function (initdisplaytab) is called, use console instead
        SetMultiMeterMode(console.MMMeasureMode);

        comboDisplayThreadPriority.SelectedIndex
            = (int)console.DisplayThreadPriority; // MW0LGE
    }
    ...
    public void SetMultiMeterMode(MultiMeterMeasureMode mode) {
        switch (mode) {
            case MultiMeterMeasureMode.SMeter:
                ...
                break;
            case MultiMeterMeasureMode.DBM: radDBM.Checked = true; break;
            case MultiMeterMeasureMode.UV: radUV.Checked = true; break;
        }
    }

    private void InitDSPTab() { udDSPCWPitch.Value = console.CWPitch; }
    ...
    private void InitAppearanceTab() {
        ...
        chkDisablePicDisplayBackgroundImage.Checked
            = console.DisableBackgroundImage; // MW0LGE
        chkShowRXFilterOnWaterfall.Checked
            = Display.ShowRXFilterOnWaterfall; // MW0LGE
        chkShowTXFilterOnWaterfall.Checked
            = Display.ShowTXFilterOnWaterfall; // MW0LGE
        chkShowRXZeroLineOnWaterfall.Checked
            = Display.ShowRXZeroLineOnWaterfall; // MW0LGE
        chkShowTXZeroLineOnWaterfall.Checked
            = Display.ShowTXZeroLineOnWaterfall; // MW0LGE
        chkShowTXFilterOnRXWaterfall.Checked
            = Display.ShowTXFilterOnRXWaterfall; // MW0LGE
    ...
    private void RefreshSkinList() {
        ...
        if (!Directory.Exists(path)) {
            MessageBox.Show(
                "The console presentation files (skins) were not found.\n"
                    + "Appearance will suffer until this is rectified.\n",
                "Skins files not found", MessageBoxButtons.OK,
    ...
    private void controlList2(Control c, ref Dictionary<string, Control> a) {
        if (c.Controls.Count > 0) {
            foreach (Control c2 in c.Controls) controlList2(c2, ref a);
        }
    ...
    private void ControlList(Control c, ref ArrayList a) {
        if (c.Controls.Count > 0) {
            foreach (Control c2 in c.Controls) ControlList(c2, ref a);
        }
    ...
    public void SaveOptions() {
        ...
        foreach (Control c in temp) {
            ...
            if (c.GetType() == typeof(CheckBoxTS)) {
                ...
                String entry
                    = c.Name + "/" + ((CheckBoxTS)c).Checked.ToString();
                a.Add(entry);
                ...
                a.Add(c.Name + "/" + clr.R + "." + clr.G + "." + clr.B + "."
                    + clr.A);
            }
#if (DEBUG)
        ...
        // add this manually because soundfile string not stored in any control
        // in setup
    ...
    public void getOptions2() {
        // MW0LGE moved to dictionary, as control names are unique, and does
        // away with old loop through every control to find the one we want
        ...
        if (a.Count
            < controls.Count) // some control values are not in the database
        ...
        // MW0LGE We have overwritten controls with data that might not match
        // the current profile load in current profile [so this code was at the
        // end of the function --G7KLJ]

        // G7KLJ VAC is saved as part of the TX profile, and this is confusing
        // to me because, as a SWL I kept wondering why VAC state was not saved
        // when I pressed Apply, because of course I care less about TX if I am
        // just a listener! So I load the profile FIRST

        // If the user subsequently loads another TX profile, then the profile
        // values will prevail.
        ...
        foreach (string s in a) {
            ...
            if (controls.ContainsKey(name)) {
                ...
                if (cc.GetType()
                    == typeof(CheckBoxTS)) // the control is a CheckBoxTS
                {
                    ...
                    == typeof(ComboBoxTS)) // the control is a ComboBoxTS
                {
                    ...
                    if (c.Items.Count > 0
                        && c.Items[0].GetType() == typeof(string)) {
                        c.Text = val;
                    } else {
                                                ...
                                                // NumericUpDownTS
                {
                    ...
                    == typeof(RadioButtonTS)) // the control is a RadioButtonTS
                {
                    ...
                    == typeof(TextBoxTS)) // the control is a TextBox
                {
                    ...
                    == typeof(TrackBarTS)) // the control is a TrackBar (slider)
                {
    ...
    private string KeyToString(Keys k) {
        if (!k.ToString().StartsWith("Oem")) return k.ToString();

        string s = "";
        switch (k) {
            case Keys.OemOpenBrackets: s = "["; break;
            case Keys.OemCloseBrackets: s = "]"; break;
            case Keys.OemQuestion: s = "/"; break;
            case Keys.OemPeriod: s = "."; break;
            case Keys.OemPipe: s = (k & Keys.Shift) == 0 ? "\\" : "|"; break;
        }
        return s;
    }
            ...
            if (c.GetType() == typeof(GroupBoxTS)) {
                foreach (Control c2 in c.Controls) {
                    ...
                    foreach (Keys k in KeyList) {
                        combo.Items.Add(k.ToString().StartsWith("Oem")
                                ? KeyToString(k)
                                : k.ToString());
                    }
                }
                ...
                combo.Items.Clear();
                foreach (Keys k in KeyList) combo.Items.Add(k.ToString());
            }
        }
    }
    ...
    private bool CheckTXProfileChanged() {
        ...
        for (int i = 1; i < 11; i++) {
            if (eq[i] != (int)rows[0]["TXEQ" + i]) return true;
        }
    ...
    public void SaveTXProfileData() {
        ...
        int[] eq = console.EQForm.TXEQ;
        dr["TXEQPreamp"] = eq[0];
        for (int i = 1; i < 11; i++) dr["TXEQ" + i.ToString()] = eq[i];
        for (int i = 11; i < 21; i++)
    ...
    public void UpdateWaterfallBandInfo() {
        if (!initializing) {
            switch (console.RX1Band) {
                case Band.B160M:
                    txtWaterFallBandLevel.Text = "160 meters";
                    break;
                ...
                case Band.BLMF:
                    txtWaterFallBandLevel.Text = "LW/MW bands";
                    break;
                case Band.B120M:
                    txtWaterFallBandLevel.Text = "120 meters";
                    break;
            ...
            switch (console.RX2Band) {
                case Band.B160M:
                    txtRX2WaterFallBandLevel.Text = "160 meters";
                    break;
                case Band.B80M:
                    txtRX2WaterFallBandLevel.Text = "80 meters";
                    break;
                case Band.B60M:
                    txtRX2WaterFallBandLevel.Text = "60 meters";
                    break;
                case Band.B40M:
                    txtRX2WaterFallBandLevel.Text = "40 meters";
                    break;
                case Band.B30M:
                    txtRX2WaterFallBandLevel.Text = "30 meters";
                    break;
                case Band.B20M:
                    txtRX2WaterFallBandLevel.Text = "20 meters";
                    break;
                case Band.B17M:
                    txtRX2WaterFallBandLevel.Text = "17 meters";
                    break;
                case Band.B15M:
                    txtRX2WaterFallBandLevel.Text = "15 meters";
                    break;
                case Band.B12M:
                    txtRX2WaterFallBandLevel.Text = "12 meters";
                    break;
                case Band.B10M:
                    txtRX2WaterFallBandLevel.Text = "10 meters";
                    break;
                case Band.B6M:
                    txtRX2WaterFallBandLevel.Text = "6 meters";
                    break;
    ...
    public void UpdateDisplayGridBandInfo() {
        if (!initializing) {
            switch (console.RX1Band) {
                case Band.B160M:
                    txtDisplayGridBandLevel.Text = "160 meters";
                    break;
                case Band.B80M:
                    txtDisplayGridBandLevel.Text = "80 meters";
                    break;
                case Band.B60M:
                    txtDisplayGridBandLevel.Text = "60 meters";
                    break;
                case Band.B40M:
                    txtDisplayGridBandLevel.Text = "40 meters";
                    break;
                case Band.B30M:
                    txtDisplayGridBandLevel.Text = "30 meters";
                    break;
                case Band.B20M:
                    txtDisplayGridBandLevel.Text = "20 meters";
                    break;
                case Band.B17M:
                    txtDisplayGridBandLevel.Text = "17 meters";
                    break;
                case Band.B15M:
                    txtDisplayGridBandLevel.Text = "15 meters";
                    break;
                case Band.B12M:
                    txtDisplayGridBandLevel.Text = "12 meters";
                    break;
                case Band.B10M:
                    txtDisplayGridBandLevel.Text = "10 meters";
                    break;
                ...
                case Band.BLMF:
                    txtDisplayGridBandLevel.Text = "LW/MW bands";
                    break;
                case Band.B120M:
                    txtDisplayGridBandLevel.Text = "120 meters";
                    break;
                case Band.B90M:
                    txtDisplayGridBandLevel.Text = "90 meters";
                    break;
                case Band.B61M:
                    txtDisplayGridBandLevel.Text = "60 meters";
                    break;
                case Band.B49M:
                    txtDisplayGridBandLevel.Text = "49 meters";
                    break;
                case Band.B41M:
                    txtDisplayGridBandLevel.Text = "41 meters";
                    break;
                case Band.B31M:
                    txtDisplayGridBandLevel.Text = "31 meters";
                    break;
                case Band.B25M:
                    txtDisplayGridBandLevel.Text = "25 meters";
                    break;
                case Band.B22M:
                    txtDisplayGridBandLevel.Text = "22 meters";
                    break;
                case Band.B19M:
                    txtDisplayGridBandLevel.Text = "19 meters";
                    break;
                case Band.B16M:
                    txtDisplayGridBandLevel.Text = "16 meters";
                    break;
                case Band.B14M:
                    txtDisplayGridBandLevel.Text = "14 meters";
                    break;
                case Band.B13M:
                    txtDisplayGridBandLevel.Text = "13 meters";
                    break;
                case Band.B11M:
                    txtDisplayGridBandLevel.Text = "11 meters";
                    break;
                // - end lge
            ...
            switch (console.RX2Band) {
                case Band.B160M:
                    txtRX2DisplayGridBandLevel.Text = "160 meters";
                    break;
                case Band.B80M:
                    txtRX2DisplayGridBandLevel.Text = "80 meters";
                    break;
                case Band.B60M:
                    txtRX2DisplayGridBandLevel.Text = "60 meters";
                    break;
                case Band.B40M:
                    txtRX2DisplayGridBandLevel.Text = "40 meters";
                    break;
                case Band.B30M:
                    txtRX2DisplayGridBandLevel.Text = "30 meters";
                    break;
                case Band.B20M:
                    txtRX2DisplayGridBandLevel.Text = "20 meters";
                    break;
                case Band.B17M:
                    txtRX2DisplayGridBandLevel.Text = "17 meters";
                    break;
                case Band.B15M:
                    txtRX2DisplayGridBandLevel.Text = "15 meters";
                    break;
                case Band.B12M:
                    txtRX2DisplayGridBandLevel.Text = "12 meters";
                    break;
                case Band.B10M:
                    txtRX2DisplayGridBandLevel.Text = "10 meters";
                    break;
                case Band.B6M:
                    txtRX2DisplayGridBandLevel.Text = "6 meters";
                    break;
                case Band.WWV: txtRX2DisplayGridBandLevel.Text = "WWV"; break;
                case Band.GEN:
                    txtRX2DisplayGridBandLevel.Text = "General";
                    break;
...
#region Properties
    ...
    public string FocusMasterTitle {
        set { txtFocusMasterWinTitle.Text = value; }
    }
    ...
    public bool EnableRX2APFControl {
        set { chkDSPRX2APFEnable.Enabled = value; }
    }

    public float RX6mGainOffset {
        get {
            if (ud6mLNAGainOffset != null)
                return (float)ud6mLNAGainOffset.Value;
            else
                return -1;
        }
        set {
            if (ud6mLNAGainOffset != null)
                ud6mLNAGainOffset.Value = (decimal)value;
        }
    }
    ...
    public bool AlexTRRelay {
        get {
            if (chkHFTRRelay != null)
                return chkHFTRRelay.Checked;
            else
                return false;
        }
        set {
            if (chkHFTRRelay != null) chkHFTRRelay.Checked = value;
        }
    }

    public bool HermesEnableAttenuator {
        get {
            if (chkHermesStepAttenuator != null)
                return chkHermesStepAttenuator.Checked;
            else
                return false;
        }
        set {
            if (chkHermesStepAttenuator != null)
                chkHermesStepAttenuator.Checked = value;
        }
    }
    ...
    public int HermesAttenuatorData {
        get {
            if (udHermesStepAttenuatorData != null)
                return (int)udHermesStepAttenuatorData.Value;
            else
                return -1;
        }
        set {
    ...
    public int VACDriver {
        get { return comboAudioDriver2.SelectedIndex; }
        set {
    ...
    public int VAC2Driver {
        get { return comboAudioDriver3.SelectedIndex; }
        set {
    ...
    public int VACInputCable {
        get { return comboAudioInput2.SelectedIndex; }
        set {
    ...
    public int VAC2InputCable {
        get { return comboAudioInput3.SelectedIndex; }
        set {
    ...
    public int VACOutputCable {
        get { return comboAudioOutput2.SelectedIndex; }
        set {
    ...
    public int VAC2OutputCable {
        get { return comboAudioOutput3.SelectedIndex; }
        set {
    ...
    public bool CATEnabled {
        get {
            if (chkCATEnable != null)
                return chkCATEnable.Checked;
            else
                return false;
        }
        set {
    ...
    public bool CAT2Enabled {
        get {
            if (chkCAT2Enable != null)
                return chkCAT2Enable.Checked;
            else
                return false;
        }
        set {
    ...
    public bool CAT3Enabled {
        get {
            if (chkCAT3Enable != null)
                return chkCAT3Enable.Checked;
            else
                return false;
        }
        set {
    ...
    public bool CAT4Enabled {
        get {
            if (chkCAT4Enable != null)
                return chkCAT4Enable.Checked;
            else
                return false;
        }
        set {
    ...
    public bool AndromedaCATEnabled {
        get {
            if (chkEnableAndromeda != null)
                return chkEnableAndromeda.Checked;
            else
                return false;
        }
        set {
    ...
    public bool AriesCATEnabled {
        get {
            if (chkEnableAries != null)
                return chkEnableAries.Checked;
            else
                return false;
        }
        set {
    ...
    public bool GanymedeCATEnabled {
        get {
            if (chkEnableGanymede != null)
                return chkEnableGanymede.Checked;
            else
                return false;
        }
        set {
    ...
    public int RXAGCAttack {
        get {
            if (udDSPAGCAttack != null)
                return (int)udDSPAGCAttack.Value;
            else
                return 0;
        }
        set {
    ...
    public int RXAGCHang {
        get {
            if (udDSPAGCHangTime != null)
                return (int)udDSPAGCHangTime.Value;
            else
                return 0;
        }
        set {
    ...
    public int RXAGCDecay {
        get {
            if (udDSPAGCDecay != null)
                return (int)udDSPAGCDecay.Value;
            else
                return 0;
        }
        set {
    ...
    public int RX2AGCAttack {
        get {
            if (udDSPAGCRX2Attack != null)
                return (int)udDSPAGCRX2Attack.Value;
            else
                return 0;
        }
        set {
    ...
    public int RX2AGCHang {
        get {
            if (udDSPAGCRX2HangTime != null)
                return (int)udDSPAGCRX2HangTime.Value;
            else
                return 0;
        }
        set {
    ...
    public int RX2AGCDecay {
        get {
            if (udDSPAGCRX2Decay != null)
                return (int)udDSPAGCRX2Decay.Value;
            else
                return 0;
        }
        set {
    ...
    public int BreakInDelay {
        get {
            if (udCWBreakInDelay != null)
                return (int)udCWBreakInDelay.Value;
            else
                return -1;
        }
        set {
    ...
    public int CWPitch {
        get {
            if (udDSPCWPitch != null)
                return (int)udDSPCWPitch.Value;
            else
                return -1;
        }
        set {
    ...
    public bool CWDisableMonitor {
        get {
            if (chkDSPKeyerSidetone != null)
                return chkDSPKeyerSidetone.Checked;
            else
                return false;
        }
        set {
            if (chkDSPKeyerSidetone != null)
                chkDSPKeyerSidetone.Checked = value;
        }
    }

    public bool CWIambic {
        get {
            if (chkCWKeyerIambic != null)
                return chkCWKeyerIambic.Checked;
            else
                return false;
        }
        set {
    ...
    public bool CFCEnabled {
        get { return chkCFCEnable.Checked; }
        set {
    ...
    public bool PhaseRotEnabled {
        get { return chkPHROTEnable.Checked; }
        set {
    ...
    public string VACSampleRate {
        get {
            if (comboAudioSampleRate2 != null)
                return comboAudioSampleRate2.Text;
            else
                return "";
        }
        set {
            if (comboAudioSampleRate2 != null)
                comboAudioSampleRate2.Text = value;
        }
    }

    public string VAC2SampleRate {
        get {
            if (comboAudioSampleRate3 != null)
                return comboAudioSampleRate3.Text;
            else
                return "";
        }
        set {
            if (comboAudioSampleRate3 != null)
                comboAudioSampleRate3.Text = value;
        }
    }
    ...
    public bool IQOutToVAC {
        get {
            if (chkAudioIQtoVAC != null)
                return chkAudioIQtoVAC.Checked;
            else
                return false;
        }
        set {
    ...
    public bool VAC2DirectIQ {
        get {
            if (chkVAC2DirectIQ != null)
                return chkVAC2DirectIQ.Checked;
            else
                return false;
        }
        set {
    ...
    public bool VACStereo {
        get {
            if (chkAudio2Stereo != null)
                return chkAudio2Stereo.Checked;
            else
                return false;
        }
        set {
    ...
    public bool VAC2Stereo {
        get {
            if (chkAudioStereo3 != null)
                return chkAudioStereo3.Checked;
            else
                return false;
        }
        set {
    ...
    public int VOXSens {
        get {
            if (udDEXPThreshold != null)
                return (int)udDEXPThreshold.Value;
            else
                return -1;
        }
        set {
    ...
    public bool NoiseGateEnabled {
        get {
            ...
            // if (chkTXNoiseGateEnabled != null) return
            // chkTXNoiseGateEnabled.Checked;
            else
                return false;
        }
        set {
            if (chkDEXPEnable != null) chkDEXPEnable.Checked = value;
            // if (chkTXNoiseGateEnabled != null) chkTXNoiseGateEnabled.Checked
            // = value;
        }
    }
    ...
    public int VACRXGain {
        get {
            if (udAudioVACGainRX != null)
                return (int)udAudioVACGainRX.Value;
            else
                return -99;
        }
        set {
    ...
    public int VAC2RXGain {
        get {
            if (udVAC2GainRX != null)
                return (int)udVAC2GainRX.Value;
            else
                return -99;
        }
        set {
    ...
    public int VACTXGain {
        get {
            if (udAudioVACGainTX != null)
                return (int)udAudioVACGainTX.Value;
            else
                return -99;
        }
        set {
    ...
    public int VAC2TXGain {
        get {
            if (udVAC2GainTX != null)
                return (int)udVAC2GainTX.Value;
            else
                return -99;
        }
        set {
    ...
    public CheckState BreakInEnabledState {
        ...
        set {
            if (chkCWBreakInEnabled != null) {
                if (chkCWBreakInEnabled.CheckState == value)
                    chkCWBreakInEnabled_CheckStateChanged(
    ...
    public int RX1subAPFFreq {
        get {
            if (tbRX1subAPFTune != null)
                return (int)tbRX1subAPFTune.Value;
            else
                return -1;
        }
        set {
    ...
    public int RX1subAPFBandwidth {
        get {
            if (tbRX1subAPFBW != null)
                return (int)tbRX1subAPFBW.Value;
            else
                return -1;
        }
        set {
    ...
    public int RX1APFGain {
        get {
            if (tbDSPAudRX1APFGain != null)
                return tbDSPAudRX1APFGain.Value;
            else
                return -1;
        }
        set {
    ...
    public int RX1subAPFGain {
        get {
            if (tbDSPAudRX1subAPFGain != null)
                return tbDSPAudRX1subAPFGain.Value;
            else
                return -1;
        }
        set {
    ...
    public int RX2APFGain {
        get {
            // if (udDSPAudRX2APFGain != null) return
            // (int)udDSPAudRX2APFGain.Value;
            if (tbDSPAudRX2APFGain != null)
                return tbDSPAudRX2APFGain.Value;
            else
                return -1;
        }
        set {
    ...
    public int AGCMaxGain {
        get {
            if (udDSPAGCMaxGaindB != null)
                return (int)udDSPAGCMaxGaindB.Value;
            else
                return -1;
        }
        set {
    ...
    public int AGCRX2FixedGain {
        get {
            if (udDSPAGCRX2FixedGaindB != null)
                return (int)udDSPAGCRX2FixedGaindB.Value;
            else
                return -1;
        }
        set {
            if (udDSPAGCRX2FixedGaindB != null)
                udDSPAGCRX2FixedGaindB.Value = value;
        }
    }

    public int AGCHangThreshold {
        get { return tbDSPAGCHangThreshold.Value; }
        set {
    ...
    public int SetAGCRX2HangThreshold {
        set {
            if (tbDSPAGCRX2HangThreshold != null) {
                ...
                if (value > tbDSPAGCRX2HangThreshold.Maximum)
                    value = (int)tbDSPAGCRX2HangThreshold.Maximum;
                if (value < tbDSPAGCRX2HangThreshold.Minimum)
                    value = (int)tbDSPAGCRX2HangThreshold.Minimum;

                if (tbDSPAGCRX2HangThreshold.Value == 0) {
                    tbDSPAGCRX2HangThreshold.Value = value + 1;
                    tbDSPAGCRX2HangThreshold_Scroll(this, EventArgs.Empty);
                    tbDSPAGCRX2HangThreshold.Value
                        = tbDSPAGCRX2HangThreshold.Value - 1;
                    tbDSPAGCRX2HangThreshold_Scroll(this, EventArgs.Empty);
                } else {
                    tbDSPAGCRX2HangThreshold.Value = value - 1;
                    tbDSPAGCRX2HangThreshold_Scroll(this, EventArgs.Empty);
    ...
    public int SetAGCHangThres {
        set {
            if (tbDSPAGCHangThreshold != null) {
                if (value > tbDSPAGCHangThreshold.Maximum)
                    value = (int)tbDSPAGCHangThreshold.Maximum;
                if (value < tbDSPAGCHangThreshold.Minimum)
                    value = (int)tbDSPAGCHangThreshold.Minimum;

                if (tbDSPAGCHangThreshold.Value == 0) {
                    tbDSPAGCHangThreshold.Value = value + 1;
                    tbDSPAGCHangThreshold_Scroll(this, EventArgs.Empty);
                    tbDSPAGCHangThreshold.Value
                        = tbDSPAGCHangThreshold.Value - 1;
                    tbDSPAGCHangThreshold_Scroll(this, EventArgs.Empty);
                } else {
                    tbDSPAGCHangThreshold.Value = value - 1;
                    tbDSPAGCHangThreshold_Scroll(this, EventArgs.Empty);
    ...
    public int DisplayFFTSize {
        ...
        set {
            if (value > tbDisplayFFTSize.Maximum)
                value = (int)tbDisplayFFTSize.Maximum;
            if (value < tbDisplayFFTSize.Minimum)
                value = (int)tbDisplayFFTSize.Minimum;
    ...
    public int TXFilterHigh {
        ...
        set {
            if (value > udTXFilterHigh.Maximum)
                value = (int)udTXFilterHigh.Maximum;
            if (value < udTXFilterHigh.Minimum)
                value = (int)udTXFilterHigh.Minimum;
    ...
    public int TXFilterLow {
        ...
        set {
            if (value > udTXFilterLow.Maximum)
                value = (int)udTXFilterLow.Maximum;
            if (value < udTXFilterLow.Minimum)
                value = (int)udTXFilterLow.Minimum;
    ...
    public int TXFilterHighSave {
        ...
        set {
            if (console.TXBand != Band.B60M) {
                if (value > udTXFilterHighSave.Maximum)
                    value = (int)udTXFilterHighSave.Maximum;
                if (value < udTXFilterHighSave.Minimum)
                    value = (int)udTXFilterHighSave.Minimum;
    ...
    public int TXFilterLowSave {
        ...
        set {
            if (console.TXBand != Band.B60M) {
                if (value > udTXFilterLowSave.Maximum)
                    value = (int)udTXFilterLowSave.Maximum;
                if (value < udTXFilterLowSave.Minimum)
                    value = (int)udTXFilterLowSave.Minimum;
    ...
    public float DisplayGridMin {
        ...
        set {
            if (udDisplayGridMin != null) {
                if (value > (float)udDisplayGridMin.Maximum)
                    value = (float)udDisplayGridMin.Maximum;
                if (value < (float)udDisplayGridMin.Minimum)
                    value = (float)udDisplayGridMin.Minimum;
    ...
    public float DisplayGridMax {
        ...
        set {
            if (udDisplayGridMax != null) {
                if (value > (float)udDisplayGridMax.Maximum)
                    value = (float)udDisplayGridMax.Maximum;
                if (value < (float)udDisplayGridMax.Minimum)
                    value = (float)udDisplayGridMax.Minimum;
    ...
    public float RX2DisplayGridMin {
        ...
        set {
            if (udRX2DisplayGridMin != null) {
                if (value > (float)udRX2DisplayGridMin.Maximum)
                    value = (float)udRX2DisplayGridMin.Maximum;
                if (value < (float)udRX2DisplayGridMin.Minimum)
                    value = (float)udRX2DisplayGridMin.Minimum;
    ...
    public float RX2DisplayGridMax {
        ...
        set {
            if (udRX2DisplayGridMax != null) {
                if (value > (float)udRX2DisplayGridMax.Maximum)
                    value = (float)udRX2DisplayGridMax.Maximum;
                if (value < (float)udRX2DisplayGridMax.Minimum)
                    value = (float)udRX2DisplayGridMax.Minimum;
    ...
    public float TXGridMax {
        get {
            if (udTXGridMax != null)
                return (float)udTXGridMax.Value;
            else
                return -1;
        }
        set {
            if (udTXGridMax != null) {
                if (value > (float)udTXGridMax.Maximum)
                    value = (float)udTXGridMax.Maximum;
                if (value < (float)udTXGridMax.Minimum)
                    value = (float)udTXGridMax.Minimum;
    ...
    public float TXGridMin {
        get {
            if (udTXGridMin != null)
                return (float)udTXGridMin.Value;
            else
                return -1;
        }
        set {
            if (udTXGridMin != null) {
                if (value > (float)udTXGridMin.Maximum)
                    value = (float)udTXGridMin.Maximum;
                if (value < (float)udTXGridMin.Minimum)
                    value = (float)udTXGridMin.Minimum;
    ...
    public void CATSetRig(string rig) { comboCATRigType.Text = rig; }
    ...
    public bool FirmwareBypass {
        get { return firmware_bypass; }
        set { firmware_bypass = value; }
    }
    ...
    public int[] CFCCOMPEQ {
        ...
        set {
            ...
            tbCFCPRECOMP.Value = Math.Max(
                tbCFCPRECOMP.Minimum, Math.Min(tbCFCPRECOMP.Maximum, value[0]));
            tbCFC0.Value
                = Math.Max(tbCFC0.Minimum, Math.Min(tbCFC0.Maximum, value[1]));
            tbCFC1.Value
                = Math.Max(tbCFC1.Minimum, Math.Min(tbCFC1.Maximum, value[2]));
            tbCFC2.Value
                = Math.Max(tbCFC2.Minimum, Math.Min(tbCFC2.Maximum, value[3]));
            tbCFC3.Value
                = Math.Max(tbCFC3.Minimum, Math.Min(tbCFC3.Maximum, value[4]));
            tbCFC4.Value
                = Math.Max(tbCFC4.Minimum, Math.Min(tbCFC4.Maximum, value[5]));
            tbCFC5.Value
                = Math.Max(tbCFC5.Minimum, Math.Min(tbCFC5.Maximum, value[6]));
            tbCFC6.Value
                = Math.Max(tbCFC6.Minimum, Math.Min(tbCFC6.Maximum, value[7]));
            tbCFC7.Value
                = Math.Max(tbCFC7.Minimum, Math.Min(tbCFC7.Maximum, value[8]));
            tbCFC8.Value
                = Math.Max(tbCFC8.Minimum, Math.Min(tbCFC8.Maximum, value[9]));
            tbCFC9.Value
                = Math.Max(tbCFC9.Minimum, Math.Min(tbCFC9.Maximum, value[10]));
            tbCFCPEQGAIN.Value = Math.Max(tbCFCPEQGAIN.Minimum,
                Math.Min(tbCFCPEQGAIN.Maximum, value[11]));
            tbCFCEQ0.Value = Math.Max(
                tbCFCEQ0.Minimum, Math.Min(tbCFCEQ0.Maximum, value[12]));
            tbCFCEQ1.Value = Math.Max(
                tbCFCEQ1.Minimum, Math.Min(tbCFCEQ1.Maximum, value[13]));
            tbCFCEQ2.Value = Math.Max(
                tbCFCEQ2.Minimum, Math.Min(tbCFCEQ2.Maximum, value[14]));
            tbCFCEQ3.Value = Math.Max(
                tbCFCEQ3.Minimum, Math.Min(tbCFCEQ3.Maximum, value[15]));
            tbCFCEQ4.Value = Math.Max(
                tbCFCEQ4.Minimum, Math.Min(tbCFCEQ4.Maximum, value[16]));
            tbCFCEQ5.Value = Math.Max(
                tbCFCEQ5.Minimum, Math.Min(tbCFCEQ5.Maximum, value[17]));
            tbCFCEQ6.Value = Math.Max(
                tbCFCEQ6.Minimum, Math.Min(tbCFCEQ6.Maximum, value[18]));
            tbCFCEQ7.Value = Math.Max(
                tbCFCEQ7.Minimum, Math.Min(tbCFCEQ7.Maximum, value[19]));
            tbCFCEQ8.Value = Math.Max(
                tbCFCEQ8.Minimum, Math.Min(tbCFCEQ8.Maximum, value[20]));
            tbCFCEQ9.Value = Math.Max(
                tbCFCEQ9.Minimum, Math.Min(tbCFCEQ9.Maximum, value[21]));
            udCFC0.Value
                = Math.Max(udCFC0.Minimum, Math.Min(udCFC0.Maximum, value[22]));
            udCFC1.Value
                = Math.Max(udCFC1.Minimum, Math.Min(udCFC1.Maximum, value[23]));
            udCFC2.Value
                = Math.Max(udCFC2.Minimum, Math.Min(udCFC2.Maximum, value[24]));
            udCFC3.Value
                = Math.Max(udCFC3.Minimum, Math.Min(udCFC3.Maximum, value[25]));
            udCFC4.Value
                = Math.Max(udCFC4.Minimum, Math.Min(udCFC4.Maximum, value[26]));
            udCFC5.Value
                = Math.Max(udCFC5.Minimum, Math.Min(udCFC5.Maximum, value[27]));
            udCFC6.Value
                = Math.Max(udCFC6.Minimum, Math.Min(udCFC6.Maximum, value[28]));
            udCFC7.Value
                = Math.Max(udCFC7.Minimum, Math.Min(udCFC7.Maximum, value[29]));
            udCFC8.Value
                = Math.Max(udCFC8.Minimum, Math.Min(udCFC8.Maximum, value[30]));
            udCFC9.Value
                = Math.Max(udCFC9.Minimum, Math.Min(udCFC9.Maximum, value[31]));
...
#region General Tab Event Handlers
    ...
    private void InitHPSDR() {
        ...
        if (console.CurrentHPSDRModel == HPSDRModel.ANAN8000D
            ...
            toolTip1.SetToolTip(
                chkEXT2OutOnTx, "Enable Rx BYPASS during transmit.");
        } else {
            ...
            toolTip1.SetToolTip(chkEXT2OutOnTx,
                "Enable RX 1 IN on Alex or Ext 2 on ANAN during transmit.");
        }
        ...
        if (console.CurrentHPSDRModel == HPSDRModel.ANAN10
            ...
            chkEnableXVTRHF.Visible = false;
        } else if (console.CurrentHPSDRModel == HPSDRModel.ANAN7000D
            || console.CurrentHPSDRModel == HPSDRModel.ANAN8000D) {
    ...
    public void AddHPSDRPages() {
        ...
        if (console.CurrentHPSDRModel == HPSDRModel.ANAN200D
            ...
            //                         Common.TabControlInsert(tcGeneral, tpADC,
            //                         2);
            //                     }
            //                 }
        }
        ...
        if (chkGeneralRXOnly.Focused && !chkGeneralRXOnly.Checked) {
            DialogResult dr = MessageBox.Show(
                "Unchecking Receive Only while in Unsupported Card mode may \n"
                    ...
                    + "to enable transmit?",
                "Warning: Enable Transmit?", MessageBoxButtons.YesNo,
    ...
    private void btnGeneralCalLevelStart_Click(
        ...
        Thread t = new Thread(new ThreadStart(CalibrateLevel)) { Name
            = "Level Calibration Thread",
            IsBackground = true, Priority = ThreadPriority.AboveNormal };
        t.Start();
    ...
    private void btnCalLevel_Click(object sender, System.EventArgs e) {
        ...
        Thread t = new Thread(new ThreadStart(CalibrateRX2Level)) { Name
            = "Level Calibration Thread",
    ...
    private void CalibrateRX2Level() {
        bool done = console.CalibrateRX2Level((float)udGeneralCalRX2Level.Value,
    ...
    private void comboGeneralProcessPriority_SelectedIndexChanged(
        ...
        if (comboGeneralProcessPriority.Text == "Real Time"
            ...
            DialogResult dr = MessageBox.Show(
                ...
                "Warning: Realtime Not Recommended", MessageBoxButtons.YesNo,
                MessageBoxIcon.Warning);
            if (dr == DialogResult.No) {
                switch (p.PriorityClass) {
                    ...
                    case ProcessPriorityClass.High:
                        ...
                        break;
                    default: comboGeneralProcessPriority.Text = "Normal"; break;
                }
                return;
            }
        }
...
#region Audio Tab Event Handlers
    ...
    private void chkVAC2Enable_CheckedChanged(
        ...
        if (console.sliderForm != null) console.sliderForm.RX2VACOnOff = val;
    }
    ...
    public bool ForceReset {
        set {
            force_reset = value;
            if (value) {
                comboAudioSampleRate1_SelectedIndexChanged(
                    this, EventArgs.Empty);
                comboAudioSampleRateRX2_SelectedIndexChanged(
                    this, EventArgs.Empty);
            }
        }
    }
    ...
    public bool AndromedaFastTune {
        get { return chkAndrFastStepTune.Checked; }
        set { chkAndrFastStepTune.Checked = value; }
    }

    // access the check box for Andromeda to control auto-show of the gain
    // setting form
    public bool AndromedaGainAutoShow {
        get { return chkAndrGainAutoshow.Checked; }
        set { chkAndrGainAutoshow.Checked = value; }
    }

    // access the check box for Andromeda to control auto-show of the diversity
    // form
    public bool AndromedaDiversityAutoShow {
        get { return chkAndrDiversityAutoshow.Checked; }
        set { chkAndrDiversityAutoshow.Checked = value; }
    }

    // access the check box for Andromeda to control Diversity form portrait or
    // landscape layout
    public bool AndromedaDiversityFormLandscape {
        get { return chkAndrDiversityLandscape.Checked; }
        set { chkAndrDiversityLandscape.Checked = value; }
    }
    ...
    private void comboAudioSampleRateRX2_SelectedIndexChanged(
        ...
        if ((NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH)
            && (new_rate != old_rate || initializing)) {
            // turn OFF the DSP channel so it gets flushed out (must do while
            // data is flowing to get slew-down and flush)
            ...
            if (console.PowerOn && console.VAC2Enabled && !initializing)
                Audio.EnableVAC2(false);
            // turn OFF the DDC for RX2; had to add rx2_enabled as a parameter
            // to UpdateReceivers() to do this --- THIS IS S HACK.  BETTER WAY?
            console.UpdateDDCs(false);
            // wait for all inflight packets to arrive
            Thread.Sleep(20); // need to experiment with this time

            // flush any buffers where these old-rate packets may be stored ...
            // this is done via operations in SetXcmInrate()

            // in the property SampleRateRX2:
            //    (1) the new samplerate will be sent to the DDC
            //    (2) SetXcmInrate() is called by Audio.SampleRateRX2 which is
            //    called by console.SampleRateRX2
            console.SampleRateRX2 = new_rate;
            // set the corresponding new buffer size
            int new_size = cmaster.GetBuffSize(new_rate);
            ...
            // turn the DDC for RX2 back ON; had to add rx2_enabled as a
            // parameter to UpdateReceivers() to do this
            if (was_enabled) {
                console.UpdateDDCs(true);
                // wait for samples at the new rate to be received
            ...
            // calculate and display the new bin_width
            double bin_width = (double)new_rate
                / (double)console.specRX.GetSpecRX(1).FFTSize;
...
#region Display Tab Event Handlers
    ...
    private void udDisplayFPS_ValueChanged(object sender, System.EventArgs e) {
        ...
        // console.specRX.GetSpecRX(cmaster.inid(1, 0)).FrameRate =
        // (int)udDisplayFPS.Value;
    ...
    private void udRX2DisplayGridMax_ValueChanged(
        ...
        switch (console.RX2Band) {
            case Band.B160M:
                console.RX2DisplayGridMax160m
                    = (float)udRX2DisplayGridMax.Value;
    ...
    private void udRX2DisplayGridMin_ValueChanged(
        ...
        switch (console.RX2Band) {
            case Band.B160M:
                console.RX2DisplayGridMin160m
                    = (float)udRX2DisplayGridMin.Value;
    ...
    private void udDisplayAVGTime_ValueChanged(
        ...
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).AvTau
            = 0.001 * (double)udDisplayAVGTime.Value;
        ...
        // Display.DisplayAvgBlocks = (int)Math.Max(2, buffersToAvg);  //MW0LGE
        // not needed
    }
    ...
    private void udDisplayWaterfallLowLevel_ValueChanged(
        ...
        switch (console.RX1Band) {
            case Band.B160M:
                console.WaterfallLowThreshold160m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold160m;
                break;
            case Band.B80M:
                console.WaterfallLowThreshold80m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold80m;
                break;
            case Band.B60M:
                console.WaterfallLowThreshold60m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold60m;
                break;
            case Band.B40M:
                console.WaterfallLowThreshold40m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold40m;
                break;
            case Band.B30M:
                console.WaterfallLowThreshold30m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold30m;
                break;
            case Band.B20M:
                console.WaterfallLowThreshold20m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold20m;
                break;
            case Band.B17M:
                console.WaterfallLowThreshold17m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold17m;
                break;
            case Band.B15M:
                console.WaterfallLowThreshold15m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold15m;
                break;
            case Band.B12M:
                console.WaterfallLowThreshold12m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold12m;
                break;
            case Band.B10M:
                console.WaterfallLowThreshold10m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThreshold10m;
                break;
            case Band.B6M:
                console.WaterfallLowThreshold6m
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold = console.WaterfallLowThreshold6m;
                break;
            case Band.WWV:
                console.WaterfallLowThresholdWWV
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThresholdWWV;
                break;
            case Band.GEN:
                console.WaterfallLowThresholdGEN
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThresholdGEN;
                break;
            default:
                console.WaterfallLowThresholdXVTR
                    = (float)udDisplayWaterfallLowLevel.Value;
                Display.WaterfallLowThreshold
                    = console.WaterfallLowThresholdXVTR;
                break;
    ...
    private void udDisplayWaterfallHighLevel_ValueChanged(
        ...
        switch (console.RX1Band) {
            case Band.B160M:
                console.WaterfallHighThreshold160m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold160m;
                break;
            case Band.B80M:
                console.WaterfallHighThreshold80m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold80m;
                break;
            case Band.B60M:
                console.WaterfallHighThreshold60m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold60m;
                break;
            case Band.B40M:
                console.WaterfallHighThreshold40m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold40m;
                break;
            case Band.B30M:
                console.WaterfallHighThreshold30m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold30m;
                break;
            case Band.B20M:
                console.WaterfallHighThreshold20m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold20m;
                break;
            case Band.B17M:
                console.WaterfallHighThreshold17m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold17m;
                break;
            case Band.B15M:
                console.WaterfallHighThreshold15m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold15m;
                break;
            case Band.B12M:
                console.WaterfallHighThreshold12m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold12m;
                break;
            case Band.B10M:
                console.WaterfallHighThreshold10m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold10m;
                break;
            case Band.B6M:
                console.WaterfallHighThreshold6m
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThreshold6m;
                break;
            case Band.WWV:
                console.WaterfallHighThresholdWWV
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThresholdWWV;
                break;
            case Band.GEN:
                console.WaterfallHighThresholdGEN
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThresholdGEN;
                break;
            default:
                console.WaterfallHighThresholdXVTR
                    = (float)udDisplayWaterfallHighLevel.Value;
                Display.WaterfallHighThreshold
                    = console.WaterfallHighThresholdXVTR;
                break;
    ...
    private void udRX2DisplayWaterfallLowLevel_ValueChanged(
        ...
        switch (console.RX2Band) {
            case Band.B160M:
                console.RX2WaterfallLowThreshold160m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold160m;
                break;
            case Band.B80M:
                console.RX2WaterfallLowThreshold80m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold80m;
                break;
            case Band.B60M:
                console.RX2WaterfallLowThreshold60m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold60m;
                break;
            case Band.B40M:
                console.RX2WaterfallLowThreshold40m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold40m;
                break;
            case Band.B30M:
                console.RX2WaterfallLowThreshold30m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold30m;
                break;
            case Band.B20M:
                console.RX2WaterfallLowThreshold20m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold20m;
                break;
            case Band.B17M:
                console.RX2WaterfallLowThreshold17m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold17m;
                break;
            case Band.B15M:
                console.RX2WaterfallLowThreshold15m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold15m;
                break;
            case Band.B12M:
                console.RX2WaterfallLowThreshold12m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold12m;
                break;
            case Band.B10M:
                console.RX2WaterfallLowThreshold10m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold10m;
                break;
            case Band.B6M:
                console.RX2WaterfallLowThreshold6m
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThreshold6m;
                break;
            case Band.WWV:
                console.RX2WaterfallLowThresholdWWV
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThresholdWWV;
                break;
            case Band.GEN:
                console.RX2WaterfallLowThresholdGEN
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThresholdGEN;
                break;
            default:
                console.RX2WaterfallLowThresholdXVTR
                    = (float)udRX2DisplayWaterfallLowLevel.Value;
                Display.RX2WaterfallLowThreshold
                    = console.RX2WaterfallLowThresholdXVTR;
                break;
    ...
    private void udRX2DisplayWaterfallHighLevel_ValueChanged(
        ...
        switch (console.RX2Band) {
            case Band.B160M:
                console.RX2WaterfallHighThreshold160m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold160m;
                break;
            case Band.B80M:
                console.RX2WaterfallHighThreshold80m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold80m;
                break;
            case Band.B60M:
                console.RX2WaterfallHighThreshold60m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold60m;
                break;
            case Band.B40M:
                console.RX2WaterfallHighThreshold40m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold40m;
                break;
            case Band.B30M:
                console.RX2WaterfallHighThreshold30m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold30m;
                break;
            case Band.B20M:
                console.RX2WaterfallHighThreshold20m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold20m;
                break;
            case Band.B17M:
                console.RX2WaterfallHighThreshold17m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold17m;
                break;
            case Band.B15M:
                console.RX2WaterfallHighThreshold15m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold15m;
                break;
            case Band.B12M:
                console.RX2WaterfallHighThreshold12m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold12m;
                break;
            case Band.B10M:
                console.RX2WaterfallHighThreshold10m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold10m;
                break;
            case Band.B6M:
                console.RX2WaterfallHighThreshold6m
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThreshold6m;
                break;
            case Band.WWV:
                console.RX2WaterfallHighThresholdWWV
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThresholdWWV;
                break;
            case Band.GEN:
                console.RX2WaterfallHighThresholdGEN
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThresholdGEN;
                break;
            default:
                console.RX2WaterfallHighThresholdXVTR
                    = (float)udRX2DisplayWaterfallHighLevel.Value;
                Display.RX2WaterfallHighThreshold
                    = console.RX2WaterfallHighThresholdXVTR;
                break;
...
#region DSP Tab Event Handlers
    ...
    private void udLMSNR_ValueChanged(object sender, System.EventArgs e) {
        ...
        console.radio.GetDSPRX(0, 1).SetNRVals((int)udLMSNRtaps.Value,
            (int)udLMSNRdelay.Value, 1e-6 * (double)udLMSNRgain.Value,
            1e-3 * (double)udLMSNRLeak.Value);
    }
    ...
    private void udLMSNR2_ValueChanged(object sender, System.EventArgs e) {
        ...
        console.radio.GetDSPRX(1, 1).SetNRVals((int)udLMSNR2taps.Value,
            (int)udLMSNR2delay.Value, 1e-6 * (double)udLMSNR2gain.Value,
            1e-3 * (double)udLMSNR2Leak.Value);
    }
...
#region Image Reject
    ...
    private void udLMSANF_ValueChanged(object sender, System.EventArgs e) {
        ...
        console.radio.GetDSPRX(0, 1).SetANFVals((int)udLMSANFtaps.Value,
            (int)udLMSANFdelay.Value, 1e-6 * (double)udLMSANFgain.Value,
            1e-3 * (double)udLMSANFLeak.Value);
    }
    ...
    private void udLMSANF2_ValueChanged(object sender, System.EventArgs e) {
        ...
        console.radio.GetDSPRX(1, 1).SetANFVals((int)udLMSANF2taps.Value,
            (int)udLMSANF2delay.Value, 1e-6 * (double)udLMSANF2gain.Value,
            1e-3 * (double)udLMSANF2Leak.Value);
    }
...
#region Keyer
    ...
    private void chkCWBreakInEnabled_CheckStateChanged(
        ...
        switch (chkCWBreakInEnabled.CheckState) {
            ...
            case CheckState.Indeterminate:
                if ((console.CurrentHPSDRHardware == HPSDRHW.Orion
                        || console.CurrentHPSDRHardware == HPSDRHW.OrionMKII)
                    && (NetworkIO.FWCodeVersion >= 17)
                    && !Alex.trx_ant_not_same) {
                    console.BreakInEnabledState
    ...
    private void comboKeyerConnSecondary_SelectedIndexChanged(
        ...
        if (comboKeyerConnSecondary.Text == "CAT") {
            if (!chkCATEnable.Checked) {
                MessageBox.Show(
                    "CAT is not Enabled.  Please enable the CAT interface before selecting this option.",
                    "CAT not enabled", MessageBoxButtons.OK,
                    MessageBoxIcon.Hand);
        ...
        if (!CWInput.SetSecondaryInput(comboKeyerConnSecondary.Text)) {
            MessageBox.Show("Error using " + comboKeyerConnSecondary.Text
                    + " for Keyer Secondary Input.\n"
                    + "The port may already be in use by another application.",
        ...
        switch (comboKeyerConnSecondary.Text) {
            ...
            case "CAT":
                ...
                comboKeyerConnPTTLine_SelectedIndexChanged(
                    this, EventArgs.Empty);
                comboKeyerConnKeyLine_SelectedIndexChanged(
                    this, EventArgs.Empty);
                break;
            default: // COMx
                lblKeyerConnPTTLine.Visible = true;
    ...
    private void comboKeyerConnKeyLine_SelectedIndexChanged(
        ...
        if (comboKeyerConnSecondary.Text == "CAT") {
            ...
            CWInput.SecondaryKeyLine
                = (KeyerLine)comboKeyerConnKeyLine.SelectedIndex;
    }
    ...
    private void comboKeyerConnPTTLine_SelectedIndexChanged(
        ...
        if (comboKeyerConnSecondary.Text == "CAT") {
            ...
            CWInput.SecondaryPTTLine
                = (KeyerLine)comboKeyerConnPTTLine.SelectedIndex;
    }
...
#region AGC
    ...
    private void udDSPAGCRX2MaxGaindB_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXAGCMaxGain
            = (double)udDSPAGCRX2MaxGaindB.Value;
        // console.radio.GetDSPRX(1, 1).RXAGCMaxGain =
        // (double)udDSPAGCRX2MaxGaindB.Value;
    ...
    private void udDSPAGCRX2FixedGaindB_ValueChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXFixedAGC
            = (double)udDSPAGCRX2FixedGaindB.Value;
        //  console.radio.GetDSPRX(1, 1).RXFixedAGC =
        //  (double)udDSPAGCRX2FixedGaindB.Value;
    ...
    private void udDSPAGCRX2Decay_ValueChanged(
        ...
        //  if (udDSPAGCRX2Decay.Enabled)
        {
            console.radio.GetDSPRX(1, 0).RXAGCDecay
                = (int)udDSPAGCRX2Decay.Value;
            //    console.radio.GetDSPRX(1, 1).RXAGCDecay =
            //    (int)udDSPAGCRX2Decay.Value;
        }
    }
    ...
    private void udDSPAGCHangTime_ValueChanged(
        ...
        // if (udDSPAGCHangTime.Enabled)
        {
            console.radio.GetDSPRX(0, 0).RXAGCHang
                = (int)udDSPAGCHangTime.Value;
            console.radio.GetDSPRX(0, 1).RXAGCHang
                = (int)udDSPAGCHangTime.Value;
        }
    }
    ...
    private void udDSPAGCRX2HangTime_ValueChanged(
        ...
        // if (udDSPAGCRX2HangTime.Enabled)
        {
            console.radio.GetDSPRX(1, 0).RXAGCHang
                = (int)udDSPAGCRX2HangTime.Value;
            //    console.radio.GetDSPRX(1, 1).RXAGCHang =
            //    (int)udDSPAGCRX2HangTime.Value;
        }
    }
...
#region Transmit Tab Event Handlers
    ...
    private bool loadTXProfile(String sProfileName) {
        DataRow[] rows
            = DB.ds.Tables["TxProfile"].Select("'" + sProfileName + "' = Name");
        ...
        for (int i = 1; i < 11; i++) eq[i] = (int)dr["TXEQ" + i.ToString()];
        for (int i = 11; i < 21; i++)
            eq[i] = (int)dr["TxEqFreq" + (i - 10).ToString()];
        console.EQForm.TXEQ = eq;

        udTXFilterLow.Value
            = Math.Min(Math.Max((int)dr["FilterLow"], udTXFilterLow.Minimum),
                udTXFilterLow.Maximum);
        udTXFilterHigh.Value
            = Math.Min(Math.Max((int)dr["FilterHigh"], udTXFilterHigh.Minimum),
                udTXFilterHigh.Maximum);
        ...
        // console.PWR = (int)dr["Power"];  // MW0LGE power is stored per band,
        // leaving this in will prevent the power per band from being recalled
        ...
        // udTXTunePower.Value = (int)dr["Tune_Power"];  //MW0LGE not applied
        // here anymore, leave in save incase we want to use it
        ...
        udPhRotFreq.Value = Math.Min(
            Math.Max((int)dr["CFCPhaseRotatorFreq"], udPhRotFreq.Minimum),
            udPhRotFreq.Maximum);
        udPHROTStages.Value = Math.Min(
            Math.Max((int)dr["CFCPhaseRotatorStages"], udPHROTStages.Minimum),
            udPHROTStages.Maximum);
    ...
    private void comboTXProfileName_SelectedIndexChanged(
        ...
        if (chkAutoSaveTXProfile.Checked) {
            ...
            if (CheckTXProfileChanged() && comboTXProfileName.Focused) {
                DialogResult result
                    = MessageBox.Show("The current profile has changed.  "
                            + "Would you like to save the current profile?",
                        "Save Current Profile?", MessageBoxButtons.YesNoCancel,
    ...
    private void btnTXProfileSave_Click(object sender, System.EventArgs e) {
        ...
        if (string.IsNullOrEmpty(name)) {
            MessageBox.Show("TX Profile Save cancelled", "TX Profile",
        ...
        if (comboTXProfileName.Items.Contains(name)) {
            DialogResult result
                = MessageBox.Show("Are you sure you want to overwrite the "
                        + name + " TX Profile?",
            ...
            foreach (DataRow d in from DataRow d in DB.ds.Tables["TxProfile"]
                         .Rows where(string) d["Name"]
                == name select d) {
                dr = d;
                break;
            }
    ...
    private void chkSaveTXProfileOnExit_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void chkShowBandControls_CheckedChanged(
        object sender, EventArgs e) {
    ...
    //
    // G8NJJ: setup control to select an Andromeda top bar when display is
    // collapsed
    //
    private void chkShowAndromedaTop_CheckedChanged(
        object sender, EventArgs e) {
    ...
    // collapsed
    //
    private void chkShowAndromedaBar_CheckedChanged(
        object sender, EventArgs e) {
...
#region PA Settings Tab Event Handlers
    ...
    private void btnPAGainCalibration_Click(object sender, System.EventArgs e) {
        ...
        DialogResult dr
            = MessageBox.Show(s, "Warning: Is dummy load properly connected?",
        ...
        Thread t
            = new Thread(CalibratePAGain) { Name = "PA Gain Calibration Thread",
                  IsBackground = true, Priority = ThreadPriority.AboveNormal };
        t.Start();
    ...
    private void CalibratePAGain() {
        ...
        bool done = false;
        done = chkPANewCal.Checked
            ? console.CalibratePAGain2(progress, run, false)
            : console.CalibratePAGain(progress, run, (int)udPACalPower.Value);
    ...
    private void btnPAGainReset_Click(object sender, System.EventArgs e) {
        ...
        if (console.CurrentHPSDRModel == HPSDRModel.ANAN100D
            && !chkBypassANANPASettings.Checked) {
            ANANPAGain160 = 49.5f;
            ANANPAGain80 = 50.5f;
            ANANPAGain60 = 50.5f;
            ANANPAGain40 = 50.0f;
...
#region CAT Setup event handlers
    ...
    public void initCATandPTTprops() {
        ...
        console.MaxMIDIMessagesPerTuneStep
            = Convert.ToInt32(udUpdatesPerStepMax.Value);
        console.MinMIDIMessagesPerTuneStep
            = Convert.ToInt32(udUpdatesPerStepMin.Value);
        ...
        if (comboCATPTTPort.Text.StartsWith("COM"))
            console.CATPTTBitBangPort
                = Int32.Parse(comboCATPTTPort.Text.Substring(3));
        console.CATParity
            = SDRSerialPort.StringToParity((string)comboCATparity.SelectedItem);
        ...
        console.CAT2Parity = SDRSerialPort.StringToParity(
            (string)comboCAT2parity.SelectedItem);
        ...
        console.CAT3Parity = SDRSerialPort.StringToParity(
            (string)comboCAT3parity.SelectedItem);
        console.CAT3DataBits
            = int.Parse((string)comboCAT3databits.SelectedItem);
        console.CAT3StopBits = SDRSerialPort.StringToStopBits(
            (string)comboCAT3stopbits.SelectedItem);
        ...
        console.CAT4Parity = SDRSerialPort.StringToParity(
            (string)comboCAT4parity.SelectedItem);
        console.CAT4DataBits
            = int.Parse((string)comboCAT4databits.SelectedItem);
        console.CAT4StopBits = SDRSerialPort.StringToStopBits(
            (string)comboCAT4stopbits.SelectedItem);
        console.CAT4Enabled = chkCAT4Enable.Checked;

        if (comboAndromedaCATPort.Text.StartsWith("COM"))
            console.AndromedaCATPort
                = Int32.Parse(comboAndromedaCATPort.Text.Substring(3));
        console.AndromedaCATEnabled = chkEnableAndromeda.Checked;

        if (comboAriesCATPort.Text.StartsWith("COM"))
            console.AriesCATPort
                = Int32.Parse(comboAriesCATPort.Text.Substring(3));
        console.AriesCATEnabled = chkEnableAries.Checked;

        if (comboGanymedeCATPort.Text.StartsWith("COM"))
            console.GanymedeCATPort
                = Int32.Parse(comboGanymedeCATPort.Text.Substring(3));
    ...
    public void copyCATPropsToDialogVars() {
        ...
        if (comboCATPort.Items.Contains(port)) comboCATPort.Text = port;
    ...
    private void chkCATEnable_CheckedChanged(
        ...
        if (comboCATPort.Text == "" || !comboCATPort.Text.StartsWith("COM")) {
            if (chkCATEnable.Focused) {
                if (chkCATEnable.Checked) {
                    MessageBox.Show("The CAT port \"" + comboCATPort.Text
                        + "\" is not a valid port.\n"
        ...
        if (chkCATEnable.Checked && console.PTTBitBangEnabled
            ...
            MessageBox.Show("CAT port cannot be the same as Bit Bang Port",
                "Port Selection Error", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            chkCATEnable.Checked = false;
        }
        ...
        if (chkCATEnable.Checked) {
            try {
                ...
                chkCATEnable.Checked = false;
                MessageBox.Show(
                    "Could not initialize CAT control.  Exception was:\n\n "
                        + ex.Message + "\n\nCAT control has been disabled.",
                    "Error Initializing CAT control", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        } else {
            if (comboKeyerConnSecondary.Text == "CAT" && chkCATEnable.Focused) {
                MessageBox.Show(
                    "The Secondary Keyer option has been changed to None since CAT has been disabled.",
                    "CAT Disabled", MessageBoxButtons.OK,
            ...
            if (comboCATPTTPort.Text == "CAT" && chkCATEnable.Focused) {
                MessageBox.Show(
                    "The PTT Control Port option has been changed to None since CAT has been disabled.",
                    "CAT Disabled", MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
    ...
    private void chkCAT2Enable_CheckedChanged(
        ...
        if (comboCAT2Port.Text == "" || !comboCAT2Port.Text.StartsWith("COM")) {
            if (chkCAT2Enable.Focused) {
                if (chkCAT2Enable.Checked) {
                    MessageBox.Show("The CAT port \"" + comboCAT2Port.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkCAT2Enable.Checked = false;
                }
    ...
    private void chkCAT3Enable_CheckedChanged(
        ...
        if (comboCAT3Port.Text == "" || !comboCAT3Port.Text.StartsWith("COM")) {
            if (chkCAT3Enable.Focused) {
                if (chkCAT3Enable.Checked) {
                    MessageBox.Show("The CAT port \"" + comboCAT3Port.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkCAT3Enable.Checked = false;
                }
    ...
    private void chkCAT4Enable_CheckedChanged(
        ...
        if (comboCAT4Port.Text == "" || !comboCAT4Port.Text.StartsWith("COM")) {
            if (chkCAT4Enable.Focused) {
                if (chkCAT4Enable.Checked) {
                    MessageBox.Show("The CAT port \"" + comboCAT4Port.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkCAT4Enable.Checked = false;
                }
    ...
    private void ChkEnableAndromeda_CheckedChanged(object sender, EventArgs e) {
        ...
        if (comboAndromedaCATPort.Text == ""
            || !comboAndromedaCATPort.Text.StartsWith("COM")) {
            if (chkEnableAndromeda.Focused) {
                if (chkEnableAndromeda.Checked) {
                    MessageBox.Show("The CAT port \""
                        + comboAndromedaCATPort.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkEnableAndromeda.Checked = false;
                }
        ...
        if (chkEnableAndromeda.Checked && console.PTTBitBangEnabled
            ...
            MessageBox.Show(
                "Andromeda CAT port cannot be the same as Bit Bang Port",
                "Port Selection Error", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            chkEnableAndromeda.Checked = false;
        }
        ...
        if (chkEnableAndromeda.Checked) {
            try {
                ...
                chkEnableAndromeda.Checked = false;
                MessageBox.Show(
                    "Could not initialize Andromeda control.  Exception was:\n\n "
                        + ex.Message
                        + "\n\nAndromeda control has been disabled.",
                    "Error Initializing Andromeda control",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        } else {
            if (comboKeyerConnSecondary.Text == "CAT" && chkCATEnable.Focused) {
    ...
    private void doEnablementOnBitBangEnable() {
        if (comboCATPTTPort.Text != "None"
            && (chkCATPTT_RTS.Checked
                || chkCATPTT_DTR
                       .Checked)) // if RTS or DTR & port is not None, enable
        {
            chkCATPTTEnabled.Enabled = true;
        } else {
            chkCATPTTEnabled.Enabled = false;
    ...
    private void chkCATPTTEnabled_CheckedChanged(
        ...
        if (comboCATPTTPort.Text == ""
            || (!comboCATPTTPort.Text.StartsWith("COM")
                && !comboCATPTTPort.Text.StartsWith("CAT"))) {
            if (chkCATPTTEnabled.Focused && chkCATPTTEnabled.Checked) {
                MessageBox.Show("The PTT port \"" + comboCATPTTPort.Text
                    + "\" is not a valid port.  Please select another port.");
            }
            chkCATPTTEnabled.Checked = false;
            return;
        }
        ...
        if (comboCATPTTPort.Text.StartsWith("CAT"))
            console.PTTBitBangEnabled = false;
        else
            console.PTTBitBangEnabled = chkCATPTTEnabled.Checked;

        if (chkCATPTTEnabled.Checked) // if it's enabled don't allow changing
                                      // settings on port
        {
            if (comboCATPTTPort.Text.StartsWith("CAT"))
    ...
    private void comboCAT4Port_SelectedIndexChanged(
        ...
        if (comboCAT4Port.Text == "None") {
            ...
            chkCAT4Enable.Enabled = true;

        if (comboCAT4Port.Text.StartsWith("COM"))
            console.CAT4Port = Int32.Parse(comboCAT4Port.Text.Substring(3));
    }

    private void ComboAndromedaCATPort_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void comboCATRigType_SelectedIndexChanged(
        ...
        switch (comboCATRigType.Text) {
            ...
            case "TS-50S": console.CATRigType = 13; break;
            case "TS-440": console.CATRigType = 20; break;
            default: console.CATRigType = 19; break;
        }
    }
...
#region Test Tab Event Handlers
    ...
    public bool TTgenrun {
        ...
        set {
            ...
            if (!ttgenrun && chkTestIMD.Checked && console.MOX)
    ...
    private void chkTestIMD_CheckedChanged(object sender, System.EventArgs e) {
        if (chkTestIMD.Checked) {
            ...
            if (chkInvertTones.Checked
                && ((mode == DSPMode.CWL) || (mode == DSPMode.DIGL)
                    || (mode == DSPMode.LSB))) {
                ttfreq1 = -ttfreq1;
                ttfreq2 = -ttfreq2;
            }
        ...
        for (int i = 0; i < 2; i++) {
            switch (cmboSigGenRXMode.Text) {
                ...
                case "Tone":
                    ...
                    console.radio.GetDSPRX(i, 0).RXPreGenToneMag
                        = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                    console.radio.GetDSPRX(i, 0).RXPreGenToneFreq
                        = (double)udRXGenFreq.Value;
                    break;
                case "Noise":
                    console.radio.GetDSPRX(i, 0).RXPreGenMode = 2;
                    console.radio.GetDSPRX(i, 0).RXPreGenNoiseMag
                        = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                    break;
                case "Sweep":
                    console.radio.GetDSPRX(i, 0).RXPreGenMode = 3;
                    console.radio.GetDSPRX(i, 0).RXPreGenSweepMag
                        = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                    console.radio.GetDSPRX(i, 0).RXPreGenSweepFreq1
                        = (double)udRXGenSweepLow.Value;
                    console.radio.GetDSPRX(i, 0).RXPreGenSweepFreq2
                        = (double)udRXGenSweepHigh.Value;
                    console.radio.GetDSPRX(i, 0).RXPreGenSweepRate
                        = (double)udRXGenSweepRate.Value;
                    break;
                case "Silence":
    ...
    private void cmboSigGenTXMode_SelectedIndexChanged(
        ...
        switch (cmboSigGenTXMode.Text) {
            ...
            case "Tone":
                ...
                console.radio.GetDSPTX(0).TXPreGenToneMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenToneFreq
                    = (double)udTXGenFreq.Value;
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Noise":
                console.radio.GetDSPTX(0).TXPreGenMode = 2;
                console.radio.GetDSPTX(0).TXPreGenNoiseMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Sweep":
                console.radio.GetDSPTX(0).TXPreGenMode = 3;
                console.radio.GetDSPTX(0).TXPreGenSweepMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenSweepFreq1
                    = (double)udTXGenSweepLow.Value;
                console.radio.GetDSPTX(0).TXPreGenSweepFreq2
                    = (double)udTXGenSweepHigh.Value;
                console.radio.GetDSPTX(0).TXPreGenSweepRate
                    = (double)udTXGenSweepRate.Value;
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Sawtooth":
                console.radio.GetDSPTX(0).TXPreGenMode = 4;
                console.radio.GetDSPTX(0).TXPreGenSawtoothMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenSawtoothFreq
                    = (double)udTXGenFreq.Value;
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Triangle":
                console.radio.GetDSPTX(0).TXPreGenMode = 5;
                console.radio.GetDSPTX(0).TXPreGenTriangleMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenTriangleFreq
                    = (double)udTXGenFreq.Value;
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Pulse":
                console.radio.GetDSPTX(0).TXPreGenMode = 6;
                console.radio.GetDSPTX(0).TXPreGenPulseMag
                    = Math.Pow(10, (double)udTXGenScale.Value / 20.0);
                console.radio.GetDSPTX(0).TXPreGenPulseToneFreq
                    = (double)udTXGenFreq.Value;
                console.radio.GetDSPTX(0).TXPreGenPulseFreq
                    = (double)udTXGenPulseFreq.Value;
                console.radio.GetDSPTX(0).TXPreGenPulseDutyCycle
                    = (double)udTXGenPulseDutyCycle.Value;
                console.radio.GetDSPTX(0).TXPreGenPulseTransition
                    = (double)udTXGenPulseTransition.Value;
                console.radio.GetDSPTX(0).TXPreGenRun = 1;
                break;
            case "Silence":
...
#region Other Event Handlers
    ...
    private void setButtonState(bool bSaving, bool bLoading) {
        if (m_bIgnoreButtonState)
            return; // not used after this is true, which is set in
                    // console_closing
    ...
    private bool CompleteImport() {
        ...
        //-W2PA Import more carefully, allowing DBs created by previous versions
        // to retain settings and
        // options
        if (DB.ImportAndMergeDatabase(
            ...
            MessageBox.Show(
                "Database could not be imported. Previous database has been kept.");
            success = false;
        }
        ...
        if (success) {
            ...
            string justFileName = console.DBFileName.Substring(
                console.DBFileName.LastIndexOf("\\") + 1);
            string datetime = DateTime.Now.ToShortDateString().Replace("/", "-")
                + "_" + DateTime.Now.ToShortTimeString().Replace(":", ".");
            File.Copy(console.DBFileName,
                archivePath + "Thetis_database_" + datetime + ".xml");
    ...
    private void txtKB_KeyDown(
        ...
        if (e.KeyCode == Keys.Apps) menu_key = true;
        ...
        if (windows_key) s += "Win+";
        if (menu_key) s += "Menu+";

        if (e.KeyCode != Keys.ShiftKey && e.KeyCode != Keys.ControlKey
            && e.KeyCode != Keys.Menu && e.KeyCode != Keys.RMenu
    ...
    private void txtKB_KeyUp(
        ...
        if (e.KeyCode == Keys.Apps) menu_key = false;

        TextBoxTS txtbox = (TextBoxTS)sender;

        if (txtbox.Text.EndsWith("+")) {
            if (shift_key || ctrl_key || alt_key || windows_key || menu_key) {
                string s = "";
...
#region Lost Focus Event Handlers
    ...
    private void udOptClickTuneOffsetDIGL_LostFocus(
        object sender, EventArgs e) {
        //      udOptClickTuneOffsetDIGL.Value = udOptClickTuneOffsetDIGL.Value;
    }

    private void udOptClickTuneOffsetDIGU_LostFocus(
        object sender, EventArgs e) {
    ...
    private void udDisplayMultiTextHoldTime_LostFocus(
        object sender, EventArgs e) {
        udDisplayMultiTextHoldTime.Value = udDisplayMultiTextHoldTime.Value;
    }

    private void udDisplayMultiPeakHoldTime_LostFocus(
        object sender, EventArgs e) {
        udDisplayMultiPeakHoldTime.Value = udDisplayMultiPeakHoldTime.Value;
    }

    private void udDisplayWaterfallLowLevel_LostFocus(
        object sender, EventArgs e) {
        udDisplayWaterfallLowLevel.Value = udDisplayWaterfallLowLevel.Value;
    }

    private void udDisplayWaterfallHighLevel_LostFocus(
        object sender, EventArgs e) {
    ...
    private void comboMeterType_SelectedIndexChanged(
        ...
        switch (comboMeterType.Text) {
            case "Original":
                console.CurrentMeterDisplayMode
    ...
    private void comboColorPalette_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void comboRX2ColorPalette_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void setWaterFallCalculatedDelayText() {
        lblRX1WaterFallCalulatedDelayMS.Text
            = ((1000f / console.DisplayFPS) * Display.WaterfallUpdatePeriod)
                  .ToString("f2")
            + " (ms)";
        lblRX2WaterFallCalulatedDelayMS.Text
            = ((1000f / console.DisplayFPS) * Display.RX2WaterfallUpdatePeriod)
                  .ToString("f2")
            + " (ms)";
    }
    ...
    private void udRX2DisplayWaterfallAvgTime_ValueChanged(
        ...
        // buffer_time); buffersToAvg = Math.Max(2, buffersToAvg);
        ...
        set {
            switch (value) {
                ...
                case MeterTXMode.SWR_POWER: // MW0LGE re-added
                    ...
                    break;
                case MeterTXMode.SWR: comboTXTUNMeter.Text = "SWR"; break;
                case MeterTXMode.OFF: comboTXTUNMeter.Text = "Off"; break;
            }
        }
    }
    ...
    private void comboTXTUNMeter_SelectedIndexChanged(
        ...
        switch (comboTXTUNMeter.Text) {
            ...
            case "Fwd SWR": // MW0LGE re-added
                ...
                break;
            case "SWR": console.TuneTXMeterMode = MeterTXMode.SWR; break;
            case "Off": console.TuneTXMeterMode = MeterTXMode.OFF; break;
        }
    }
    ...
    private void btnTXProfileDefImport_Click(
        ...
        DialogResult result = MessageBox.Show(
            "Include this Additional TX profile in your profiles list?",
            "Include?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

        if (result == DialogResult.No) return;

        string name = lstTXProfileDef.Text;
        DataRow[] rows
            = DB.ds.Tables["TxProfileDef"].Select("'" + name + "' = Name");
        ...
        DataRow dr = null;
        if (comboTXProfileName.Items.Contains(name)) {
            result = MessageBox.Show("Are you sure you want to overwrite the "
                    + name + " TX Profile?",
                "Overwrite Profile?", MessageBoxButtons.YesNo,
                MessageBoxIcon.Question);
    ...
    private void ExportCurrentTxProfile() {
        ...
        if (rows.Length > 0) {
            ...
            MessageBox.Show("Can not locate " + current_profile
        ...
        try {
            ...
            MessageBox.Show("Can not write " + fileName + ".", "Export error",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        MessageBox.Show(
            "Profile" + current_profile + " has been saved in file " + fileName,
        ...
        if (e.Control == true && e.Alt == true) {
            switch (e.KeyCode) {
                case Keys.A:
                    ...
                    break;
                case Keys.O: break;
                case Keys.P: chkAutoPACalibrate.Visible = true; break;
                case Keys.V: grpDisplayDriverEngine.Visible = true; break;
            }
        }
    }
    ...
    private void comboAppSkin_SelectedIndexChanged(object sender, EventArgs e) {
        if (initializing)
            return; // prevents call on getoptions, need to force call after
                    // options loaded MW0LGE
    ...
    private void tbRX1FilterAlpha_Scroll(object sender, EventArgs e) {
        ...
        toolTip1.SetToolTip(
            tbRX1FilterAlpha, tbRX1FilterAlpha.Value.ToString());
    }
    ...
    private void tbMultiRXFilterAlpha_Scroll(object sender, EventArgs e) {
        ...
        toolTip1.SetToolTip(
            tbMultiRXFilterAlpha, tbMultiRXFilterAlpha.Value.ToString());
    }

    public static Color DisplayGrayLineColor
        = Color.FromArgb(70, Color.Black); // ke9ns add default value
    public void clrbtnGrayLine_Changed(object sender, EventArgs e) {
        DisplayGrayLineColor
            = Color.FromArgb(tbGrayLineAlpha.Value, clrbtnGrayLine.Color);
    }
    ...
    private void btnExportDB_Click(object sender, EventArgs e) {
        ...
        saveFileDialog1.FileName
            = path + "\\Thetis_database_export_" + datetime + ".xml";
    ...
    private void checkHPSDRDefaults(object sender, System.EventArgs e) {
        ...
        // !chkPennyLane.Checked)
        //{
        //    radMercury12288MHz.Checked = true;
        //    radMercury12288MHz_CheckedChanged(sender, e);
        //}

        // if (chkMercuryPresent.Checked && !radMercury10MHz.Checked &&
        // !radAtlas10MHz.Checked &&
    ...
    private void chkAlexPresent_CheckedChanged(
        ...
        //        !radGenModelANAN100D.Checked) console.comboRX2Preamp.Visible =
        //        false; if (chkApolloPresent.Checked) chkApolloPresent.Checked
        //        = false;
        ...
        //        radGenModelANAN200D.Checked) NetworkIO.SetHermesFilter(0);
        //    }
        //    else
        //    {
        ...
        //        //  if (console.RX2PreampPresent)
        //        console.comboRX2Preamp.Visible = true;
        //    }
        //    // if (radGenModelANAN100D.Checked) console.comboRX2Preamp.Visible
        //    = true; console.AlexPresent = chkAlexPresent.Checked;
    ...
    private void chkPenOCrcv160_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(Band.B160M, (byte)(val & BandBBitMask),
            false); // 000 xxxx 0x70  000 0xxx 0x78
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
    ...
    private void chkPenOCxmit160_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B160M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv80_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B80M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit80_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B80M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv60_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B60M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit60_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B60M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv40_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B40M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit40_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B40M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv30_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B30M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit30_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B30M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv20_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B20M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit20_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B20M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv17_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B17M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit17_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B17M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv15_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B15M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit15_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B15M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv12_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B12M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit12_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B12M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv10_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B10M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit10_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B10M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv6_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B6M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit6_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B6M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCrcv2_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B2M, (byte)(val & BandBBitMask), false); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void chkPenOCxmit2_CheckedChanged(
        ...
        Penny.getPenny().setBandBBitMask(
            Band.B2M, (byte)(val & BandBBitMask), true); // 0000xxx
        console.PennyExtCtrlEnabled
            = chkPennyExtCtrl.Checked; // need side effect of this to push
                                       // change to native code
    }
    ...
    private void InitAlexAntTables() {
        ...
        AlexRxOnlyAntCheckBoxes = new[] { new CheckBoxTS[] { chkAlex160R1,
                                             chkAlex160R2, chkAlex160XV },
        ...
        AlexRxAntButtons = new[] { new RadioButtonTS[] { radAlexR1_160,
                                      radAlexR2_160, radAlexR3_160 },
        ...
        AlexTxAntButtons = new[] { new RadioButtonTS[] { radAlexT1_160,
                                      radAlexT2_160, radAlexT3_160 },
    ...
    private void ProcessAlexAntCheckBox(object sender, Band band) {
        ...
        // find a name for the "winning" check box. In some radios the signals
        // add, so multiple selections would be bad.
        if (cboxes[2].Checked)
            Name = RXAntChk3Name;
        ...
        console.AlexAntCtrlEnabled
            = true; // need side effect of prop set to push data down to C code

        return;
    }

    private void ProcessAlexAntRadioButton(
        object sender, Band band, bool is_xmit) {
        if (sender == null) return;
        ...
        RadioButtonTS[] buttons
            = is_xmit ? AlexTxAntButtons[idx] : AlexRxAntButtons[idx];
        ...
        if (is_xmit) {
            ...
            console.SetAriesTXAntenna(
                ant, band); // temporaily disable needs more work
        } else {
            Alex.getAlex().setRxAnt(band, (byte)ant);
            pi_RxAnt = ant; // Path_Illustrator support
            console.SetAriesRXAntenna(
                ant, band); // temporaily disable needs more work
        }
    ...
    public void SetTXAntenna(int Antenna, Band band) {
        if ((band >= Band.B160M) && (band <= Band.B6M)) {
            ...
            if (Antenna == 2) {
                if (chkBlockTxAnt2.Checked) TXAllowed = false;
            } else if (Antenna == 3) {
                if (chkBlockTxAnt3.Checked) TXAllowed = false;
            }
    ...
    private void tpHPSDR_Paint(
        ...
        if (console.PowerOn && console.CurrentHPSDRModel == HPSDRModel.HPSDR) {
            ...
            //{
            //    lblOzyFX2.Text =
            //    Convert.ToUInt32(JanusAudio.getFX2FirmwareVersionString()).ToString("Ozy
            //    FX2: 0000 00 00");
            //}
            // catch (Exception)
            //{
            ...
            // {
            // lblOzyFWVer.Text = JanusAudio.getOzyFWVersion().ToString("Metis:
            // 0\\.0");
            lblOzyFX2.Text = "";
            // }

            // lblMercuryFWVer.Text = console.MercuryPresent ?
            // JanusAudio.getMercuryFWVersion().ToString("Mercury: 0\\.0") :
            // "Mercury: n/a"; if (lblMercury2FWVer.Visible)
            // lblMercury2FWVer.Text = console.MercuryPresent ?
            // JanusAudio.getMercury2FWVersion().ToString("Mercury2: 0\\.0") :
            // "Mercury2: n/a"; if (console.PennyPresent ||
            // console.PennyLanePresent) lblPenelopeFWVer.Text =
            // JanusAudio.getPenelopeFWVersion().ToString("Penny[Lane]: 0\\.0");
        ...
        //    lblOzyFX2.Text = NetworkIO.FWCodeVersion.ToString("Hermes:
        //    0\\.0");//ver_bytes[0].ToString("Hermes: 0\\.0"); lblOzyFWVer.Text
        //    = ""; lblMercuryFWVer.Text = ""; lblMercury2FWVer.Text = "";
        //    lblPenelopeFWVer.Text = "";
        //}

        // if (console.PowerOn && (radGenModelANAN10.Checked ||
        // radGenModelANAN10E.Checked || radGenModelANAN100.Checked ||
        // radGenModelANAN100D.Checked))
        //{
        //    //byte[] ver_bytes = new byte[1];
        //    //JanusAudio.GetMetisCodeVersion(ver_bytes);
        //    lblOzyFX2.Text = NetworkIO.FWCodeVersion.ToString("ANAN: 0\\.0");
        //    //ver_bytes[0].ToString("ANAN: 0\\.0"); lblOzyFWVer.Text = "";
        //    lblMercuryFWVer.Text = ""; lblMercury2FWVer.Text = "";
        //    lblPenelopeFWVer.Text = "";
        //}
        ...
        //    //byte[] ver_bytes = new byte[1];
        //    //JanusAudio.GetMetisCodeVersion(ver_bytes);
        //    lblOzyFX2.Text = NetworkIO.FWCodeVersion.ToString("Orion: 0\\.0");
        //    //ver_bytes[0].ToString("Orion: 0\\.0"); lblOzyFWVer.Text = "";
        //    lblMercuryFWVer.Text = ""; lblMercury2FWVer.Text = "";
        //    lblPenelopeFWVer.Text = "";
        //}
    }
    ...
    private void tpGeneralHardware_Paint(
        ...
        // lblMetisMAC.Text = BitConverter.ToString(mac_bytes).Replace("-",
        // ":").ToLower();
    ...
    private void comboFRSRegion_SelectedIndexChanged(
        object sender, EventArgs e) // w5wc
    {
        if (comboFRSRegion.Text == "") return;

        FRSRegion region = console.CurrentRegion;
        switch (comboFRSRegion.Text) {
            case "Australia": region = FRSRegion.Australia; break;
            case "Europe": region = FRSRegion.Europe; break;
            case "India": region = FRSRegion.India; break;
            case "Italy": region = FRSRegion.Italy_Plus; break;
            case "Spain": region = FRSRegion.Spain; break;
            case "Japan": region = FRSRegion.Japan; break;
            case "United Kingdom": region = FRSRegion.UK; break;
            case "United States": region = FRSRegion.US; break;
            case "Norway": region = FRSRegion.Norway; break;
            case "Denmark": region = FRSRegion.Denmark; break;
            case "Latvia": region = FRSRegion.Latvia; break;
            case "Slovakia": region = FRSRegion.Slovakia; break;
            case "Bulgaria": region = FRSRegion.Bulgaria; break;
            case "Greece": region = FRSRegion.Greece; break;
            case "Hungary": region = FRSRegion.Hungary; break;
            case "Netherlands": region = FRSRegion.Netherlands; break;
            case "France": region = FRSRegion.France; break;
            case "Israel": region = FRSRegion.Israel; break;
            case "Russia": region = FRSRegion.Russia; break;
            case "Sweden": region = FRSRegion.Sweden; break;
            case "Region1": region = FRSRegion.Region1; break;
            case "Region2": region = FRSRegion.Region2; break;
            case "Region3": region = FRSRegion.Region3; break;
            case "Germany": region = FRSRegion.Germany; break;
        }
        if (console.CurrentRegion != region) console.CurrentRegion = region;

        if (!console.initializing) {
            setButtonState(
                true, false); // MW0LGE this causes an update of the whole DB,
                              // so disable buttons while happening
    ...
    private void tbTXVGridFineAlpha_Scroll(object sender, EventArgs e) {
        ...
        toolTip1.SetToolTip(
            tbTXVGridFineAlpha, tbTXVGridFineAlpha.Value.ToString());
    }

    private void clrbtnHGridColor_Changed(object sender, EventArgs e) {
        Display.HGridColor
            = Color.FromArgb(tbHGridColorAlpha.Value, clrbtnHGridColor.Color);
    }

    private void clrbtnTXHGridColor_Changed(object sender, EventArgs e) {
        Display.TXHGridColor = Color.FromArgb(
            tbTXHGridColorAlpha.Value, clrbtnTXHGridColor.Color);
    }

    private void tbHGridColorAlpha_Scroll(object sender, EventArgs e) {
        clrbtnHGridColor_Changed(this, EventArgs.Empty);
        toolTip1.SetToolTip(
            tbHGridColorAlpha, tbHGridColorAlpha.Value.ToString());
    }
    ...
    private void chkPenOCrcvVHF0_CheckedChanged(object sender, EventArgs e) {
        ...
        Penny.getPenny().setBandBBitMask(
            Band.VHF0, (byte)(val & 0x70), false); // 0000xxx
    ...
    private void chkOCrcvLMW_CheckedChanged(object sender, EventArgs e) {
        ...
        Penny.getPenny().setBandBBitMask(
            Band.BLMF, (byte)(val & 0x70), false); // 0000xxx
    ...
    private void chkDisable6mLNAonTX_CheckedChanged(
        object sender, EventArgs e) {
        if (initializing) return;
        console.Disable6mLNAonTX = chkDisable6mLNAonTX.Checked;
    }

    private void chkDisable6mLNAonRX_CheckedChanged(
        object sender, EventArgs e) {
        if (initializing) return;
    ...
    public bool AlexHPFBypass {
        get {
            if (chkAlexHPFBypass != null)
                return chkAlexHPFBypass.Checked;
            else
                return false;
        }
        set {
    ...
    public bool Alex2HPFBypass {
        get {
            if (chkAlex2HPFBypass != null)
                return chkAlex2HPFBypass.Checked;
            else
                return false;
        }
        set {
    ...
    private void chkAGCDisplayHangLine_CheckedChanged(
        object sender, EventArgs e) {
        console.DisplayAGCHangLine = chkAGCDisplayHangLine.Checked;
    }

    private void chkAGCHangSpectrumLine_CheckedChanged(
        object sender, EventArgs e) {
        Display.RX1HangSpectrumLine = chkAGCHangSpectrumLine.Checked;
    }

    private void chkDisplayRX2GainLine_CheckedChanged(
        object sender, EventArgs e) {
        console.DisplayRX2GainLine = chkDisplayRX2GainLine.Checked;
    }

    private void chkRX2GainSpectrumLine_CheckedChanged(
        object sender, EventArgs e) {
        Display.RX2GainSpectrumLine = chkRX2GainSpectrumLine.Checked;
    }

    private void chkDisplayRX2HangLine_CheckedChanged(
        object sender, EventArgs e) {
        console.DisplayRX2HangLine = chkDisplayRX2HangLine.Checked;
    }

    private void chkRX2HangSpectrumLine_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void chkStrictCharSpacing_CheckedChanged(
        object sender, EventArgs e) {
        NetworkIO.EnableCWKeyerSpacing(
            Convert.ToInt32(chkStrictCharSpacing.Checked));
    }
    ...
    private void chkApolloFilter_CheckedChanged(object sender, EventArgs e) {
        if (chkApolloFilter.Checked)
            NetworkIO.EnableApolloFilter(1);
        else
            NetworkIO.EnableApolloFilter(0);
    }
    ...
    private void udHermesStepAttenuatorData_ValueChanged(
        object sender, EventArgs e) {
    ...
    private void udAlex160mLPFStart_ValueChanged(object sender, EventArgs e) {
        if (udAlex160mLPFStart.Value
            >= udAlex160mLPFEnd.Value + (decimal)0.000001) {
            udAlex160mLPFEnd.Value
                = udAlex160mLPFStart.Value + (decimal)0.000001;
        }
    }

    private void udAlex160mLPFEnd_ValueChanged(object sender, EventArgs e) {
        if (udAlex160mLPFEnd.Value <= udAlex160mLPFStart.Value) {
            udAlex160mLPFStart.Value
                = udAlex160mLPFEnd.Value - (decimal)0.000001;
        } else if (udAlex160mLPFEnd.Value >= udAlex80mLPFStart.Value) {
            udAlex80mLPFStart.Value
                = udAlex160mLPFEnd.Value + (decimal)0.000001;
        }
    }
    ...
    private void udAlex80mLPFStart_ValueChanged(object sender, EventArgs e) {
        if (udAlex80mLPFStart.Value <= udAlex160mLPFEnd.Value) {
            udAlex160mLPFEnd.Value
                = udAlex80mLPFStart.Value - (decimal)0.000001;
        }
    }
    ...
    private void chkSWRTuneProtection_CheckedChanged(
        object sender, EventArgs e) {
        console.DisableSWRonTune = chkSWRTuneProtection.Checked;
    }

    private void tbDisplayFFTSize_Scroll(object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).FFTSize = (int)(4096
            * Math.Pow(2, Math.Floor((double)(tbDisplayFFTSize.Value))));
        // console.specRX.GetSpecRX(2).FFTSize = (int)(4096 * Math.Pow(2,
        // Math.Floor((double)(tbDisplayFFTSize.Value))));
        //  console.specRX.GetSpecRX(1).FFTSize = (int)(4096 * Math.Pow(2,
        //  Math.Floor((double)(tbDisplayFFTSize.Value))));
        console.UpdateRXSpectrumDisplayVars();
        double bin_width = (double)Display.SampleRateRX1
            / (double)console.specRX.GetSpecRX(0).FFTSize;
        lblDisplayBinWidth.Text = bin_width.ToString("N3");
    ...
    private void tbRX2DisplayFFTSize_Scroll(object sender, EventArgs e) {
        // console.specRX.GetSpecRX(0).FFTSize = (int)(4096 * Math.Pow(2,
        // Math.Floor((double)(tbDisplayFFTSize.Value))));
        console.specRX.GetSpecRX(1).FFTSize = (int)(4096
            * Math.Pow(2, Math.Floor((double)(tbRX2DisplayFFTSize.Value))));
        double bin_width = (double)Display.SampleRateRX2
            / (double)console.specRX.GetSpecRX(1).FFTSize;
        lblRX2DisplayBinWidth.Text = bin_width.ToString("N3");
        // Display.RX1FFTSizeOffset = tbDisplayFFTSize.Value * 2;
    ...
    private void comboDispWinType_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).WindowType = comboDispWinType.SelectedIndex;
        console.UpdateRXSpectrumDisplayVars();
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).WindowType
            = comboDispWinType.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void comboRX2DispWinType_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).WindowType
            = comboRX2DispWinType.SelectedIndex;
    }

    private void udDSPNBTransition_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).NBTau
            = 0.001 * (double)udDSPNBTransition.Value;
        console.radio.GetDSPRX(1, 0).NBTau
            = 0.001 * (double)udDSPNBTransition.Value;
    }

    private void udDSPNBLead_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).NBAdvTime
            = 0.001 * (double)udDSPNBLead.Value;
        console.radio.GetDSPRX(1, 0).NBAdvTime
            = 0.001 * (double)udDSPNBLead.Value;
    }

    private void udDSPNBLag_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).NBHangTime
            = 0.001 * (double)udDSPNBLag.Value;
        console.radio.GetDSPRX(1, 0).NBHangTime
            = 0.001 * (double)udDSPNBLag.Value;
    }
    ...
    private void btnConfigure_Click(object sender, EventArgs e) {
        if (ConfigMidi2Cat == null) {
            ...
            ConfigMidi2Cat = new Midi2Cat.Midi2CatSetupForm(
                console.Midi2Cat.Midi2CatDbFile);
            ConfigMidi2Cat.Show();
            ConfigMidi2Cat.Focus();
            ConfigMidi2Cat.FormClosed
                += new FormClosedEventHandler(ConfigMidi2CatSetupClosed);
        }
    }

    private void ConfigMidi2CatSetupClosed(
    ...
    private void btnSetIPAddr_Click(object sender, EventArgs e) {
        if (radStaticIP1.Checked) {
            console.HPSDRNetworkIPAddr = udStaticIP1.Text + "."
                + udStaticIP2.Text + "." + udStaticIP3.Text + "."
                + udStaticIP4.Text;
        }
        if (radStaticIP2.Checked) {
            console.HPSDRNetworkIPAddr = udStaticIP5.Text + "."
                + udStaticIP6.Text + "." + udStaticIP7.Text + "."
                + udStaticIP8.Text;
        }
        if (radStaticIP3.Checked) {
            console.HPSDRNetworkIPAddr = udStaticIP9.Text + "."
                + udStaticIP10.Text + "." + udStaticIP11.Text + "."
                + udStaticIP12.Text;
        }
        if (radStaticIP4.Checked) {
            console.HPSDRNetworkIPAddr = udStaticIP13.Text + "."
                + udStaticIP14.Text + "." + udStaticIP15.Text + "."
                + udStaticIP16.Text;
        }
    }
    ...
    private void comboDSPRxWindow_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void comboDSPTxWindow_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void udRXGenScale_ValueChanged(object sender, EventArgs e) {
        switch (cmboSigGenRXMode.Text) {
            case "Tone":
                console.radio.GetDSPRX(0, 0).RXPreGenToneMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                console.radio.GetDSPRX(1, 0).RXPreGenToneMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                break;
            case "Noise":
                console.radio.GetDSPRX(0, 0).RXPreGenNoiseMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                console.radio.GetDSPRX(1, 0).RXPreGenNoiseMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                break;
            case "Sweep":
                console.radio.GetDSPRX(0, 0).RXPreGenSweepMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                console.radio.GetDSPRX(1, 0).RXPreGenSweepMag
                    = Math.Pow(10, (double)udRXGenScale.Value / 20.0);
                break;
        }
    }
    ...
    private void udRXGenFreq_ValueChanged(object sender, EventArgs e) {
        switch (cmboSigGenRXMode.Text) {
            case "Tone":
                console.radio.GetDSPRX(0, 0).RXPreGenToneFreq
                    = (double)udRXGenFreq.Value;
                console.radio.GetDSPRX(1, 0).RXPreGenToneFreq
                    = (double)udRXGenFreq.Value;
                break;
        }
    }
    ...
    private void udRXGenSweepLow_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXPreGenSweepFreq1
            = (double)udRXGenSweepLow.Value;
        console.radio.GetDSPRX(1, 0).RXPreGenSweepFreq1
            = (double)udRXGenSweepLow.Value;
    }

    private void udRXGenSweepHigh_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXPreGenSweepFreq2
            = (double)udRXGenSweepHigh.Value;
        console.radio.GetDSPRX(1, 0).RXPreGenSweepFreq2
            = (double)udRXGenSweepHigh.Value;
    }

    private void udRXGenSweepRate_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXPreGenSweepRate
            = (double)udRXGenSweepRate.Value;
        console.radio.GetDSPRX(1, 0).RXPreGenSweepRate
            = (double)udRXGenSweepRate.Value;
    }
    ...
    private void udTXGenPulseDutyCycle_ValueChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPTX(0).TXPreGenPulseDutyCycle
            = (double)udTXGenPulseDutyCycle.Value;
    }

    private void udTXGenPulseTransition_ValueChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPTX(0).TXPreGenPulseTransition
            = (double)udTXGenPulseTransition.Value;
    }
    ...
    private void udDSPEERmdelay_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPTX(0).TXEERModeMdelay
            = (double)udDSPEERmdelay.Value / 1.0e+06;
    }
    ...
    private void radDDCADC_CheckedChanged(object sender, EventArgs e) {
        ...
        if (radDDC0ADC0.Checked)
            val += 0; // bits 1 & 0 set to 00 => DDC0 to ADC0
        if (radDDC0ADC1.Checked)
        ...
        if (radDDC4ADC0.Checked)
            val += 0; // bits 1 & 0 set to 00 => DDC4 to ADC0
        if (radDDC4ADC1.Checked)
        ...
        if (radDDC6ADC1.Checked)
            val += 1 << 4; // bits 5 & 4 set to 01 => DDC6 to ADC1
        if (radDDC6ADC2.Checked)
            val += 1 << 5; // bits 5 & 4 set to 10 => DDC6 to ADC2
    ...
    private void radP1DDCADC_CheckedChanged(object sender, EventArgs e) {
        ...
        if (radP1DDC0ADC0.Checked)
            val += 0; // bits 1 & 0 set to 00 => DDC0 to ADC0
        if (radP1DDC0ADC1.Checked)
            val += 1 << 0; // bits 1 & 0 set to 01 => DDC0 to ADC1
        ...
        if (radP1DDC6ADC1.Checked)
            val += 1 << 12; // bits 13 & 12 set to 01 => DDC6 to ADC1
        if (radP1DDC6ADC2.Checked)
            val += 1 << 13; // bits 13 & 12 set to 10 => DDC6 to ADC2
    ...
    private void comboDSPNOBmode_SelectedIndexChanged(
        object sender, EventArgs e) {
    ...
    private void chkDSPRX1APFEnable_CheckedChanged(object sender, EventArgs e) {
        ...
        if (chkDSPRX1APFEnable.Checked) {
            if (chkDSPRX1subAPFEnable.Checked)
                console.RX1APFlabel = "APFb";
            else
                console.RX1APFlabel = "APFm";
        } else if (chkDSPRX1subAPFEnable.Checked)
            console.RX1APFlabel = "APFs";
        else
            console.RX1APFlabel = "";
    }

    private void chkDSPRX1subAPFEnable_CheckedChanged(
        object sender, EventArgs e) {
        if (console.RX1DSPMode == DSPMode.CWL
            || console.RX1DSPMode == DSPMode.CWU)
            console.radio.GetDSPRX(0, 1).RXAPFRun
                = chkDSPRX1subAPFEnable.Checked;
        ...
        if (chkDSPRX1subAPFEnable.Checked) {
            if (chkDSPRX1APFEnable.Checked)
                ...
                console.RX1APFlabel = "APFs";
        } else if (chkDSPRX1APFEnable.Checked)
            console.RX1APFlabel = "APFm";
        else
            console.RX1APFlabel = "";
    }
    ...
    private void chkDSPRX2APFEnable_CheckedChanged(object sender, EventArgs e) {
        ...
        if (chkDSPRX2APFEnable.Checked)
            console.RX2APFlabel = "APF";
        else
            console.RX2APFlabel = "";
    }

    private void tbDSPAudRX1APFGain_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXAPFGain
            = Math.Pow(10.0, (double)tbDSPAudRX1APFGain.Value / 200.0);
        if (radDSPRX1APFControls.Checked)
            console.APFGain = tbDSPAudRX1APFGain.Value;
    }

    private void tbDSPAudRX1subAPFGain_ValueChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 1).RXAPFGain
            = Math.Pow(10.0, (double)tbDSPAudRX1subAPFGain.Value / 200.0);
        if (radDSPRX1subAPFControls.Checked)
            console.APFGain = tbDSPAudRX1subAPFGain.Value;
    }

    private void tbDSPAudRX2APFGain_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXAPFGain
            = Math.Pow(10.0, (double)tbDSPAudRX2APFGain.Value / 200.0);
        if (radDSPRX2APFControls.Checked)
            console.APFGain = tbDSPAudRX2APFGain.Value;
    }

    private void tbRX1APFTune_Scroll(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXAPFFreq
            = console.CWPitch + (double)tbRX1APFTune.Value;
        if (radDSPRX1APFControls.Checked) console.APFFreq = tbRX1APFTune.Value;
    }

    private void tbRX1subAPFTune_Scroll(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 1).RXAPFFreq
            = console.CWPitch + (double)tbRX1subAPFTune.Value;
        if (radDSPRX1subAPFControls.Checked)
            console.APFFreq = tbRX1subAPFTune.Value;
    }

    private void tbRX2APFTune_Scroll(object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXAPFFreq
            = console.CWPitch + (double)tbRX2APFTune.Value;
        if (radDSPRX2APFControls.Checked) console.APFFreq = tbRX2APFTune.Value;
    }
    ...
    private void radDSPRX1APFControls_CheckedChanged(
        object sender, EventArgs e) {
        if (radDSPRX1APFControls.Checked) {
            chkDSPRX1APFEnable.Checked = console.APFEnabled;
            if (console.RX1DSPMode == DSPMode.CWL
                || console.RX1DSPMode == DSPMode.CWU)
                console.radio.GetDSPRX(0, 0).RXAPFRun
                    = chkDSPRX1APFEnable.Checked;
    ...
    private void radDSPRX1subAPFControls_CheckedChanged(
        object sender, EventArgs e) {
        if (radDSPRX1subAPFControls.Checked) {
    ...
    private void radDSPRX2APFControls_CheckedChanged(
        object sender, EventArgs e) {
        if (radDSPRX2APFControls.Checked) {
            chkDSPRX2APFEnable.Checked = console.APFEnabled;
            if (console.RX2DSPMode == DSPMode.CWL
                || console.RX2DSPMode == DSPMode.CWU)
                console.radio.GetDSPRX(1, 0).RXAPFRun
                    = chkDSPRX2APFEnable.Checked;
    ...
    private void chkDSPRX1DollyEnable_CheckedChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXADollyRun = chkDSPRX1DollyEnable.Checked;
    }

    private void chkDSPRX1DollySubEnable_CheckedChanged(
        object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 1).RXADollyRun
            = chkDSPRX1SubDollyEnable.Checked;
    }

    private void chkDSPRX2DollyEnable_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void udDSPRX1DollyF0_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXADollyFreq0
            = (double)udDSPRX1DollyF0.Value;
    }

    private void udDSPRX1SubDollyF0_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 1).RXADollyFreq0
            = (double)udDSPRX1SubDollyF0.Value;
    }

    private void udDSPRX2DollyF0_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXADollyFreq0
            = (double)udDSPRX2DollyF0.Value;
    }

    private void udDSPRX1DollyF1_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXADollyFreq1
            = (double)udDSPRX1DollyF1.Value;
    }

    private void udDSPRX2DollyF1_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(1, 0).RXADollyFreq1
            = (double)udDSPRX2DollyF1.Value;
    }

    private void udDSPRX1SubDollyF1_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 1).RXADollyFreq1
            = (double)udDSPRX1SubDollyF1.Value;
    }
    ...
    private void udDSPEERpdelay_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPTX(0).TXEERModePdelay
            = (double)udDSPEERpdelay.Value / 1.0e+06;
    }

    private void chkDSPEERRunDelays_CheckedChanged(object sender, EventArgs e) {
        console.radio.GetDSPTX(0).TXEERModeRunDelays
            = chkDSPEERRunDelays.Checked;
    }
    ...
    private void comboKeyerConnSecondary_MouseClick(
    ...
    private void udRXAMSQMaxTail_ValueChanged(object sender, EventArgs e) {
        console.radio.GetDSPRX(0, 0).RXAMSquelchMaxTail
            = (double)udRXAMSQMaxTail.Value;
        console.radio.GetDSPRX(0, 1).RXAMSquelchMaxTail
            = (double)udRXAMSQMaxTail.Value;
        console.radio.GetDSPRX(1, 0).RXAMSquelchMaxTail
            = (double)udRXAMSQMaxTail.Value;
        console.radio.GetDSPRX(1, 1).RXAMSquelchMaxTail
            = (double)udRXAMSQMaxTail.Value;
    }
    ...
    private void chkLimitExtAmpOnOverload_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void comboFocusMasterMode_SelectedIndexChanged(
        object sender, EventArgs e) {
        switch (comboFocusMasterMode.Text) {
            ...
            case "Enter Window Title":
                ...
                txtFocusMasterWinTitle.Enabled = true;
                txtFocusMasterWinTitle.Text
                    = "Enter Window Title and Press Enter";
...
#region MultiNotchFilter
    ...
    private void btnMNFAdd_Click(object sender, EventArgs e) {
        ...
        // the new notch will be inserted just after the previous one being
        // viewed zero/initialize the values in preparation for input
        udMNFFreq.Value = 0;
        udMNFWidth.Value = 0;
    ...
    private void btnMNFEnter_Click(object sender, EventArgs e) {
        if (AddActive) {
            ...
            WDSP.RXANBPAddNotch(WDSP.id(0, 0), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
            WDSP.RXANBPAddNotch(WDSP.id(0, 1), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
            WDSP.RXANBPAddNotch(WDSP.id(2, 0), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
        ...
        if (EditActive) {
            ...
            udMNFWidth.BackColor = SystemColors.Control;
            chkMNFActive.BackColor = SystemColors.Control;
            WDSP.RXANBPEditNotch(WDSP.id(0, 0), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
            WDSP.RXANBPEditNotch(WDSP.id(0, 1), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
            WDSP.RXANBPEditNotch(WDSP.id(2, 0), (int)udMNFNotch.Value,
                1.0e6 * (double)udMNFFreq.Value, (double)udMNFWidth.Value,
                chkMNFActive.Checked);
        }
        ...
        SaveNotchesToDatabase(); // aligns notch db with what has actually
                                 // happened MW0LGE
    }
    ...
    unsafe private void btnMNFCancel_Click(object sender, EventArgs e) {
        ...
        if (AddActive) {
            AddActive = false;
            if (numnotches > 0) {
                // Add had potentially increased these values ... put them back
                // the way they were
                udMNFNotch.Value -= 1;
                udMNFNotch.Maximum -= 1;
            }
        ...
        if (numnotches > 0) {
            ...
            int active;
            WDSP.RXANBPGetNotch(WDSP.id(0, 0), (int)udMNFNotch.Value, &fcenter,
                &fwidth, &active);
    ...
    private void btnMNFDelete_Click(object sender, EventArgs e) {
        ...
        // if the notch number still points to a valid notch, get the notch
        // information
        if ((int)udMNFNotch.Value < numnotches) {
            unsafe {
                double fcenter, fwidth;
                int active;
                WDSP.RXANBPGetNotch(WDSP.id(0, 0), (int)udMNFNotch.Value,
                    &fcenter, &fwidth, &active);
                udMNFFreq.Value = (decimal)(fcenter / 1.0e6);
                udMNFWidth.Value = (decimal)fwidth;
                if (active != 0)
                    chkMNFActive.Checked = true;
        ...
        // if the deleted notch was the 'top' notch, get the next notch moving
        // down
        else if (numnotches > 0) {
            unsafe {
                double fcenter, fwidth;
                int active;
                WDSP.RXANBPGetNotch(WDSP.id(0, 0), (int)udMNFNotch.Value - 1,
                    &fcenter, &fwidth, &active);
                udMNFNotch.Value -= 1;
                udMNFFreq.Value = (decimal)(fcenter / 1.0e6);
                udMNFWidth.Value = (decimal)fwidth;
    ...
    unsafe public void SaveNotchesToDatabase() {
        ...
        for (int i = 0; i < numnotches; i++) {
            ...
            // HERE:  SAVE fcenter, fwidth, and active FOR THIS NOTCH TO THE
            // DATABASE
            MNotchDB.List.Add(
                new MNotch(fcenter, fwidth, Convert.ToBoolean(active)));
        }
    }
    ...
    unsafe public void UpdateNotchDisplay() {
        ...
        if (numnotches > 0) {
            ...
            // MW0LGE added because udMNFFreq could be set to some value even
            // though no MNotchDB entries this caused by generic form save,
            // saving out the control value
            udMNFFreq.Value = 0;
            udMNFWidth.Value = 0;
            btnMNFDelete.Enabled = false;
            btnMNFEdit.Enabled = false;
    ...
    unsafe public void RestoreNotchesFromDatabase() {
        ...
        for (int i = 0; i < MNotchDB.List.Count; i++) {
            ...
            // HERE:  READ VALUES OF fcenter, fwidth, and active FOR NOTCH[i]
            // FROM THE DATABASE
        ...
        UpdateNotchDisplay(); // sets max limits, and selects first notch if one
                              // exists MW0LGE
    }
    ...
    private void btnVFOFreq_Click(object sender, EventArgs e) {
        ...
        if (console.RIT)
            vfo += (double)console.RITValue * 1e-6; // check for RIT
    ...
    private void comboRX2DispPanDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).DetTypePan
            = comboRX2DispPanDetector.SelectedIndex;
    }

    private void comboRX2DispPanAveraging_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).AverageMode
            = comboRX2DispPanAveraging.SelectedIndex;
    }

    private void comboRX2DispWFDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).DetTypeWF
            = comboRX2DispWFDetector.SelectedIndex;
    }

    private void comboRX2DispWFAveraging_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).AverageModeWF
            = comboRX2DispWFAveraging.SelectedIndex;
    }

    private void udRX2DisplayWFAVTime_ValueChanged(object sender, EventArgs e) {
        console.specRX.GetSpecRX(1).AvTauWF
            = 0.001 * (double)udRX2DisplayWFAVTime.Value;
    }

    private void comboDispPanDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).DetTypePan
            = comboDispPanDetector.SelectedIndex;
    }

    private void comboDispWFDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).DetTypeWF
            = comboDispWFDetector.SelectedIndex;
    }

    private void comboDispPanAveraging_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).AverageMode
    ...
    private void udDisplayAVTimeWF_ValueChanged(object sender, EventArgs e) {
        console.specRX.GetSpecRX(0).AvTauWF
            = 0.001 * (double)udDisplayAVTimeWF.Value;
    }

    private void chkDispRX2Normalize_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void comboTXDispPanDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).DetTypePan
            = comboTXDispPanDetector.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void comboTXDispPanAveraging_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).AverageMode
            = comboTXDispPanAveraging.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void udTXDisplayAVGTime_ValueChanged(object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).AvTau
            = 0.001 * (double)udTXDisplayAVGTime.Value;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void chkDispTXNormalize_CheckedChanged(object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).NormOneHzPan
            = chkDispTXNormalize.Checked;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void tbTXDisplayFFTSize_Scroll(object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).FFTSize = (int)(4096
            * Math.Pow(2, Math.Floor((double)(tbTXDisplayFFTSize.Value))));
        console.UpdateTXSpectrumDisplayVars();
        double bin_width
            = (double)console.specRX.GetSpecRX(cmaster.inid(1, 0)).SampleRate
            / (double)console.specRX.GetSpecRX(cmaster.inid(1, 0)).FFTSize;
        lblTXDispBinWidth.Text = bin_width.ToString("N3");
    }

    private void comboTXDispWinType_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).WindowType
            = comboTXDispWinType.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void comboTXDispWFDetector_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).DetTypeWF
            = comboTXDispWFDetector.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void comboTXDispWFAveraging_SelectedIndexChanged(
        object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).AverageModeWF
            = comboTXDispWFAveraging.SelectedIndex;
        console.UpdateTXSpectrumDisplayVars();
    }

    private void udTXDisplayAVTime_ValueChanged(object sender, EventArgs e) {
        console.specRX.GetSpecRX(cmaster.inid(1, 0)).AvTauWF
            = 0.001 * (double)udTXDisplayAVTime.Value;
        console.UpdateTXSpectrumDisplayVars();
    }
    ...
    private void chkANAN8000DLEDisplayVoltsAmps_CheckedChanged(
        object sender, EventArgs e) {
        console.ANAN8000DLEDisplayVoltsAmps
            = chkANAN8000DLEDisplayVoltsAmps.Checked;
    }
    ...
    private void setCFCProfile(object sender, EventArgs e) {
        ...
        unsafe {
            fixed(double* Fptr = &F[0], Gptr = &G[0], Eptr = &E[0]) {
                WDSP.SetTXACFCOMPprofile(
                    WDSP.id(1, 0), nfreqs, Fptr, Gptr, Eptr);
            }
        }
    }
    ...
    private void udDEXPThreshold_ValueChanged(object sender, EventArgs e) {
        cmaster.CMSetTXAVoxThresh(
            0, Math.Pow(10.0, (double)udDEXPThreshold.Value / 20.0));
        console.VOXSens = (int)udDEXPThreshold.Value;
    }

    private void udDEXPExpansionRatio_ValueChanged(object sender, EventArgs e) {
        cmaster.SetDEXPExpansionRatio(
            0, Math.Pow(10.0, (double)udDEXPExpansionRatio.Value / 20.0));
    }

    private void udDEXPHysteresisRatio_ValueChanged(
        object sender, EventArgs e) {
        cmaster.SetDEXPHysteresisRatio(
            0, Math.Pow(10.0, -(double)udDEXPHysteresisRatio.Value / 20.0));
    }
    ...
    private void chkDEXPLookAheadEnable_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void udUpdatesPerStepMax_ValueChanged(object sender, EventArgs e) {
        ...
        if (selectedMax <= currMin)
            udUpdatesPerStepMin.Value = selectedMax
                - 1; // keep min and max sensible and separated by at least 1
    ...
    private void udUpdatesPerStepMin_ValueChanged(object sender, EventArgs e) {
        ...
        if (selectedMin >= currMax)
            udUpdatesPerStepMax.Value = selectedMin
                + 1; // keep min and max sensible and separated by at least 1
    ...
    private void udAntiVoxGain_ValueChanged(object sender, EventArgs e) {
        cmaster.SetAntiVOXGain(
            0, Math.Pow(10.0, (double)udAntiVoxGain.Value / 20.0));
    }
    ...
    /// MW0LGE
    ///
    private void chkDisablePicDisplayBackgroundImage_CheckedChanged(
        object sender, EventArgs e) {
        if (initializing)
            return; // ignore until we force call this after init MW0LGE
        console.DisableBackgroundImage
            = chkDisablePicDisplayBackgroundImage.Checked;
    }

    private void clrbtnDataFill_Changed(object sender, EventArgs e) {
        Display.DataFillColor
            = Color.FromArgb(tbDataFillAlpha.Value, clrbtnDataFill.Color);
    }
    ...
    private void chkWheelTunesOutsideSpectral_CheckedChanged(
        object sender, EventArgs e) {
        console.WheelTunesOutsideSpectral
            = chkWheelTunesOutsideSpectral.Checked;
    }

    private void ComboHIDMouseWheel_SelectedIndexChanged(
        object sender, EventArgs e) {
        ...
        if (objItem != null) {
            ...
            txtDeviceHID_hidden.Text
                = objItem.DeviceID; // this control will be saved out
        }
    }

#region RawInput WIP
    ///--- RAWINPUT WIP
    private void ChkGlobalListenForMouseWheel_CheckedChanged(
        object sender, EventArgs e) {
        console.GlobalListenForMouseWheel
            = chkGlobalListenForMouseWheel.Checked;
    }

    private void ChkWheelOnlyAdjustsVFO_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void ChkAlsoUseSpecificMouseWheel_CheckedChanged(
        object sender, EventArgs e) {
        console.AlsoUseSpecificMouseWheel
            = chkAlsoUseSpecificMouseWheel.Checked;
    ...
    public void UpdateRawInputMouseDevices(
        Dictionary<IntPtr, MouseEvent> mouseDevices) {
        comboHIDMouseWheel.Items.Clear();

        foreach (KeyValuePair<IntPtr, MouseEvent> entry in mouseDevices) {
            MouseEvent me = entry.Value;

            HIDComboItem objItem
                = new HIDComboItem(me.Name, me.DeviceHandle, me.DeviceName);
            int nIndex = comboHIDMouseWheel.Items.Add(objItem);

            if (me.DeviceName == txtDeviceHID_hidden.Text)
                comboHIDMouseWheel.SelectedIndex = nIndex;
        }
    }
    ...
    private class HIDComboItem : object {
        ...
        public override string ToString() { return m_sName; }
    ...
    private void ChkShowRXFilterOnWaterfall_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowRXFilterOnWaterfall = chkShowRXFilterOnWaterfall.Checked;
    }

    private void ChkShowTXFilterOnWaterfall_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowTXFilterOnWaterfall = chkShowTXFilterOnWaterfall.Checked;
    }

    private void ChkShowRXZeroLineOnWaterfall_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowRXZeroLineOnWaterfall
            = chkShowRXZeroLineOnWaterfall.Checked;
    }

    private void ChkShowTXZeroLineOnWaterfall_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowTXZeroLineOnWaterfall
            = chkShowTXZeroLineOnWaterfall.Checked;
    }

    private void ChkShowTXFilterOnRXWaterfall_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowTXFilterOnRXWaterfall
            = chkShowTXFilterOnRXWaterfall.Checked;
    }

    private void ChkZoomShiftModifier_CheckedChanged(
        object sender, EventArgs e) {
        console.ZoomShiftModifier = chkZoomShiftModifier.Checked;
        chkReverseShiftZoomModifier.Enabled = chkZoomShiftModifier.Checked;
    }

    private void ChkReverseShiftZoomModifier_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void ChkWaterfallUseRX1SpectrumMinMax_CheckedChanged(
        object sender, EventArgs e) {
        ...
        console.WaterfallUseRX1SpectrumMinMax
            = chkWaterfallUseRX1SpectrumMinMax.Checked;
    }

    private void ChkWaterfallUseRX2SpectrumMinMax_CheckedChanged(
        object sender, EventArgs e) {
        ...
        console.WaterfallUseRX2SpectrumMinMax
            = chkWaterfallUseRX2SpectrumMinMax.Checked;
    }
    ...
    private void chkSmallModeFilteronVFOs_CheckedChanged(
        object sender, EventArgs e) {
        console.ShowSmallModeFilterOnVFOs = chkSmallModeFilteronVFOs.Checked;
    }

    private void comboDisplayThreadPriority_SelectedIndexChanged(
        object sender, EventArgs e) {
        // Below Normal
        // Normal
        // Above Normal
        ...
        if (comboDisplayThreadPriority.SelectedIndex < 0)
            return; // ignore 0 or not selected

        console.DisplayThreadPriority
            = (ThreadPriority)comboDisplayThreadPriority.SelectedIndex;
    }
    ...
    private void chkAccurateFrameTiming_CheckedChanged(
        object sender, EventArgs e) {
        console.UseAccurateFramingTiming = chkAccurateFrameTiming.Checked;
    }

    private void chkPeakBlobsEnabled_CheckedChanged(
        object sender, EventArgs e) {
    ...
    private void chkPeakBlobInsideFilterOnly_CheckedChanged(
        object sender, EventArgs e) {
        Display.ShowPeakBlobsInsideFilterOnly
            = chkPeakBlobInsideFilterOnly.Checked;
    }
    ...
    private void clrbtnSignalHistoryColour_Changed(object sender, EventArgs e) {
        console.SignalHistoryColour = Color.FromArgb(
            tbSignalHistoryAlpha.Value, clrbtnSignalHistoryColour.Color);
    }

    private void tbSignalHistoryAlpha_Scroll(object sender, EventArgs e) {
        clrbtnSignalHistoryColour_Changed(this, EventArgs.Empty);
        toolTip1.SetToolTip(
            tbSignalHistoryAlpha, tbSignalHistoryAlpha.Value.ToString());
    }
    ...
    private void chkBlobPeakHold_CheckedChanged(object sender, EventArgs e) {
        chkPeakHoldFade.Enabled
            = chkBlobPeakHold.Checked && chkBlobPeakHold.Enabled;
        udBlobPeakHoldMS.Enabled
            = chkBlobPeakHold.Checked && chkBlobPeakHold.Enabled;
    ...
    private void udSignalHistoryDuration_ValueChanged(
        object sender, EventArgs e) {
    ...
    private void chkBypassVACPlayingRecording_CheckedChanged(
        object sender, EventArgs e) {
        console.BypassVACWhenPlayingRecording
            = chkBypassVACPlayingRecording.Checked;
    }
    ...
    private void ComboGanymedeCATPort_SelectedIndexChanged(
        object sender, EventArgs e) {
        if (comboGanymedeCATPort.Text == "None") {
            ...
            chkEnableGanymede.Enabled = true;
    ...
    private void ComboAriesCATPort_SelectedIndexChanged(
        object sender, EventArgs e) {
        if (comboAriesCATPort.Text == "None") {
            if (chkEnableAries.Checked) {
                if (comboAriesCATPort.Focused) chkEnableAries.Checked = false;
            }

            chkEnableAries.Enabled = false;
        } else
            chkEnableAries.Enabled = true;
    ...
    private void ChkEnableGanymede_CheckedChanged(object sender, EventArgs e) {
        ...
        if (comboGanymedeCATPort.Text == ""
            || !comboGanymedeCATPort.Text.StartsWith("COM")) {
            if (chkEnableGanymede.Focused) {
                if (chkEnableGanymede.Checked) {
                    MessageBox.Show("The CAT port \""
                        + comboGanymedeCATPort.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkEnableGanymede.Checked = false;
                }
        ...
        if (chkEnableGanymede.Checked && console.PTTBitBangEnabled
            ...
            MessageBox.Show(
                "Ganymede CAT port cannot be the same as Bit Bang Port",
                "Port Selection Error", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            chkEnableGanymede.Checked = false;
        }
        ...
        if (chkEnableGanymede.Checked) {
            try {
                ...
                chkEnableGanymede.Checked = false;
                MessageBox.Show(
                    "Could not initialize Ganymede control.  Exception was:\n\n "
                        + ex.Message
                        + "\n\nGanymede control has been disabled.",
                    "Error Initializing Ganymede control", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        } else
            console.GanymedeCATEnabled = false;
    ...
    private void ChkEnableAries_CheckedChanged(object sender, EventArgs e) {
        ...
        if (comboAriesCATPort.Text == ""
            || !comboAriesCATPort.Text.StartsWith("COM")) {
            if (chkEnableAries.Focused) {
                if (chkEnableAries.Checked) {
                    MessageBox.Show("The CAT port \"" + comboAriesCATPort.Text
                        + "\" is not a valid port.\n"
                        + "Please select another port.");
                    chkEnableAries.Checked = false;
                }
        ...
        if (chkEnableAries.Checked && console.PTTBitBangEnabled
            ...
            MessageBox.Show(
                "Aries CAT port cannot be the same as Bit Bang Port",
                "Port Selection Error", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            chkEnableAries.Checked = false;
        }
        ...
        if (chkEnableAries.Checked) {
            try {
                ...
                chkEnableAries.Checked = false;
                MessageBox.Show(
                    "Could not initialize Aries control.  Exception was:\n\n "
                        + ex.Message + "\n\nAries control has been disabled.",
                    "Error Initializing Aries control", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        } else
            console.AriesCATEnabled = false;
    ...
    private void chkQSOTimerOnlyDuringMOX_CheckedChanged(
        object sender, EventArgs e) {
        console.QSOTimerDuringMoxOnly = chkQSOTimerOnlyDuringMOX.Checked;
    }

    private void chkQSOTimerPlaySoundOnExpiry_CheckedChanged(
        object sender, EventArgs e) {
        bool bEnabled = chkQSOTimerEnabled.Checked
            && chkQSOTimerPlaySoundOnExpiry.Checked;

        btnQSOTimerSelectWAV.Enabled = bEnabled;
        btnQSOTimerPlaySelectedWAV.Enabled
            = bEnabled && console.QSOTimerAudioPlayer.IsReady;
    ...
    private void audioFileLoaded(bool bOk) {
        btnQSOTimerPlaySelectedWAV.Enabled
            = chkQSOTimerPlaySoundOnExpiry.Checked && bOk;
    }
    ...
    private void chkQSOTimerResetOnMOX_CheckedChanged(
        object sender, EventArgs e) {
        console.QSOTimerResetOnMox = chkQSOTimerResetOnMOX.Checked;
    }

    private void setQSOTimerDuration() {
        int nDurationSecs = ((int)udQSOTimerMinutes.Value * 60)
            + (int)udQSOTimerSeconds.Value;
        if (nDurationSecs < 5) {
            nDurationSecs = 5;
    ...
    private void chkQSOTimerResetOnExpiry_CheckedChanged(
        object sender, EventArgs e) {
        console.QSOTimerResetOnExpiry = chkQSOTimerResetOnExpiry.Checked;

        chkQSOTimerFlashTimerIfResetOnExpiry.Enabled
            = chkQSOTimerResetOnExpiry.Enabled
            && chkQSOTimerResetOnExpiry.Checked;

        chkQSOTimerFlashTimerIfResetOnExpiry_CheckedChanged(
            this, EventArgs.Empty);
    }

    private void chkQSOTimerFlashTimerIfResetOnExpiry_CheckedChanged(
        object sender, EventArgs e) {
        console.QSOTimerFlashAfterAutoReset
            = chkQSOTimerFlashTimerIfResetOnExpiry.Checked;
    }
    ...
    private void btnVAC1Refresh_Click(object sender, EventArgs e) {
        ...
        Debug.Assert((PortAudioForThetis.PaErrorCode)result
            == PortAudioForThetis.PaErrorCode.paNoError);
        var result2 = PortAudioForThetis.PA_Initialize();
        Debug.Assert((PortAudioForThetis.PaErrorCode)result2
            == PortAudioForThetis.PaErrorCode.paNoError);
    ...
    private void btnResetCFC_Click(object sender, EventArgs e) {
        DialogResult dr = MessageBox.Show(
            "Are you sure you want to reset the CFC controls\n"
                + "to flat (zero)?",
...
public class Channel60m {
    private double freq;
    public Channel60m(double f) : base() { freq = f; }
