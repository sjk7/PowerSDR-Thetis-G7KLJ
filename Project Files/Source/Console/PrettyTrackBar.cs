        ...
        switch (orientation) {
            case Orientation.Horizontal:
                ...
                if (head_image == null) {
                    ...
                    head_x = (int)Math.Round(
                                 (val - min) / (double)(max - min) * width)
                        + Padding.Left;
                    head_y = (int)((this.Height - Padding.Vertical) / 2
                                 - head_image.Height / 2)
                        + Padding.Top;
                }
                break;
            case Orientation.Vertical:
                ...
                if (head_image == null) {
                    ...
                    head_x = (int)((this.Width - Padding.Horizontal) / 2
                                 - head_image.Width / 2)
                        + Padding.Top;
                    head_y = (int)(height
                                 - (val - min) / (double)(max - min) * height)
                        + Padding.Top;
                }
                break;
        }
...
#region Event Handlers
        ...
        if (this.Enabled) {
            if (head_rect.Contains(e.X, e.Y)) {
                ...
                switch (orientation) {
                    case Orientation.Horizontal:
                        if (e.Y >= head_rect.Y
                            && e.Y <= head_rect.Y + head_rect.Height) {
                            if (e.X < head_rect.X)
                                new_val = old_val - large_change;
                            else
                                new_val = old_val + large_change;
                    ...
                    case Orientation.Vertical:
                        if (e.X >= head_rect.X
                            && e.X <= head_rect.X + head_rect.Width) {
                            if (e.Y > head_rect.Y)
                                new_val = old_val - large_change;
                            else
                                new_val = old_val + large_change;
        ...
        if (this.Enabled && sliding) {
            ...
            switch (orientation) {
                case Orientation.Horizontal:
                    ...
                    if (head_rect.X <= Padding.Left && delta < 0)
                        return;
                    else if (head_rect.X >= (width + Padding.Left) && delta > 0)
                        return;

                    percent
                        = (head_rect.X - Padding.Left + delta) / (double)width;
                    ...
                    if (down_x > width + Padding.Left + head_image.Width)
                        down_x = width + Padding.Left + head_image.Width;

                    head_rect.X += delta;
                    if (head_rect.X < Padding.Left) head_rect.X = Padding.Left;
                    if (head_rect.X > width + Padding.Left)
                        head_rect.X = width + Padding.Left;
                    break;
                case Orientation.Vertical:
                    delta = e.Y - down_y;
                    height = this.Height - head_image.Height - Padding.Vertical;

                    if (head_rect.Y <= Padding.Top && delta < 0)
                        return;
                    else if (head_rect.Y >= (height + Padding.Top) && delta > 0)
                        return;

                    percent = 1.0
                        - (head_rect.Y - Padding.Top + delta) / (double)height;
                    ...
                    if (down_y > height + Padding.Top + head_image.Height)
                        down_y = height + Padding.Top + head_image.Height;

                    head_rect.Y += delta;
                    if (head_rect.Y < Padding.Top) head_rect.Y = Padding.Top;
                    if (head_rect.Y > height + Padding.Top)
                        head_rect.Y = height + Padding.Top;
                    break;
            }
            ...
            this.Invalidate();
            // Debug.WriteLine("delta: "+delta+"  percent:
            // "+percent.ToString("f4")+"  new_val: " + new_val+"  down_x:
            // "+down_x+"  head_x: "+head_rect.X);
        }
    }
    ...
    protected override void OnPaint(PaintEventArgs pe) {
        ...
        if (this.Enabled) {
            ...
            // draw head
            if (head_image != null)
                g.DrawImage(head_image, head_rect.X, head_rect.Y,
                    head_image.Width, head_image.Height);
        } else {
            ...
            float[][] ptsArray
                = { new float[] { contrast, 0, 0, 0, 0 }, // scale red
                      ...
                      new float[] { newBrightness, newBrightness, newBrightness,
                          0, 1 } };
            ...
            /*if(base.BackgroundImage != null)
                g.DrawImage(base.BackgroundImage,
                    ...
                    0, 0, base.BackgroundImage.Width,
               base.BackgroundImage.Height, GraphicsUnit.Pixel,
                    imageAttributes);*/

            // draw head
            if (head_image != null)
                g.DrawImage(head_image,
                    new Rectangle(head_rect.X, head_rect.Y, head_image.Width,
                        head_image.Height),
    ...
    protected override void OnMouseWheel(MouseEventArgs e) {
        if (e.Delta >= 120 && this.Value + large_change <= this.Maximum) {
            ...
            && this.Value - large_change >= this.Minimum) {
            Value -= large_change;
            OnScroll(this, EventArgs.Empty);
        }
        base.OnMouseWheel(e);
