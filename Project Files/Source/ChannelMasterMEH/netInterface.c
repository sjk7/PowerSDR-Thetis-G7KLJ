...
PORT int StartAudioNative() {
    ...
    UpdateRadioProtocolSampleSize(); // we know the radio protocol - need to
                                     // update sample sizes
    ...
    if (RadioProtocol == USB) {
        do { // once
            ...
            if (prn != NULL) {
                ...
                WaitForSingleObject(prn->hReadThreadInitSem,
                    INFINITE); // wait for the thread to get going
        ...
        do {
            ...
            prn->hReadThreadInitSem = CreateSemaphore(NULL, 0, 1, NULL);
            prn->hReadThreadMain
                = (HANDLE)_beginthreadex(NULL, 0, ReadThreadMain, 0, 0, NULL);
            WaitForSingleObject(prn->hReadThreadInitSem,
                INFINITE); // wait for the thread to get going
            prn->hKeepAliveThread
                = (HANDLE)_beginthreadex(NULL, 0, KeepAliveMain, 0, 0, NULL);
...
PORT int nativeGetDotDashPTT() {
    return (prn->dot_in | prn->dash_in | prn->ptt_in) & 0x7;
}

PORT int getAndResetADC_Overload() {
    int n = prn->adc[0].adc_overload | (prn->adc[1].adc_overload << 1)
        | (prn->adc[2].adc_overload << 2);

    return n;
}

PORT int getOOO() { // OOO == Out Of Order packet
    int result = 0;
...
PORT int getSeqInDelta(int nInit, int rx, int deltas[], char* dateTimeStamp,
    int* received_seqnum, int* last_seqnum) {
    int nRet = 0;
    ...
    if (prn->rx[rx].snapshot != NULL) {
        memcpy(deltas, prn->rx[rx].snapshot->rx_in_seq_snapshot,
            sizeof(int) * MAX_IN_SEQ_LOG);
        memcpy(dateTimeStamp, prn->rx[rx].snapshot->dateTimeStamp,
            sizeof(char) * 24);
...
PORT int getExciterPower() {

    return prn->tx[0].exciter_power;
}

PORT float getFwdPower() {

    return FwdPower;
}

PORT float getRevPower() {

    return RevPower;
}

PORT int getUserADC0() {

    return prn->user_adc0;
}

PORT int getUserADC1() {

    return prn->user_adc1;
}

PORT int getUserADC2() {

    return prn->user_adc2;
}

PORT int getUserADC3() {

    return prn->user_adc3;
}

PORT int getHermesDCVoltage() {

    return prn->supply_volts;
}

PORT void SetPttOut(int xmit) {
...
PORT void EnableEClassModulation(int bit) {
    if (prn->cw.eer != bit) {
        prn->cw.eer = bit;
        if (listenSock != INVALID_SOCKET) CmdRx();
    }
}

PORT void SetOCBits(int b) {
    if (prn->oc_output != b) {
        prn->oc_output = b;
        if (listenSock != INVALID_SOCKET && prn->sendHighPriority != 0)
            CmdHighPriority();
...
PORT void SetAlexAtten(int bits) {
    if (mkiibpf) return;
...
PORT void SetADCDither(int bits) {
...
PORT void SetADCRandom(int bits) {
...
PORT void SetAntBits(int rx_only_ant, int trx_ant, int rx_out, char tx) {
    ...
    prbpfilter->_XVTR_Rx_In
        = (rx_only_ant & (0x01 | 0x02)) == (0x01 | 0x02); // XVTR
...
PORT void SetVFOfreq(int id, int freq, int tx) {
    if (tx) {
...
PORT void SetOutputPowerFactor(int u) {
...
PORT void SetMicBoost(int bits) {
...
PORT void EnableApolloFilter(int bits) {
    if (bits != 0)
        ApolloFilt = 0x4;
    else
        ApolloFilt = 0;
...
PORT void EnableApolloTuner(int bits) {
    if (bits != 0)
        ApolloTuner = 0x8;
    else
        ApolloTuner = 0;
...
PORT void EnableApolloAutoTune(int bits) {
    if (bits != 0)
        ApolloATU = 0x10;
    else
        ApolloATU = 0;
...
PORT void SelectApolloFilter(int bits) {
    if (bits != 0)
...
PORT void SetAlexHPFBits(int bits) {
    if (AlexHPFMask != bits) {
...
PORT void DisablePA(int bit) {
    if (prn->tx[0].pa != bit) {
        prn->tx[0].pa = bit;
        if (listenSock != INVALID_SOCKET) CmdGeneral();
    }
}

PORT void SetAlex2HPFBits(int bits) {
...
PORT void SetAlex3HPFBits(int bits) {
    Alex3HPFMask = bits;
}

PORT void SetAlex4HPFBits(int bits) {
    Alex4HPFMask = bits;
}

PORT void SetAlexLPFBits(int bits) {
    if (AlexLPFMask != bits) {
...
PORT void SetAlex2LPFBits(int bits) {
    Alex2LPFMask = bits;
}

PORT void SetAlex3LPFBits(int bits) {
    Alex3LPFMask = bits;
}

PORT void SetAlex4LPFBits(int bits) {
    Alex4LPFMask = bits;
}

PORT void SetRX1Preamp(int bits) {
...
PORT void SetRX2Preamp(int bits) {
...
PORT void SetMicTipRing(int bits) {
...
PORT void SetLineBoost(int bits) {
...
PORT void SetPureSignal(int bit) {
...
PORT void SetUserOut0(int out) {
    prn->user_dig_out = out & 1;
}

PORT void SetUserOut1(int out) {
    prn->user_dig_out = (out << 1) & 2;
}

PORT void SetUserOut2(int out) {
    prn->user_dig_out = (out << 2) & 4;
}

PORT void SetUserOut3(int out) {
    prn->user_dig_out = (out << 3) & 8;
}

PORT void SetADC1StepAttenData(int data) {
...
PORT void SetADC2StepAttenData(int data) {
...
PORT void SetADC3StepAttenData(int data) {
    if (prn->adc[2].rx_step_attn != data) {
        prn->adc[2].rx_step_attn = data;
        if (listenSock != INVALID_SOCKET) CmdHighPriority();
    }
}

PORT void ReversePaddles(int bits) {
...
PORT void SetCWKeyerSpeed(int speed) {
...
PORT void SetCWKeyerMode(int mode) {
...
PORT void SetCWKeyerWeight(int weight) {
...
PORT void EnableCWKeyerSpacing(int bits) {
...
PORT void SetADC_cntrl1(int bits) {
    if (ADC_cntrl1 != bits) {
...
PORT int GetADC_cntrl1() {
    return ADC_cntrl1;
}

PORT void SetADC_cntrl2(int bits) {
    if (ADC_cntrl2 != bits) {
...
PORT int GetADC_cntrl2() {
    return ADC_cntrl2;
}

PORT void SetADC_cntrl_P1(int bits) // ADC assignment for protocol1.  14 bits, 2
                                    // per DDC:  66554433221100
{
    P1_adc_cntrl = bits;
}

PORT int GetADC_cntrl_P1() {
    return P1_adc_cntrl;
}

PORT void SetTxAttenData(int bits) {
    int i;
...
PORT void EnableCWKeyer(int enable) {
...
PORT void SetCWSidetoneVolume(int vol) {
...
PORT void SetCWPTTDelay(int delay) {
...
PORT void SetCWHangTime(int time) {
...
PORT void SetCWSidetoneFreq(int freq) {
...
PORT void SetEERPWMmin(int min) {
...
PORT void SetEERPWMmax(int max) {
...
PORT void SetCWIambic(int enable) {
...
PORT void SetCWBreakIn(int enable) {
...
PORT int getHaveSync() {
    return HaveSync;
}

PORT int getControlByteIn(int n) {
    if (n < 0 || n > 4) {
        return -1;
    }
...
PORT void EnableRx(int id, int enable) {
...
PORT void EnableRxSync(int id, int sync) {
    if (prn->rx[id].sync != sync) {
        prn->rx[id].sync = sync & 0xff;
    }
}

PORT void Protocol1DDCConfig(
    int ddcconfig, int en_diversity, int rxcount, int inddc) {
...
PORT void SetDDCRate(int id, int rate) {
    switch (rate) {
        case 0: prn->rx[id].sampling_rate = 0; break;
        case 48000: prn->rx[id].sampling_rate = 48; break;
        case 96000: prn->rx[id].sampling_rate = 96; break;
        case 192000: prn->rx[id].sampling_rate = 192; break;
        case 384000: prn->rx[id].sampling_rate = 384; break;
        case 768000: prn->rx[id].sampling_rate = 768; break;
        case 1536000: prn->rx[id].sampling_rate = 1536; break;
    }
...
PORT void SetWBPacketsPerFrame(int ppf) {
    prn->wb_packets_per_frame = ppf;
    if (listenSock != INVALID_SOCKET) CmdGeneral();
}

PORT void SetWBUpdateRate(int ur) {
    prn->wb_update_rate = ur;
    if (listenSock != INVALID_SOCKET) CmdGeneral();
}

PORT void SetWBEnable(int adc, int enable) {
    if (enable)
        InterlockedBitTestAndSet(&prn->wb_enable, adc);
    else
        InterlockedBitTestAndReset(&prn->wb_enable, adc);
    if (listenSock != INVALID_SOCKET) CmdGeneral();
}

PORT void SendHighPriority(int send) {
    if (send) {
        prn->sendHighPriority = 1;
        if (listenSock != INVALID_SOCKET) CmdHighPriority();
    } else
        prn->sendHighPriority = 0;
}

PORT void SetWatchdogTimer(int enable) {
    if (prn->wdt != enable) {
        prn->wdt = enable;
        if (listenSock != INVALID_SOCKET) CmdGeneral();
    }
...
PORT void SetMKIIBPF(int bpf) {
    mkiibpf = bpf;
}

PORT void SetXVTREnable(int enable) {
...
PORT void LRAudioSwap(int swap) {
    if (prn->lr_audio_swap != swap) prn->lr_audio_swap = swap;
}
...
PORT void create_rnet() {
    ...
    if (prn) {
        prn->RxBuff = (SAMPLETYPE**)calloc(8, sizeof(SAMPLETYPE*));
        for (int i = 0; i < 8; i++)
            prn->RxBuff[i] = (SAMPLETYPE*)calloc(64, 2 * sizeof(SAMPLETYPE));
        prn->RxReadBufp = (SAMPLETYPE*)calloc(1, 2 * sizeof(SAMPLETYPE) * 240);
        prn->TxReadBufp = (SAMPLETYPE*)calloc(1, 2 * sizeof(SAMPLETYPE) * 720);
        prn->ReadBufp = (unsigned char*)calloc(1, sizeof(unsigned char) * 1444);
        prn->OutBufp = (char*)calloc(1, sizeof(char) * 1440);
        prn->outLRbufp = (SAMPLETYPE*)calloc(1, sizeof(SAMPLETYPE) * 1440);
        prn->outIQbufp = (SAMPLETYPE*)calloc(1, sizeof(SAMPLETYPE) * 1440);
        ...
        for (i = 0; i < 6; i++) prn->discovery.MACAddr[i] = 0;
...
PORT void destroy_rnet() {
    ...
    if (prn->OutBufp != NULL) free(prn->OutBufp);
    free(prn->TxReadBufp);
    free(prn->RxReadBufp);
    for (i = 0; i < 8; i++) free(prn->RxBuff[i]);
    free(prn->RxBuff);
    free(prn->ReadBufp);
    clearSnapshots();
...
void PrintTimeHack() {
    GetLocalTime(&lt);
    printf("(%02d/%02d %02d:%02d:%02d:%03d) ", lt.wMonth, lt.wDay, lt.wHour,
...
void UpdateRadioProtocolSampleSize() {
    ...
    for (i = 0; i < MAX_RX_STREAMS; i++) {
        prn->rx[i].spp
            = RadioProtocol == USB ? 63 : 238; // IQ-samples per packet
    }
    for (i = 0; i < MAX_TX_STREAMS; i++) {
        prn->tx[i].spp
            = RadioProtocol == USB ? 126 : 240; // IQ-samples per packet
    }
