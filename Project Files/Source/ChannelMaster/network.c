...
PORT int nativeInitMetis(
    char* netaddr, char* localaddr, int localport, int protocol) {
    IPAddr DestIp = 0;
...
PORT int GetMetisIPAddr(void) {
    return MetisAddr;
}

PORT void GetMACAddr(unsigned char addr_bytes[]) {
    memcpy(addr_bytes, prn->discovery.MACAddr, 6);
}

PORT void GetCodeVersion(unsigned char addr_bytes[]) {
    memcpy(addr_bytes, &(prn->discovery.fwCodeVersion), 1);
}

PORT void GetBoardID(char addr_bytes[]) {
...
void addSnapShot(
    int rx, unsigned int received_seqnum, unsigned int last_seqnum) {
    // add newest to head
    ...
    memcpy(snapshot->rx_in_seq_snapshot, prn->rx[rx].rx_in_seq_delta,
    ...
    sprintf(snapshot->dateTimeStamp, "%02d/%02d %02d:%02d:%02d:%03d",
    ...
    if (prn->rx[rx].snapshot_length == MAX_IN_SEQ_SNAPSHOTS) {
        ...
        // prn->rx[rx].snapshot is used in the retreive snapshot code in
        // netinterface
...
int ReadUDPFrame(unsigned char* bufp) {
    ...
    switch (inport = ntohs(fromaddr.sin_port)) {
        ...
        case WB0Port: // 1027: // 1028 bytes - 16-bit raw ADC (default values)
        {
            ...
            double* wb_buff
                = prn->adc[adc_id].wb_buff; // data buffer for wideband samples
            // NOTE:  This code assumes 16-bits per sample ... can add other
            // options as needed.
            int ii, jj;
            for (ii = 0, jj = 4; ii < wb_spp;
                 ii++, jj += 2) // convert the samples to doubles
            ...
            switch (prn->adc[adc_id].wb_state) {
                ...
                case 1: // continue within the frame
                    if (seqnum
                        == prn->adc[adc_id]
                               .wb_seqnum) // sequence correct:  send the data
                    {
                        Spectrum(disp_id, 0, 0, wb_buff, wb_buff);
        ...
        default: printf("Rcvd data on Port %d\n", nrecv); break;
    }
...
void ReadThreadMainLoop() {
    ...
    while (io_keep_running != 0) {
        DWORD retVal = WSAWaitForMultipleEvents(
            1, &prn->hDataEvent, FALSE, prn->wdt ? 3000 : WSA_INFINITE, FALSE);
        if ((retVal == WSA_WAIT_FAILED) || (retVal == WSA_WAIT_TIMEOUT)) {
            ...
            WSAEnumNetworkEvents(
                listenSock, prn->hDataEvent, &prn->wsaProcessEvents);
            if (prn->wsaProcessEvents.lNetworkEvents & FD_READ) {
                ...
                switch (rc) {
                    case 1025:
                        ...
                        //         [3] - ADC3  Overload 1 = active, 0 = inactive
                        //         Bit [4] - ADC4  Overload 1 = active, 0 =
                        //         inactive Bit [5] - ADC5  Overload 1 = active,
                        //         0 = inactive Bit [6] - ADC6  Overload 1 =
                        //         active, 0 = inactive Bit [7] - ADC7  Overload
                        //         1 = active, 0 = inactive
                        for (i = 0; i < MAX_ADC; i++)
                            prn->adc[i].adc_overload
                                = ((prn->ReadBufp[1] >> i) & 0x1) != 0;

                        // Bytes 2,3      Exciter Power [15:0]     * 12 bits
                        // sign extended to 16 Bytes 10,11    FWD Power [15:0]
                        // ditto Bytes 18,19    REV Power [15:0]           ditto
                        prn->tx[0].exciter_power
                            = prn->ReadBufp[2] << 8 | prn->ReadBufp[3];
                        prn->tx[0].fwd_power
                            = prn->ReadBufp[10] << 8 | prn->ReadBufp[11];
                        prn->tx[0].rev_power
                            = prn->ReadBufp[18] << 8 | prn->ReadBufp[19];
                        PeakFwdPower((float)(prn->tx[0].fwd_power));
                        PeakRevPower((float)(prn->tx[0].rev_power));
                        // Bytes 45,46  Supply Volts [15:0]
                        prn->supply_volts
                            = prn->ReadBufp[45] << 8 | prn->ReadBufp[46];
                        ...
                        prn->user_adc3
                            = prn->ReadBufp[47] << 8 | prn->ReadBufp[48];
                        prn->user_adc2
                            = prn->ReadBufp[49] << 8 | prn->ReadBufp[50];
                        prn->user_adc1 = prn->ReadBufp[51] << 8
                            | prn->ReadBufp[52]; // AIN4
                        prn->user_adc0 = prn->ReadBufp[53] << 8
                            | prn->ReadBufp[54]; // AIN3

                        SetAmpProtectADCValue(0, prn->user_adc0);

                        // Byte 55 - Bit [0] - User I/O (IO4) 1 = active, 0 =
                        // inactive
                        //          Bit [1] - User I/O (IO5) 1 = active, 0 =
                        //          inactive Bit [2] - User I/O (IO6) 1 =
                        //          active, 0 = inactive Bit [3] - User I/O
                        //          (IO8) 1 = active, 0 = inactive Bit [4] -
                        //          User I/O (IO2) 1 = active, 0 = inactive
                        prn->user_dig_in = prn->ReadBufp[55];

                        prn->hardware_LEDs
                            = prn->ReadBufp[26] << 8 | prn->ReadBufp[27];

                        break;
                    case 1026: // 1440 bytes 16-bit mic samples
                        for (i = 0, k = 0; i < prn->mic.spp; i++, k += 2) {
                            // prn->TxReadBufp[2 * i] = ((prn->ReadBufp[k + 0] &
                            // 0xff) | 	(prn->ReadBufp[k + 1] << 8)) / 32768.0;
                        ...
                        Inbound(inid(1, 0), prn->mic.spp, prn->TxReadBufp);
                        break;
                    case 1027: // 1024 bytes 16bit raw ADC data, handled in
                               // ReadUDPFrame()
                    case 1028:
                    case 1029:
                    case 1030:
...
void CmdHighPriority() { // port 1027
    ...
    // Bit [2]  - Dash 1 = active, 0 = inactive
    packetbuf[5]
        = (prn->tx[0].dash << 2 | prn->tx[0].dot << 1 | prn->tx[0].cwx) & 0x7;

    // RX0 (Hz)/Mercury0 - Rx0 Frequency
    packetbuf[9] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 24) & 0xff
        : (prn->rx[0].frequency >> 24) & 0xff;
    packetbuf[10] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 16) & 0xff
        : (prn->rx[0].frequency >> 16) & 0xff;
    packetbuf[11] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 8) & 0xff
        : (prn->rx[0].frequency >> 8) & 0xff;
    packetbuf[12] = prn->tx[0].ptt_out && prn->puresignal_run
        ? prn->tx[0].frequency & 0xff
        : prn->rx[0].frequency & 0xff;

    // RX1 (Hz)/Mercury0 - Rx1_Frequency
    packetbuf[13] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 24) & 0xff
        : (prn->rx[1].frequency >> 24) & 0xff;
    packetbuf[14] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 16) & 0xff
        : (prn->rx[1].frequency >> 16) & 0xff;
    packetbuf[15] = prn->tx[0].ptt_out && prn->puresignal_run
        ? (prn->tx[0].frequency >> 8) & 0xff
        : (prn->rx[1].frequency >> 8) & 0xff;
    packetbuf[16] = prn->tx[0].ptt_out && prn->puresignal_run
        ? prn->tx[0].frequency & 0xff
        : prn->rx[1].frequency & 0xff;
    ...
    // Enable transverter T/R relay 8   Mute Audio Amp bit 1 from J16 pin 9
    // IO4---DLE
    packetbuf[1400]
        = xvtr_enable | ((!(prn->user_dig_in & 0x01)) << 1 | atu_tune << 2);
...
PORT void CmdRx() { // port 1025
    ...
    packetbuf[7] = (prn->rx[6].enable << 6 | prn->rx[5].enable << 5
                       | prn->rx[4].enable << 4 | prn->rx[3].enable << 3
                       | prn->rx[2].enable << 2 | prn->rx[1].enable << 1
                       | prn->rx[0].enable)
        & 0xff;
...
void CmdTx() { // port 1026
    ...
    // Bit [3]  - 0 = Orion mic ptt to ring and mic/mic bias to tip, 1 = Orion
    // mic ptt to tip and mic/mic bias to ring Bit [4]  - 0 = disables Orion mic
    // bias, 1 = enables Orion microphone bias
