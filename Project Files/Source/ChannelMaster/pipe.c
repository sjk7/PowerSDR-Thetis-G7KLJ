...
void create_spc0() {
    ...
    for (i = 0; i < pcm->cmSPC[0]; i++) {
        int in_id = inid(2, i);
        // noise blanker
        ppip->spc0[i].panb = create_anb(0, // run
        ...
        // noise blanker II
        ppip->spc0[i].pnob = create_nob(0, // run
            pcm->xcm_insize[in_id], // buffsize
            pcm->in[in_id], // input buffer
            pcm->in[in_id], // output buffer
            pcm->xcm_inrate[in_id], // sample rate
...
void xpipe(int stream, int pos, double** buffs) {
    ...
    if (stream == 0) // PowerSDR RX1
    {
        switch (pos) {
            case 0: // IQ data
                ...
                Spectrum0(
                    _InterlockedAnd(&pip.rcvr[0].top_pan3_run, 0xffffffff), rx,
                    1, 0, buff); // stitched pan
            ...
            case 1: // Audio data
                ...
                (*pip.rcvr[rx].xrecordwave)(
                    0, 1, ppip->rbuff[rx]); // wav recorder
                break;
        }
    ...
    } else if (stream == inid(1, 0)) // PowerSDR single transmitter
    {
        ...
        switch (pos) {
            ...
            case 0: // MIC data
                /*/ // KLJ BUGFIX: buff is populated in the xvacIN calls, so
                this
                // needs to be AFTER that, not before (you record silence on VAC
                MIC!!)
...
PORT void SendCBCreateScope(void(__stdcall* create_Scope)(int id)) {
    pip.create_Scope = create_Scope;
}

PORT void SendCBScope(
    int id, void(__stdcall* xscope)(int state, double* data)) {
    pip.rcvr[id].xscope = xscope;
}

PORT void SendCBCreateWRecord(void(__stdcall* create_WaveRecord)(int id)) {
    pip.create_WaveRecord = create_WaveRecord;
}

PORT void SendCBWaveRecorder(
    int id, void(__stdcall* xrecordwave)(int state, int pos, double* data)) {
    pip.rcvr[id].xrecordwave = xrecordwave;
}

PORT void SendCBCreateWPlay(void(__stdcall* create_WavePlay)(int id)) {
    pip.create_WavePlay = create_WavePlay;
}

PORT void SendCBWavePlayer(
    int id, void(__stdcall* xplaywave)(int state, double* data)) {
    pip.rcvr[id].xplaywave = xplaywave;
}

PORT void SetTopPan3Run(int run) {
    _InterlockedExchange(&pip.rcvr[0].top_pan3_run, run);
}

PORT void SetTXVAC(int txid, int txvac) {
