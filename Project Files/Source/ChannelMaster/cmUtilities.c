...
PORT void print_cmbuff_parameters(const char* filename, int id) {
    CMB a = pcm->pcbuff[id];
...
// Audacity:  Import Raw Data, Signed 32-bit PCM, Little-endian, Mono/Stereo per
// mode selection, 48K rate
...
PORT void WriteAudio(
    double seconds, int rate, int size, double* indata, int mode) {
        ...
        if (audiocount < n && data) {
            switch (mode) {
                ...
                case 2: // envelope (mono)
                    data[audiocount++] = (int)(conv
                        * sqrt(indata[2 * i + 0] * indata[2 * i + 0]
...
// WriteCharFiles(...) is specifically to dissect radio=>PC Protocol_1 packets
// (frames portion)
//    and write out files for Audacity.
// This is to be called ONCE FOR EACH PACKET (2 frames, 1024 bytes) of data.
// The ddc files written should be imported as 'Raw Data' into Audacity as
// Signed 32-bit Stereo. The mic file written should be imported as 'Raw Data'
// into Audacity as Signed 32-bit, Mono, 48Khz.
...
PORT void WriteCharFiles(
    int seconds, int rate, unsigned char* indata, int num_ddcs) {
    ...
    static int spf; // samples for each ddc in one frame of data = sample_sets
                    // per frame
    static int nframes; // total number of frames needed
    static int*
        ddc_data[8]; // array or pointers to complex samples from each ddc
    ...
    if (!char_ready) {
        ...
        switch (rate) {
            case 48000: mic_dec = 1; break;
            case 96000: mic_dec = 2; break;
            case 192000: mic_dec = 4; break;
            case 384000: mic_dec = 8; break;
        }
        n_mic = n / mic_dec;
        spf = 504 / (6 * num_ddcs + 2);
        nframes = (int)ceil((double)n / (double)spf);
        for (int i = 0; i < num_ddcs; i++)
            ddc_data[i] = (int*)calloc(nframes * spf,
                2 * sizeof(int)); // large enough for complex samples
    ...
    for (int frame = 0, bidx = 8; frame < 2;
         frame++, bidx += 512) // for each frame in the packet
    {
        if (frame_count < nframes) {
            int offset = frame_count * spf;
            for (int j = 0, ssidx = bidx; j < spf; j++,
                     ssidx
                 += 6 * num_ddcs + 2) // for each sample set in the frame
            {
                int stidx = 2 * (offset + j);
                for (int iddc = 0, idx = ssidx; iddc < num_ddcs;
                     iddc++, idx += 6) // for each ddc in the sample set
                {
                    ddc_data[iddc][stidx + 0] = (int)((indata[idx + 0] << 24)
                        | (indata[idx + 1] << 16)
                        | (indata[idx + 2] << 8)); // I
                    ddc_data[iddc][stidx + 1] = (int)((indata[idx + 3] << 24)
                        | (indata[idx + 4] << 16)
                        | (indata[idx + 5] << 8)); // Q
                }
    ...
    if ((frame_count >= nframes) && !char_done) {
        for (int i = 0; i < 8; i++) EscribeCosas.ddc[i] = ddc_data[i];
