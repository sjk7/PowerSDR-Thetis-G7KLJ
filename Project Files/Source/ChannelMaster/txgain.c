...
TXGAIN create_txgain(int run_fixed, int run_amp_protect, int size, double* in,
    double* out, double Igain, double Qgain, int adc_value, int adc_supply) {
    TXGAIN a = (TXGAIN)malloc0(sizeof(txgain));
    if (run_fixed) InterlockedBitTestAndSet(&a->run_fixed, 0);
    if (run_amp_protect) InterlockedBitTestAndSet(&a->run_amp_protect, 0);
    a->size = size;
    a->in = in;
    a->out = out;
    a->Igain = Igain;
...
PORT // set from console
    void
    SetTXFixedGainRun(int txid, int run) {
...
PORT // set from console
    void
    SetTXFixedGain(int txid, double Igain, double Qgain) {
    TXGAIN a = pcm->xmtr[txid].pgain;
    EnterCriticalSection(&a->cs_update0);
    a->Igain = Igain;
    a->Qgain = Qgain;
...
PORT // call from console
    int
    GetAndResetAmpProtect(int txid) {
    TXGAIN a = pcm->xmtr[txid].pgain;
...
PORT // set from console
    void
    SetAmpProtectRun(int txid, int run) {
    TXGAIN a = pcm->xmtr[txid].pgain;
    if (run)
