...
#pragma pack(pop)
    ...
    struct _rx {
        ...
        int rx_in_seq_delta[MAX_IN_SEQ_LOG]; // ring buffer that contains a
                                             // delta expected frame number vs
                                             // recevied frame number
        int rx_in_seq_delta_index; // next slot to use in ring
        _seqLogSnapshot_t*
            snapshots_head; // simple linked list of snapshots of this ring
                            // buffer when a seq error occurs
        _seqLogSnapshot_t*
            snapshots_tail; // simple linked list of snapshots of this ring
                            // buffer when a seq error occurs
        int snapshot_length; // len of this snapshot list (used to limit)
        _seqLogSnapshot_t* snapshot; // used by netInterface to work through the
                                     // list each call;
    } rx[MAX_RX_STREAMS];
