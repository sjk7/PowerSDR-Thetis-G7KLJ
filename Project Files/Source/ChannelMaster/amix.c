...
AMIX create_amix(int id, int run, int size, double** in0, double** in1,
    double* out, unsigned int what0, unsigned int what1, double volume) {
    int i;
...
__declspec(align(16)) AMIX
    pamix[MAX_EXT_AMIX]; // array of pointers for AMIXs used EXTERNAL to wdsp

void create_amixEXT(int id, int run, int size, unsigned int what0,
    unsigned int what1, double volume) {
...
PORT void SetAudioMixWhat(int id, int bank, unsigned int stream, int state) {
    AMIX a = pamix[id];
    EnterCriticalSection(&a->cs_update);
    switch (bank) {
        case 0:
...
PORT void SetAudioMixSize(int id, int size) {
    AMIX a = pamix[id];
    EnterCriticalSection(&a->cs_update);
    a->size = size;
    LeaveCriticalSection(&a->cs_update);
...
PORT void SetAudioMixVolume(int id, double volume) {
    int i;
    AMIX a = pamix[id];
    EnterCriticalSection(&a->cs_update);
    a->volume = volume;
...
PORT void SetAudioMixVol(int id, int bank, unsigned int stream, double vol) {
    AMIX a = pamix[id];
    EnterCriticalSection(&a->cs_update);
    switch (bank) {
        case 0:
