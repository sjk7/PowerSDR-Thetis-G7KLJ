...
void create_rcvr() {
    ...
    for (i = 0; i < pcm->cmRCVR; i++) {
        ...
        pcm->rcvr[i].panb = create_anb(0, // run
            ...
            30.0); // threshold
        // noise blanker II
        pcm->rcvr[i].pnob = create_nob(0, // run
            pcm->xcm_insize[in_id], // buffsize
            pcm->in[in_id], // input buffer
            pcm->in[in_id], // output buffer
            pcm->xcm_inrate[in_id], // sample rate
        ...
        for (j = 0; j < pcm->cmSubRCVR; j++) {
            OpenChannel(chid(in_id, j), // channel number
...
void destroy_rcvr() {
    ...
    for (i = 0; i < pcm->cmRCVR; i++) {
        DestroyAnalyzer(i);
        for (j = 0; j < pcm->cmSubRCVR; j++) CloseChannel(chid(inid(0, i), j));
...
void create_xmtr() {
    ...
    for (i = 0; i < pcm->cmXMTR; i++) {
        ...
        pcm->xmtr[i].pavoxmix = (AAMIX)create_aamix(
            ...
            (1 << (pcm->cmRCVR * pcm->cmSubRCVR))
                - 1, // inputs to currently mix ('what' variable)
            1.0, // volume
            ...
            0.000); // tslewdown
        // dsp channel
        OpenChannel(chid(in_id, 0), // channel number
            pcm->xcm_insize[in_id], // input buffer size
            4096, // dsp buffer size
            pcm->xcm_inrate[in_id], // input sample rate
        ...
        // Penelope gain
        pcm->xmtr[i].pgain = create_txgain(0, // run fixed gain (Penelope)
        ...
        // eer
        pcm->xmtr[i].peer = create_eer(0, // run
            pcm->xmtr[i].ch_outsize, // size
            pcm->xmtr[i].out[0], // in
            pcm->xmtr[i].out[0], // out
            ...
            1); // amiq
        // interleave (for eer)
        pcm->xmtr[i].pilv = create_ilv(0, // run
...
void destroy_xmtr() {
    ...
    for (i = 0; i < pcm->cmXMTR; i++) {
        ...
        destroy_dexp(i);
        for (j = 0; j < 2; j++) _aligned_free(pcm->xmtr[i].out[j]);
    }
}
...
void create_cmaster() {
    ...
    for (i = 0; i < pcm->cmSTREAM; i++) {
        InitializeCriticalSectionAndSpinCount(
            &pcm->update[i], 2500); // 'update' critical section
    ...
    { // audio mixer
        ...
        int what = (1 << (pcm->cmRCVR * pcm->cmSubRCVR + pcm->cmXMTR))
            - 1; // mix all
        for (i = 0; i < pcm->cmRCVR; i++)
            for (j = 0; j < pcm->cmSubRCVR; j++)
                pcm->aamix_inrates[pcm->cmSubRCVR * i + j]
                    = pcm->rcvr[i].ch_outrate;
        for (i = 0; i < pcm->cmXMTR; i++)
            pcm->aamix_inrates[pcm->cmRCVR * pcm->cmSubRCVR + i]
                = pcm->xmtr[i].ch_outrate;
        create_aamix(0, // aamix id
...
PORT void xcmaster(int stream) {
    ...
    switch (stype(stream)) {
        ...
        case 0: // standard receiver
            ...
            Spectrum0(_InterlockedAnd(&pcm->rcvr[rx].run_pan, 0xffffffff), rx,
                0, 0, // panadapter
                pcm->in[stream]);
            for (j = 0; j < pcm->cmSubRCVR; j++)
                fexchange0(chid(stream, j), pcm->in[stream],
                    pcm->rcvr[rx].audio[j], &error); // dsp
            xpipe(stream, 1, pcm->rcvr[rx].audio);
            for (j = 0; j < pcm->cmSubRCVR; j++) {
                xMixAudio(
                    0, 0, chid(stream, j), pcm->rcvr[rx].audio[j]); // mix audio
                for (k = 0; k < pcm->cmXMTR; k++)
                    xMixAudio(pcm->xmtr[k].pavoxmix, -1, chid(stream, j),
                        pcm->rcvr[rx]
                            .audio[j]); // send audio to anti-vox mixer(s)
            }
            // if (rx == 0) WriteAudio(30.0, 48000, 64, pcm->rcvr[0].audio[0],
            // 3);
            break;
        ...
        case 1: // standard transmitter
            ...
            xilv(pcm->xmtr[tx].pilv,
                pcm->xmtr[tx].out); // interleave EER, call Outbound()
            break;
...
PORT void SendpOutbound(void (*Outbound)(int id, int nsamples, double* buff)) {
    pcm->Outbound = Outbound;
    SetAAudioMixOutputPointer(0, 0, pcm->Outbound);
    SetILVOutputPointer(0, pcm->Outbound);
}

PORT void SetRunPanadapter(int id, int run) {
    _InterlockedExchange(&pcm->rcvr[id].run_pan, run);
}

PORT void SetXcmInrate(
    int in_id, int rate) // 2014-12-18:  called for streams 0, 1, 3, 4 (RX).
                         // Stream 2 (TX) called in CMCreateCMaster().
{
    int i, rx, tx, sp0;
    ...
    if (pcm->xcm_inrate[in_id] != rate) {
        ...
        switch (stype(in_id)) {
            case 0: // receiver
                rx = rxid(in_id);
                SetRCVRANBBuffsize(
                    0, rx, pcm->xcm_insize[in_id]); // set anb input size
                SetRCVRANBSamplerate(0, rx, rate); // set anb input rate
                SetRCVRNOBBuffsize(
                    0, rx, pcm->xcm_insize[in_id]); // set nob input size
                SetRCVRNOBSamplerate(0, rx, rate); // set nob input rate
                // set display (currently in C#)
                for (i = 0; i < pcm->cmSubRCVR; i++) {
                    SetInputSamplerate(
                        chid(in_id, i), rate); // dsp channel input rate
                    SetInputBuffsize(chid(in_id, i),
                ...
                if (rx == 0)
                    SetSiphonInsize(
                        rx, pcm->xcm_insize[in_id]); // PIPE - set siphon for
                                                     // phase2 display, RX1 only
                SetIVACiqSizeAndRate(rx, pcm->xcm_insize[in_id],
                    pcm->xcm_inrate[in_id]); // PIPE - set vacOUT size and rate
                                             // for IQ data
                break;
            case 1: // transmitter
                tx = txid(in_id);
                ...
                // there) PIPE - set wave player, rcvr1 (leave in C# since
                // player is there) PIPE - set wave recorder, rcvr0 (leave in C#
                // since recorder is there) PIPE - set wave recorder, rcvr1
                // (leave in C# since recorder is there)
                SetIVACmicSize(
                    0, pcm->xcm_insize[in_id]); // PIPE - set vacIN0 input size
                SetIVACmicRate(0, rate); // PIPE - set vacIN0 input rate
                SetIVACmicSize(
                    1, pcm->xcm_insize[in_id]); // PIPE - set vacIN1 input size
                SetIVACmicRate(1, rate); // PIPE - set vacIN1 input rate
                break;
            case 2: // special0 for stitched rx display
                sp0 = sp0id(in_id);
                SetRCVRANBBuffsize(2, sp0,
                    pcm->xcm_insize[in_id]); // PIPE - set anb input size
                SetRCVRANBSamplerate(2, sp0, rate); // PIPE - set anb input rate
                SetRCVRNOBBuffsize(2, sp0,
                    pcm->xcm_insize[in_id]); // PIPE - set nob input size
...
PORT void SetCMAudioOutrate(
    int in_id, int rate) // 2014-11-24:  NOT called by console because this is
                         // fixed at 48K by the
...
PORT void SetRcvrChannelOutrate(
    int rcvr_id, int rate, int state) // 2014-12-18:  NOT called by console as
                                      // values are set at creation and
{ //   not changed.
    int j;
    ...
    for (j = 0; j < pcm->cmSubRCVR; j++) {
        SetOutputSamplerate(chid(in_id, j),
            rate); // set DSP Channel output rate (sets size too)
        mix_in_id = mixinid(in_id, j); // set audio mixer
        SetAAudioMixState(
            0, 0, mix_in_id, 0); //	.Make this stream INACTIVE in AAMixer
        SetAAudioStreamRate(
            0, 0, mix_in_id, rate); //	.Set Mixer input rate (sets size too)
        SetAAudioMixState(0, 0, mix_in_id,
            state); //	.Conditionally make this stream ACTIVE in AAMixer
    }
...
PORT void SetXmtrChannelOutrate(int xmtr_id, int rate,
    int state) // 2014-11-24:  Called by console when TX rate is set
{
    ...
    SetOutputSamplerate(
        chid(in_id, 0), rate); // set DSP Channel output rate (sets size too)
    ...
    SetAAudioMixState(0, 0, mix_in_id,
        state); //	.Conditionally make this stream ACTIVE in AAMixer
                //
    SetTXGainSize(
        pcm->xmtr[xmtr_id].pgain, size); // set size for Penelope Gain Block
    pSetEERSize(pcm->xmtr[xmtr_id].peer, size); // set size for EER
    pSetILVInsize(
        pcm->xmtr[xmtr_id].pilv, size); // set size for Interleave & output
...
PORT void SetAntiVOXSourceStates(int txid, int streams, int states) {
    void* a = (void*)pcm->xmtr[txid].pavoxmix;
    SetAAudioMixStates(a, -1, streams, states);
}

PORT void SetAntiVOXSourceWhat(int txid, int stream, int state) {
    void* a = (void*)pcm->xmtr[txid].pavoxmix;
