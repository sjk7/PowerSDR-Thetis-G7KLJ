                ...
                if ((bptr[0] == 0x7f) && (bptr[1] == 0x7f)
                    ...
                    switch (ControlBytesIn[0] & 0xf8) {
                        ...
                        case 0x10: // C0 0001 0xxx
                            ...
                            prn->user_adc0 = ((ControlBytesIn[3] << 8) & 0xff00)
                                | (((int)(ControlBytesIn[4]))
                                    & 0xff); // AIN3 MKII PA Volts
                            break;
                        case 0x18: // C0 0001 1xxx
                            prn->user_adc1 = ((ControlBytesIn[1] << 8) & 0xff00)
                                | (((int)(ControlBytesIn[2]))
                        ...
                        case 0x20: // C0 0010 0xxx
                            ...
                            prn->adc[1].adc_overload = (ControlBytesIn[2] & 1)
                                << 1;
                            prn->adc[2].adc_overload = (ControlBytesIn[3] & 1)
                                << 2;
                            break;
                    }
                         ...
                         iddc++) // 'nddc' is the number of DDCs running
                    {
                        for (isample = 0; isample < spr; isample++) {
                            ...
                            prn->RxBuff[iddc][2 * isample + 1]
                                = const_1_div_2147483648_
                                * (double)(bptr[k + 3] << 24 | bptr[k + 4] << 16
                                    | bptr[k + 5] << 8);
                        }
                    }
                    switch (nddc) {
        ...
        switch (out_control_idx) // now m=pick the frame of control bytes
        {
            ...
            case 10:
                ...
                C2 = ((prn->mic.mic_boost & 1) | ((prn->mic.line_in & 1) << 1)
                         | ApolloFilt | ApolloTuner | ApolloATU
                         | ApolloFiltSelect | 0b01000000)
                    & 0x7f;
    ...
    while (io_keep_running != 0) {
        ...
        if (dwWait == WAIT_TIMEOUT) {
            ...
            if (consecutive_timeouts >= 10) {
                // network not responding, we are in a tizzy. All we can do is
                // notify caller and he can do what he wants.
                HaveSync = 0;
            }
            continue;
        } else {
