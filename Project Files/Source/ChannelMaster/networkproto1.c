...
int MetisReadDirect(unsigned char* bufp) {
    ...
    if (tid != 0)
        assert(mytid
            == tid); // removed locks so let's make sure we are the one and only
    ...
    int startEnter = timeGetTime();
    // EnterCriticalSection(&prn->rcvpktp1); <-- G7KLJ: there is no contention
    // for this, so why lock it?
    fromlen = sizeof(fromaddr);
...
void MetisReadThreadMainLoop(void) {
        ...
        MetisReadDirectPOLL(FPGAReadBufp);
...

            {
                ...
                for (frame = 0; frame < 2; frame++) {
                    bptr = FPGAReadBufp + (uint64_t)512 * (uint64_t)frame;
                    if ((bptr[0] == 0x7f) && (bptr[1] == 0x7f)
                        && (bptr[2] == 0x7f)) {
                        for (cb = 0; cb < 5; cb++)
                        ...
                        switch (ControlBytesIn[0] & 0xf8) {
                            case 0x00: // C0 0000 0000
                                prn->adc[0].adc_overload
                                    = ControlBytesIn[1] & 0x01;
                                prn->user_dig_in
                                    = ((ControlBytesIn[1] >> 1) & 0xf);
                                break;
                            case 0x08: // C0 0000 1xxx
                                prn->tx[0].exciter_power
                                    = ((ControlBytesIn[1] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[2]))
                                        & 0xff); // (AIN5) drive power
                                prn->tx[0].fwd_power
                                    = ((ControlBytesIn[3] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[4]))
                                        & 0xff); // (AIN1) PA coupler
                            ...
                            case 0x10: // C0 0001 0xxx
                                prn->tx[0].rev_power
                                    = ((ControlBytesIn[1] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[2]))
                                        & 0xff); // (AIN2) PA reverse power
                                PeakRevPower((float)(prn->tx[0].rev_power));
                                prn->user_adc0
                                    = ((ControlBytesIn[3] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[4]))
                                        & 0xff); // AIN3 MKII PA Volts
                                break;
                            case 0x18: // C0 0001 1xxx
                                prn->user_adc1
                                    = ((ControlBytesIn[1] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[2]))
                                        & 0xff); // AIN4 MKII PA Amps
                                prn->supply_volts
                                    = ((ControlBytesIn[3] << 8) & 0xff00)
                                    | (((int)(ControlBytesIn[4]))
                                        & 0xff); // AIN6 Hermes Volts
                                break;
                            case 0x20: // C0 0010 0xxx
                                prn->adc[0].adc_overload
                                    = ControlBytesIn[1] & 1;
                                prn->adc[1].adc_overload
                                    = (ControlBytesIn[2] & 1) << 1;
                                prn->adc[2].adc_overload
                                    = (ControlBytesIn[3] & 1) << 2;
                                break;
                        }
                        spr = 504 / (6 * nddc + 2); // samples per ddc
                        for (iddc = 0; iddc < nddc;
                             iddc++) // 'nddc' is the number of DDCs running
                        {
                            for (isample = 0; isample < spr; isample++) {
                                int k = 8 + isample * (6 * nddc + 2) + iddc * 6;
                                prn->RxBuff[iddc][2 * isample + 0]
                                    = const_1_div_2147483648_
                                    * (double)(bptr[k + 0] << 24
                                        | bptr[k + 1] << 16 | bptr[k + 2] << 8);
                                prn->RxBuff[iddc][2 * isample + 1]
                                    = const_1_div_2147483648_
                                    * (double)(bptr[k + 3] << 24
                                        | bptr[k + 4] << 16 | bptr[k + 5] << 8);
                            }
                        }
                        switch (nddc) {
                        ...
                        for (isamp = 0; isamp < spr;
                             isamp++) // for each set of samples
                        {
    ...
    void WriteMainLoop(char* bufp) {
        ...
        for (txframe = 0; txframe < 2; txframe++) {
            txbptr = FPGAWriteBufp + (uint64_t)512 * (uint64_t)txframe;
            txbptr[0] = 0x7f; // add the 3 sync bytes
            txbptr[1] = 0x7f;
            txbptr[2] = 0x7f;

            // if TX/RX has changed we need to change the DDC0 frequency for
            // Hermes-II; so jump to that C&C next
            if (XmitBit != PreviousTXBit) {
                if (nddc == 2) out_control_idx = 2;
            ...
            switch (out_control_idx) // now m=pick the frame of control bytes
            {
                case 0: // C0=0: general settings
                    ...
                    C3 = (prbpfilter->_10_dB_Atten & 1)
                        | ((prbpfilter->_20_dB_Atten << 1) & 2)
                ...
                case 1: // TX VFO
                    C0 |= 2;
                    C1 = (prn->tx[0].frequency >> 24)
                        & 0xff; // byte 0 of tx freq
                    C2 = (prn->tx[0].frequency >> 16)
                        & 0xff; // byte 1 of tx freq
                    C3 = (prn->tx[0].frequency >> 8)
                        & 0xff; // byte 2 of tx freq
                    C4 = (prn->tx[0].frequency) & 0xff; // byte 3 of tx freq
                    break;

                case 2: // RX1 VFO (DDC0)
                    C0 |= 4;
                    // DDC0 is always RX0 freqency, except if Puresignal TX with
                    // hermes-II
                ...
                case 3: // RX2 VFO (DDC1)
                    ...
                    // ADC assignments hardwired according to the configuration
                    // (see spreadsheet) orion: taken from setup form
                ...
                case 5: // RX3 VFO (DDC2)
                    C0 |= 8;
                    // if Orion, DDC2 is RX2 frequency; else TX frequency for
                    // Hermes
                    if (nddc == 5)
                ...
                case 7: // RX5 VFO (DDC4)
                    C0 |= 0x0c;
                    // DDC4 is TX frequency for Orion2 TX with puresignal,
                    // otherwise not used, so make TX always
                    ddc_freq = prn->tx[0].frequency;
                ...
                case 10:
                    ...
                    C1 = prn->tx[0].drive_level; // SWR adjustment before this
                                                 // in SetOutputPowerFactor()
                    C2 = ((prn->mic.mic_boost & 1)
                             | ((prn->mic.line_in & 1) << 1) | ApolloFilt
                             | ApolloTuner | ApolloATU | ApolloFiltSelect
                             | 0b01000000)
                        & 0x7f;
                    C3 = (prbpfilter->_13MHz_HPF & 1)
                ...
                case 12: // Step ATT control
                    ...
                    C4 = (prn->cw.keyer_weight & 0b01111111)
        ...
        memcpy(FPGAWriteBufp + 8, bufp,
            sizeof(char) * 8 * 63); // add LRIQ to frame 1recf
    ...
    DWORD WINAPI sendProtocol1Samples(LPVOID n) {
        ...
        while (io_keep_running != 0) {
            ...
            for (i = 0; i < 2 * 63;
                ...
                for (j = 0; j < 2;
                     j++) // for a sample from each set, 4 bytes per
                    for (k = 0; k < 2;
                         k++) // for each component of the sample, 2 per
                    {
                        temp = pbuffs[j][i * 2 + k] >= 0.0
                            ? (short)floor(pbuffs[j][i * 2 + k] * 32767.0 + 0.5)
                            : (short)ceil(pbuffs[j][i * 2 + k] * 32767.0 - 0.5);
                        if (prn->cw.cw_enable && j == 1)
                            temp = (prn->tx[0].dot << 2 | prn->tx[0].dash << 1
                                       | prn->tx[0].cwx)
                                & 0b00000111;
                        prn->OutBufp[8 * i + 4 * j + 2 * k + 0]
                            = (char)((temp >> 8) & 0xff);
                        prn->OutBufp[8 * i + 4 * j + 2 * k + 1]
                            = (char)(temp & 0xff);
                    }
